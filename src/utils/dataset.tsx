


export const regex = {
email:new RegExp(/^[^\s@]+@[^\s@]+\.[^\s@]+$/),
password:new RegExp(/^(?=.*[0-9])(?=.*[a-zA-Z])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{5,20}$/)
}





export const AI_prop = [

{lang:'Afrikaans',lcode:'af',lcodex:'af-ZA',codeic:'ZA',name:{male:'af-ZA-Standard-A',female:'af-ZA-Standard-A'}},

{lang:'Arabic',lcode:'ar',lcodex:'ar-XA',codeic:'SA',name:{male:'ar-XA-Chirp3-HD-Orus',female:'ar-XA-Chirp3-HD-Leda'}},

{lang:'Armenian',lcode:'hy',lcodex:'hy-AM',codeic:'AM',name:{male:'',female:''}},

{lang:'Azerbaijani',lcode:'az',lcodex:'az-AZ',codeic:'AZ',name:{male:'',female:''}},

{lang:'Bengali',lcode:'bn',lcodex:'bn-IN',codeic:'IN',name:{male:'bn-IN-Chirp3-HD-Iapetus',female:'bn-IN-Chirp3-HD-Kore'}},

{lang:'Bulgarian',lcode:'bg',lcodex:'bg-BG',codeic:'BG',name:{male:'bg-BG-Standard-B',female:'bg-BG-Standard-B'}},

{lang:'Czech',lcode:'cs',lcodex:'cs-CZ',codeic:'CZ',name:{male:'cs-CZ-Chirp3-HD-Alnilam',female:'cs-CZ-Chirp3-HD-Aoede'}},

{lang:'Danish',lcode:'da',lcodex:'da-DK',codeic:'DK',name:{male:'da-DK-Chirp3-HD-Algieba',female:'da-DK-Chirp3-HD-Autonoe'}},

{lang:'Dutch',lcode:'nl',lcodex:'nl-NL',codeic:'NL',name:{male:'nl-NL-Chirp3-HD-Zubenelgenubi',female:'nl-NL-Chirp3-HD-Vindemiatrix'}},

{lang:'English',lcode:'en',lcodex:'en-US',codeic:'US',name:{male:'en-US-Chirp-HD-D',female:'en-US-Chirp3-HD-Aoede'}},

{lang:'Estonian',lcode:'et',lcodex:'et-EE',codeic:'EE',name:{male:'et-EE-Chirp3-HD-Enceladus',female:'et-EE-Chirp3-HD-Erinome'}},

{lang:'Filipino',lcode:'fil',lcodex:'fil-PH',codeic:'PH',name:{male:'fil-PH-Standard-D',female:'fil-PH-Standard-B'}},

{lang:'Finnish',lcode:'fi',lcodex:'fi-FI',codeic:'FI',name:{male:'fi-FI-Chirp3-HD-Algenib',female:'fi-FI-Chirp3-HD-Achernar'}},

{lang:'French',lcode:'fr',lcodex:'fr-FR',codeic:'FR',name:{male:'fr-FR-Chirp3-HD-Fenrir',female:'fr-FR-Chirp3-HD-Gacrux'}},

{lang:'German',lcode:'de',lcodex:'de-DE',codeic:'DE',name:{male:'de-DE-Chirp-HD-D',female:'de-DE-Chirp-HD-O'}},

{lang:'Greek',lcode:'el',lcodex:'el-GR',codeic:'GR',name:{male:'el-GR-Chirp3-HD-Umbriel',female:'el-GR-Chirp3-HD-Sulafat'}},

{lang:'Hebrew',lcode:'he',lcodex:'he-IL',codeic:'IL',name:{male:'he-IL-Chirp3-HD-Schedar',female:'he-IL-Chirp3-HD-Sulafat'}},

{lang:'Hindi',lcode:'hi',lcodex:'hi-IN',codeic:'IN',name:{male:'',female:''}},

{lang:'Hungarian',lcode:'hu',lcodex:'hu-HU',codeic:'HU',name:{male:'hu-HU-Chirp3-HD-Sadaltager',female:'hu-HU-Chirp3-HD-Sulafat'}},

{lang:'Indonesian',lcode:'id',lcodex:'id-ID',codeic:'ID',name:{male:'id-ID-Chirp3-HD-Achird',female:'id-ID-Chirp3-HD-Achernar'}},

{lang:'Italian',lcode:'it',lcodex:'it-IT',codeic:'IT',name:{male:'it-IT-Chirp3-HD-Algenib',female:'it-IT-Chirp-HD-F'}},

{lang:'Japanese',lcode:'ja',lcodex:'ja-JP',codeic:'JP',name:{male:'ja-JP-Chirp3-HD-Charon',female:'ja-JP-Chirp3-HD-Aoede'}},

{lang:'Korean',lcode:'ko',lcodex:'ko-KR',codeic:'KR',name:{male:'ko-KR-Chirp3-HD-Umbriel',female:'ko-KR-Chirp3-HD-Sulafat'}},

{lang:'Malay',lcode:'ms',lcodex:'ms-MY',codeic:'MY',name:{male:'',female:''}},

{lang:'Mandarin',lcode:'zh',lcodex:'zh-CN',codeic:'CN',name:{male:'cmn-CN-Chirp3-HD-Umbriel',female:'cmn-CN-Chirp3-HD-Zephyr'}},

{lang:'Norwegian',lcode:'no',lcodex:'nb-NO',codeic:'NO',name:{male:'nb-NO-Chirp3-HD-Charon',female:'nb-NO-Chirp3-HD-Despina'}},

{lang:'Persian',lcode:'fa',lcodex:'fa-IR',codeic:'IR',name:{male:'',female:''}},

{lang:'Polish',lcode:'pl',lcodex:'pl-PL',codeic:'PL',name:{male:'pl-PL-Chirp3-HD-Algenib',female:'pl-PL-Chirp3-HD-Achernar'}},

{lang:'Portuguese',lcode:'pt',lcodex:'pt-PT',codeic:'PT',name:{male:'pt-PT-Standard-F',female:'pt-PT-Standard-E'}},

{lang:'Punjabi',lcode:'pa',lcodex:'pa-IN',codeic:'IN',name:{male:'pa-IN-Chirp3-HD-Sadachbia',female:'pa-IN-Chirp3-HD-Sulafat'}},

{lang:'Romanian',lcode:'ro',lcodex:'ro-RO',codeic:'RO',name:{male:'ro-RO-Standard-B',female:'ro-RO-Standard-B'}},

{lang:'Russian',lcode:'ru',lcodex:'ru-RU',codeic:'RU',name:{male:'ru-RU-Chirp3-HD-Fenrir',female:'ru-RU-Chirp3-HD-Aoede'}},

{lang:'Slovak',lcode:'sk',lcodex:'sk-SK',codeic:'SK',name:{male:'sk-SK-Chirp3-HD-Alnilam',female:'sk-SK-Chirp3-HD-Aoede'}},

{lang:'Slovenian',lcode:'sl',lcodex:'sl-SI',codeic:'SI',name:{male:'sl-SI-Chirp3-HD-Alnilam',female:'sl-SI-Chirp3-HD-Aoede'}},

{lang:'Spanish',lcode:'es',lcodex:'es-ES',codeic:'ES',name:{male:'es-ES-Chirp3-HD-Achird',female:'es-ES-Chirp-HD-O'}},

{lang:'Swahili',lcode:'sw',lcodex:'sw-TZ',codeic:'TZ',name:{male:'',female:''}},

{lang:'Swedish',lcode:'sv',lcodex:'sv-SE',codeic:'SE',name:{male:'sv-SE-Chirp3-HD-Achird',female:'sv-SE-Chirp3-HD-Achernar'}},

{lang:'Thai',lcode:'th',lcodex:'th-TH',codeic:'TH',name:{male:'th-TH-Chirp3-HD-Enceladus',female:'th-TH-Chirp3-HD-Callirrhoe'}},

{lang:'Turkish',lcode:'tr',lcodex:'tr-TR',codeic:'TR',name:{male:'tr-TR-Chirp3-HD-Algieba',female:'tr-TR-Chirp3-HD-Achernar'}},

{lang:'Ukrainian',lcode:'uk',lcodex:'uk-UA',codeic:'UA',name:{male:'uk-UA-Chirp3-HD-Algenib',female:'uk-UA-Chirp3-HD-Achernar'}},

{lang:'Urdu',lcode:'ur',lcodex:'ur-PK',codeic:'PK',name:{male:'ur-IN-Chirp3-HD-Rasalgethi',female:'ur-IN-Chirp3-HD-Sulafat'}},

{lang:'Vietnamese',lcode:'vi',lcodex:'vi-VN',codeic:'VN',name:{male:'vi-VN-Chirp3-HD-Iapetus',female:'vi-VN-Chirp3-HD-Gacrux'}},

{lang:'Yoruba',lcode:'yo',lcodex:'yo-NG',codeic:'NG',name:{male:'',female:''}},

{lang:'Igbo',lcode:'ig',lcodex:'ig-NG',codeic:'NG',name:{male:'',female:''}},

{lang:'Hausa',lcode:'ha',lcodex:'ha-NG',codeic:'NG',name:{male:'',female:''}},

{lang:'Irish',lcode:'ga',lcodex:'ga-IE',codeic:'IE',name:{male:'',female:''}},

{lang:'Zulu',lcode:'zu',lcodex:'zu-ZA',codeic:'ZA',name:{male:'',female:''}},

{lang:'Latin',lcode:'la',lcodex:'la-VA',codeic:'VA',name:{male:'',female:''}}

]








export const data = [
  { abbr:"AFG",name: "AFGHANISTAN", icon: "af",lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male: 'ar-XA-Chirp3-HD-Orus' },
  { abbr:"ALB",name: "ALBANIA", icon: "al", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"DZA",name: "ALGERIA", icon: "dz", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male: 'ar-XA-Chirp3-HD-Orus' },
  { abbr:"AGO",name: "ANGOLA", icon: "ao",lcodex: "pt-BR", female: "'pt-BR-Chirp3-HD-Aoede'", male: 'pt-BR-Chirp3-HD-Sadaltager'},
  { abbr:"ARG",name: "ARGENTINA", icon: "ar", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male:'es-ES-Chirp3-HD-Achird' },
  { abbr:"ARM",name: "ARMENIA", icon: "am", lcodex: "ru-RU", female: "ru-RU-Chirp3-HD-Aoede", male: "ru-RU-Chirp3-HD-Aoede" },
  { abbr:"AUS",name: "AUSTRALIA", icon: "au", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"AUT",name: "AUSTRIA", icon: "at", lcodex: "de-DE", female: "de-DE-Chirp-HD-O", male: "de-DE-Chirp-HD-D" },
  { abbr:"BGD",name: "BANGLADESH", icon: "bd", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"BEL",name: "BELGIUM", icon: "be", lcodex: "nl-BE", female: "nl-BE-Chirp3-HD-Despina", male: "nl-BE-Chirp3-HD-Alnilam" },
  { abbr:"BEN",name: "BENIN", icon: "bj", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:'BRA',name: "BRAZIL", icon: "br", lcodex: "pt-BR", female: "'pt-BR-Chirp3-HD-Aoede'", male: 'pt-BR-Chirp3-HD-Sadaltager'},
  { abbr:"BRN",name: "BRUNEI", icon: "bn", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"BGR",name: "BULGARIA", icon: "bg", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"BFA",name: "BURKINA FASO", icon: "bf", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },

  { abbr:"KHM",name: "CAMBODIA", icon: "kh", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"CMR",name: "CAMEROON", icon: "cm", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:"CAN",name: "CANADA", icon: "ca", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"CHL",name: "CHILE", icon: "cl", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male: 'es-ES-Chirp3-HD-Achird' },
  { abbr:"CHN",name: "CHINA", icon: "cn", lcodex:'yue-HK', female:'yue-HK-Standard-C', male:'yue-HK-Standard-B'},
  { abbr:"COL",name: "COLOMBIA", icon: "co", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male: 'es-ES-Chirp3-HD-Achird' },
  { abbr:"CRI",name: "COSTA RICA", icon: "cr", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male: 'es-ES-Chirp3-HD-Achird' },
  { abbr:"CIV",name: "CÔTE D’IVOIRE", icon: "ci", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:"HRV",name: "CROATIA", icon: "hr", lcodex: "hr-HR", female: "hr-HR-Chirp3-HD-Despina", male: "hr-HR-Chirp3-HD-Enceladus" },
  { abbr:"CUB",name: "CUBA", icon: "cu", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male: 'es-ES-Chirp3-HD-Achird' },
  { abbr:"CYP",name: "CYPRUS", icon: "cy", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },

  { abbr:"DNK",name: "DENMARK", icon: "dk", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"ECU",name: "ECUADOR", icon: "ec", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male:'es-ES-Chirp3-HD-Achird' },
  { abbr:"EGY",name: "EGYPT", icon: "eg", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male: 'ar-XA-Chirp3-HD-Orus' },
  { abbr:"GNQ",name: "EQUATORIAL GUINEA", icon: "gq", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:"EST",name: "ESTONIA", icon: "ee", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"ETH",name: "ETHIOPIA", icon: "et", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },

  { abbr:"FIN",name: "FINLAND", icon: "fi", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"FRA",name: "FRANCE", icon: "fr", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:"GAB",name: "GABON", icon: "ga", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:"GEO",name: "GEORGIA", icon: "ge", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"DEU",name: "GERMANY", icon: "de", lcodex: "de-DE", female: "de-DE-Chirp-HD-O", male: "de-DE-Chirp-HD-D" },
  { abbr:"GHA",name: "GHANA", icon: "gh", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"GRC",name: "GREECE", icon: "gr", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"GIN",name: "GUINEA", icon: "gn", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },

  { abbr:"HTI",name: "HAITI", icon: "ht", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:"HKG",name: "HONG KONG", icon: "hk", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"HUN",name: "HUNGARY", icon: "hu", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },

  { abbr:"ISL",name: "ICELAND", icon: "is", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"IND",name: "INDIA", icon: "in",lcodex:'bn-IN', female: 'bn-IN-Chirp3-HD-Sulafat', male:'bn-IN-Chirp3-HD-Sadaltager'},
  { abbr:"IDN",name: "INDONESIA", icon: "id", lcodex: "id-ID", female: "	id-ID-Chirp3-HD-Leda", male: "	id-ID-Chirp3-HD-Puck" },
  { abbr:"IRN",name: "IRAN", icon: "ir", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male: 'ar-XA-Chirp3-HD-Orus' },
  { abbr:"IRQ",name: "IRAQ", icon: "iq", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male:'ar-XA-Chirp3-HD-Orus' },
  { abbr:"IRL",name: "IRELAND", icon: "ie", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"ISR",name: "ISRAEL", icon: "il", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"ITA",name: "ITALY", icon: "it", lcodex:'it-IT', female: 'it-IT-Chirp-HD-F', male:'it-IT-Chirp3-HD-Algenib'},

  { abbr:"JAM",name: "JAMAICA", icon: "jm", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"JPN",name: "JAPAN", icon: "jp", lcodex: "ja-JP", female: 'ja-JP-Chirp3-HD-Aoede', male: 'ja-JP-Chirp3-HD-Charon'},
  { abbr:"KEN",name: "KENYA", icon: "ke", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },

  { abbr:"LBN",name: "LEBANON", icon: "lb", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male: 'ar-XA-Chirp3-HD-Orus' },
  { abbr:"LBY",name: "LIBYA", icon: "ly", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male: 'ar-XA-Chirp3-HD-Orus' },
  { abbr:"MYS",name: "MALAYSIA", icon: "my", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"MLI",name: "MALI", icon: "ml", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:"MEX",name: "MEXICO", icon: "mx", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male: 'es-ES-Chirp3-HD-Achird' },
  { abbr:"MAR",name: "MOROCCO", icon: "ma", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male: 'ar-XA-Chirp3-HD-Orus' },

  { abbr:"NLD",name: "NETHERLANDS", icon: "nl", lcodex: 'nl-NL', female: 'nl-NL-Chirp3-HD-Vindemiatrix', male:'nl-NL-Chirp3-HD-Charon'},
  { abbr:"NZL",name: "NEW ZEALAND", icon: "nz", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"NER",name: "NIGER", icon: "ne", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:"NGA",name: "NIGERIA", icon: "ng", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"NOR",name: "NORWAY", icon: "no", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },

  { abbr:"OMN",name: "OMAN", icon: "om", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male: 'ar-XA-Chirp3-HD-Orus' },
  { abbr:"PAK",name: "PAKISTAN", icon: "pk", lcodex: "pa-IN", female: "pa-IN-Chirp3-HD-Sulafat", male: "pa-IN-Chirp3-HD-Umbriel" },
  { abbr:"PER",name: "PERU", icon: "pe", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male:'es-ES-Chirp3-HD-Achird' },
  { abbr:"PHL",name: "PHILIPPINES", icon: "ph",lcodex:'fil-PH', female: 'fil-PH-Standard-B', male:'	fil-PH-Standard-D'},
  { abbr:"PRT",name: "PORTUGAL", icon: "pt", lcodex: "pt-BR", female: 'pt-BR-Chirp3-HD-Aoede', male: 'pt-BR-Chirp3-HD-Sadaltager' },
  { abbr:"PRI",name: "PUERTO RICO", icon: "pr", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male:'es-ES-Chirp3-HD-Achird' },

  { abbr:"QAT",name: "QATAR", icon: "qa", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male:'ar-XA-Chirp3-HD-Orus'},
  { abbr:"ROU",name: "ROMANIA", icon: "ro", lcodex: "ro-RO", female: "ro-RO-Chirp3-HD-Leda", male: "ro-RO-Chirp3-HD-Zubenelgenubi" },
  { abbr:"RUS",name: "RUSSIA", icon: "ru", lcodex: "ru-RU", female: "ru-RU-Chirp3-HD-Aoede", male: "ru-RU-Chirp3-HD-Aoede" },
  { abbr:"RWA",name: "RWANDA", icon: "rw", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },

  { abbr:"SAU",name: "SAUDI ARABIA", icon: "sa", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male:'ar-XA-Chirp3-HD-Orus' },
  { abbr:"SEN",name: "SENEGAL", icon: "sn", lcodex: "fr-FR", female: "fr-FR-Chirp3-HD-Gacrux", male: "fr-FR-Chirp3-HD-Fenrir" },
  { abbr:"SRB",name: "SERBIA", icon: "rs", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"SGP",name: "SINGAPORE", icon: "sg", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"ZAF",name: "SOUTH AFRICA", icon: "za", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"ESP",name: "SPAIN", icon: "es", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male: 'es-ES-Chirp3-HD-Achird' },
  { abbr:"SWE",name: "SWEDEN", icon: "se", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"CHE",name: "SWITZERLAND", icon: "ch", lcodex: "de-DE", female: "de-DE-Chirp-HD-O", male: "de-DE-Chirp-HD-D" },

  { abbr:"TZA",name: "TANZANIA", icon: "tz", lcodex: "af-ZA", female: "af-ZA-Standard-A", male: "af-ZA-Standard-A" },
  { abbr:"THA",name: "THAILAND", icon: "th", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"TUN",name: "TUNISIA", icon: "tn", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male:'ar-XA-Chirp3-HD-Orus'},
  { abbr:"TUN",name: "TURKEY", icon: "tr", lcodex: "tr-TR", female: "tr-TR-Chirp3-HD-Leda", male: "tr-TR-Chirp3-HD-Zubenelgenubi" },

  { abbr:"UGA",name: "UGANDA", icon: "ug", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"UKR",name: "UKRAINE", icon: "ua", lcodex: "uk-UA", female: "uk-UA-Chirp3-HD-Aoede", male: "uk-UA-Chirp3-HD-Rasalgethi" },
  { abbr:'ARE',name: "UNITED ARAB EMIRATES", icon: "ae", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male: "'ar-XA-Chirp3-HD-Orus'" },
  { abbr:"GBR",name: "UNITED KINGDOM", icon: "gb", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"USA",name: "UNITED STATES OF AMERICA", icon: "us", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"URY",name: "URUGUAY", icon: "uy", lcodex: "es-ES", female: 'es-ES-Chirp-HD-O', male: 'es-ES-Chirp3-HD-Achird'},

  { abbr:"VNM",name: "VIETNAM", icon: "vn", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"YEM",name: "YEMEN", icon: "ye", lcodex: "ar-XA", female: 'ar-XA-Chirp3-HD-Leda', male:'ar-XA-Chirp3-HD-Orus'},
  { abbr:"ZMB",name: "ZAMBIA", icon: "zm", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" },
  { abbr:"ZWE",name: "ZIMBABWE", icon: "zw", lcodex: "en-US", female: "en-US-Chirp3-HD-Aoede", male: "en-US-Chirp-HD-D" }
]












export const Days = [
{label:'01', value:'01'},
{label:'02', value:'02'},
{label:'03', value:'03'},
{label:'04', value:'04'},
{label:'05', value:'05'},
{label:'06', value:'06'},
{label:'07', value:'07'},
{label:'08', value:'08'},
{label:'09', value:'09'},
{label:'10', value:'10'},
{label:'11', value:'11'},
{label:'12', value:'12'},
{label:'13', value:'13'},
{label:'14', value:'14'},
{label:'15', value:'15'},
{label:'16', value:'16'},
{label:'17', value:'17'},
{label:'18', value:'18'},
{label:'19', value:'19'},
{label:'20', value:'20'},
{label:'21', value:'21'},
{label:'22', value:'22'},
{label:'23', value:'23'},
{label:'24', value:'24'},
{label:'25', value:'25'},
{label:'26', value:'26'},
{label:'27', value:'27'},
{label:'28', value:'28'},
{label:'29', value:'29'},
{label:'30', value:'30'},
{label:'31', value:'31'},
]



export const Months = [
{label:'01', value:'01'},
{label:'02', value:'02'},
{label:'03', value:'03'},
{label:'04', value:'04'},
{label:'05', value:'05'},
{label:'06', value:'06'},
{label:'07', value:'07'},
{label:'08', value:'08'},
{label:'09', value:'09'},
{label:'10', value:'10'},
{label:'11', value:'11'},
{label:'12', value:'12'},
]


export const Years = [
{label:'2020', value:'2020'},
{label:'2021', value:'2021'},
{label:'2022', value:'2022'},
{label:'2023', value:'2023'},
{label:'2024', value:'2024'},
{label:'2025', value:'2025'},

]







export const myPrivacyPolicy = [

{

languages: [{ label:'English', value:"en"}, { label:'French', value:"fr"}, {label:'German', value:"de"}, { label:'Spanish',value:"es"}, { label:'Italian', value:"it"}, {label:'Dutch', value:"nl"}, {label:'Swedish', value:"sv"}],

region:"eu",

title:{
en: "This section covers privacy policies for EU residents under GDPR.This page informs users regarding our policies with the collection, use, and disclosure of Personal Information for those who choose to use our Service. At NEWSWORLD CORPORATION, the privacy of our customers, users of our services and their personal information is of importance to us.",

fr: "Cette section couvre les politiques de confidentialité pour les résidents de l'UE conformément au RGPD. Cette page informe les utilisateurs de nos politiques concernant la collecte, l'utilisation et la divulgation des informations personnelles pour ceux qui choisissent d'utiliser notre service. Chez NEWSWORLD CORPORATION, la confidentialité de nos clients, utilisateurs de nos services et de leurs informations personnelles est importante pour nous.",


de: "Dieser Abschnitt behandelt Datenschutzrichtlinien für EU-Bürger gemäß DSGVO.Diese Seite informiert die Nutzer über unsere Richtlinien zur Erhebung, Nutzung und Offenlegung personenbezogener Daten für diejenigen, die unseren Service nutzen. Bei NEWSWORLD CORPORATION ist der Schutz der Privatsphäre unserer Kunden, Nutzer unserer Dienste und ihrer persönlichen Daten für uns von Bedeutung.",


es: "Esta sección cubre las políticas de privacidad para residentes de la UE según el RGPD.Esta página informa a los usuarios sobre nuestras políticas con respecto a la recopilación, el uso y la divulgación de información personal para aquellos que eligen usar nuestro servicio. En NEWSWORLD CORPORATION, la privacidad de nuestros clientes, usuarios de nuestros servicios y su información personal es importante para nosotros.",


it: "Questa sezione riguarda le politiche sulla privacy per i residenti dell'UE secondo il GDPR.Questa pagina informa gli utenti sulle nostre politiche relative alla raccolta, all'uso e alla divulgazione delle informazioni personali per coloro che scelgono di utilizzare il nostro servizio. In NEWSWORLD CORPORATION, la privacy dei nostri clienti, utenti dei nostri servizi e delle loro informazioni personali è importante per noi.",


nl: "Deze sectie behandelt privacybeleid voor EU-bewoners volgens de AVG.Deze pagina informeert gebruikers over ons beleid met betrekking tot het verzamelen, gebruiken en bekendmaken van persoonlijke informatie voor degenen die ervoor kiezen onze dienst te gebruiken. Bij NEWSWORLD CORPORATION is de privacy van onze klanten, gebruikers van onze diensten en hun persoonlijke gegevens belangrijk voor ons.",


sv: "Denna sektion omfattar sekretesspolicy för EU-invånare enligt GDPR. Denna sida informerar användare om våra policys för insamling, användning och utlämning av personlig information för dem som väljer att använda vår tjänst. På NEWSWORLD CORPORATION är våra kunders, användares och deras personuppgifters integritet viktig för oss."
},

dataCollect:{
en: "We collect personal information that you provide to us voluntarily, such as name, email, and preferences.",
fr: "Nous collectons les informations personnelles que vous nous fournissez volontairement, telles que le nom, l'e-mail et les préférences.",
de: "Wir sammeln personenbezogene Daten, die Sie uns freiwillig zur Verfügung stellen, wie Name, E-Mail und Präferenzen.",
es: "Recopilamos información personal que usted nos proporciona voluntariamente, como nombre, correo electrónico y preferencias.",
it: "Raccogliamo informazioni personali che ci fornisci volontariamente, come nome, e-mail e preferenze.",
nl: "We verzamelen persoonlijke gegevens die u vrijwillig aan ons verstrekt, zoals naam, e-mail en voorkeuren.",
sv: "Vi samlar in personlig information som du frivilligt tillhandahåller, såsom namn, e-post och preferenser."

},

dataUsage:{

en: "Your information is used to improve our services, personalize content, and communicate with you.",
fr: "Vos informations sont utilisées pour améliorer nos services, personnaliser le contenu et communiquer avec vous.",
de: "Ihre Informationen werden verwendet, um unsere Dienste zu verbessern, Inhalte zu personalisieren und mit Ihnen zu kommunizieren.",
es: "Su información se utiliza para mejorar nuestros servicios, personalizar contenido y comunicarse con usted.",
it: "Le tue informazioni vengono utilizzate per migliorare i nostri servizi, personalizzare i contenuti e comunicare con te.",
nl: "Uw informatie wordt gebruikt om onze diensten te verbeteren, inhoud te personaliseren en met u te communiceren.",
sv: "Din information används för att förbättra våra tjänster, anpassa innehåll och kommunicera med dig."

},

dataSharing:{
en: "We do not share your personal data with third parties without your consent, except as required by law.",
fr: "Nous ne partageons pas vos données personnelles avec des tiers sans votre consentement, sauf si la loi l'exige.",
de: "Wir geben Ihre persönlichen Daten nicht ohne Ihre Zustimmung an Dritte weiter, außer wenn dies gesetzlich vorgeschrieben ist.",
es: "No compartimos sus datos personales con terceros sin su consentimiento, excepto cuando lo requiera la ley.",
it: "Non condividiamo i tuoi dati personali con terze parti senza il tuo consenso, salvo se richiesto dalla legge.",
nl: "We delen uw persoonlijke gegevens niet met derden zonder uw toestemming, behalve indien wettelijk verplicht.",
sv: "Vi delar inte dina personuppgifter med tredje part utan ditt samtycke, förutom när lagen kräver det."
},

userRights:{
en: "Under GDPR, you have the right to access, correct, or request deletion of your data.",
fr: "En vertu du RGPD, vous avez le droit d'accéder, de corriger ou de demander la suppression de vos données.",
de: "Nach der DSGVO haben Sie das Recht, auf Ihre Daten zuzugreifen, diese zu korrigieren oder deren Löschung zu verlangen.",
es: "Bajo el RGPD, usted tiene derecho a acceder, corregir o solicitar la eliminación de sus datos.",
it: "Ai sensi del GDPR, hai il diritto di accedere, correggere o richiedere la cancellazione dei tuoi dati.",
nl: "Onder de AVG hebt u het recht om toegang te krijgen tot, correcties aan te brengen in of verwijdering van uw gegevens te verzoeken.",
sv: "Enligt GDPR har du rätt att få tillgång till, korrigera eller begära radering av dina uppgifter."
},

cookies:{
en: "We use cookies and similar tracking technologies to enhance user experience, analyze usage, and deliver personalized content and advertisements.",
fr: "Nous utilisons des cookies et des technologies de suivi similaires pour améliorer l'expérience utilisateur, analyser l'utilisation et fournir un contenu et des publicités personnalisés.",
de: "Wir verwenden Cookies und ähnliche Tracking-Technologien, um die Benutzererfahrung zu verbessern, die Nutzung zu analysieren und personalisierte Inhalte und Werbung bereitzustellen.",
es: "Utilizamos cookies y tecnologías de seguimiento similares para mejorar la experiencia del usuario, analizar el uso y ofrecer contenido y anuncios personalizados.",
it: "Utilizziamo cookie e tecnologie di tracciamento simili per migliorare l'esperienza dell'utente, analizzare l'utilizzo e fornire contenuti e pubblicità personalizzati.",
nl: "We gebruiken cookies en vergelijkbare trackingtechnologieën om de gebruikerservaring te verbeteren, het gebruik te analyseren en gepersonaliseerde inhoud en advertenties te leveren.",
sv: "Vi använder cookies och liknande spårningstekniker för att förbättra användarupplevelsen, analysera användning och leverera personligt innehåll och annonser."

},

dataRetent:{
en: "We store personal information only as long as necessary for the purposes set out in this Privacy Policy, unless a longer retention period is required or permitted by law.",
fr: "Nous conservons les informations personnelles uniquement aussi longtemps que nécessaire aux fins définies dans cette politique de confidentialité, sauf si une période de conservation plus longue est requise ou autorisée par la loi.",
de: "Wir speichern personenbezogene Daten nur so lange, wie es für die in dieser Datenschutzrichtlinie festgelegten Zwecke erforderlich ist, es sei denn, eine längere Aufbewahrungsfrist ist gesetzlich vorgeschrieben oder erlaubt.",
es: "Almacenamos la información personal solo durante el tiempo necesario para los fines establecidos en esta Política de Privacidad, a menos que la ley requiera o permita un período de retención más largo.",
it: "Conserviamo le informazioni personali solo per il tempo necessario agli scopi indicati in questa Informativa sulla privacy, salvo che la legge richieda o consenta un periodo di conservazione più lungo.",
nl: "Wij bewaren persoonlijke gegevens alleen zolang als nodig is voor de doeleinden die in dit privacybeleid zijn uiteengezet, tenzij een langere bewaartermijn wettelijk vereist of toegestaan is.",
sv: "Vi lagrar personlig information endast så länge som är nödvändigt för de syften som anges i denna integritetspolicy, om inte en längre lagringsperiod krävs eller tillåts enligt lag."
},

security:{
en: "We implement technical, administrative, and physical safeguards designed to protect your information against unauthorized access, disclosure, alteration, or destruction.",
fr: "Nous mettons en œuvre des mesures de protection techniques, administratives et physiques conçues pour protéger vos informations contre tout accès, divulgation, altération ou destruction non autorisés.",
de: "Wir setzen technische, administrative und physische Schutzmaßnahmen ein, die darauf ausgelegt sind, Ihre Daten vor unbefugtem Zugriff, Offenlegung, Änderung oder Zerstörung zu schützen.",
es: "Implementamos medidas técnicas, administrativas y físicas diseñadas para proteger su información contra el acceso, la divulgación, la alteración o la destrucción no autorizados.",
it: "Applichiamo misure tecniche, amministrative e fisiche progettate per proteggere le tue informazioni da accessi, divulgazioni, alterazioni o distruzioni non autorizzate.",
nl: "Wij treffen technische, administratieve en fysieke beveiligingsmaatregelen om uw gegevens te beschermen tegen onbevoegde toegang, openbaarmaking, wijziging of vernietiging.",
sv: "Vi implementerar tekniska, administrativa och fysiska skyddsåtgärder utformade för att skydda din information mot obehörig åtkomst, avslöjande, ändring eller förstörelse."
},

children:{
en: "Our Service is not directed to individuals under the age of 13 (or 16 in certain jurisdictions). We do not knowingly collect personal information from children.",
fr: "Notre service ne s'adresse pas aux personnes de moins de 13 ans (ou 16 ans dans certaines juridictions). Nous ne collectons pas sciemment d'informations personnelles auprès des enfants.",
de: "Unser Dienst richtet sich nicht an Personen unter 13 Jahren (oder 16 Jahren in bestimmten Gerichtsbarkeiten). Wir erheben wissentlich keine personenbezogenen Daten von Kindern.",
es: "Nuestro servicio no está dirigido a personas menores de 13 años (o 16 en algunas jurisdicciones). No recopilamos intencionadamente información personal de niños.",
it: "Il nostro servizio non è rivolto a persone di età inferiore ai 13 anni (o 16 in alcune giurisdizioni). Non raccogliamo consapevolmente informazioni personali da bambini.",
nl: "Onze dienst is niet gericht op personen jonger dan 13 jaar (of 16 jaar in sommige rechtsgebieden). Wij verzamelen niet bewust persoonlijke gegevens van kinderen.",
sv: "Vår tjänst riktar sig inte till personer under 13 år (eller 16 år i vissa jurisdiktioner). Vi samlar inte medvetet in personlig information från barn."
},

change:{
en: "We may update our Privacy Policy from time to time. We will notify you of any changes by posting the new Privacy Policy on this page and updating the 'Last Updated' date.",
fr: "Nous pouvons mettre à jour notre politique de confidentialité de temps à autre. Nous vous informerons de tout changement en publiant la nouvelle politique de confidentialité sur cette page et en mettant à jour la date de 'dernière mise à jour'.",
de: "Wir können unsere Datenschutzrichtlinie von Zeit zu Zeit aktualisieren. Wir werden Sie über Änderungen informieren, indem wir die neue Datenschutzrichtlinie auf dieser Seite veröffentlichen und das Datum 'Zuletzt aktualisiert' anpassen.",
es: "Podemos actualizar nuestra Política de Privacidad de vez en cuando. Le notificaremos cualquier cambio publicando la nueva Política de Privacidad en esta página y actualizando la fecha de 'Última actualización'.",
it: "Potremmo aggiornare la nostra Informativa sulla privacy di tanto in tanto. Ti informeremo di eventuali modifiche pubblicando la nuova Informativa sulla privacy in questa pagina e aggiornando la data 'Ultimo aggiornamento'.",
nl: "Wij kunnen ons privacybeleid van tijd tot tijd bijwerken. Wij zullen u op de hoogte stellen van wijzigingen door het nieuwe privacybeleid op deze pagina te plaatsen en de datum 'Laatst bijgewerkt' bij te werken.",
sv: "Vi kan komma att uppdatera vår integritetspolicy från tid till annan. Vi meddelar dig om ändringar genom att publicera den nya integritetspolicyn på denna sida och uppdatera datumet för 'Senast uppdaterad'."
},

contact:{
en: "If you have any questions or concerns about this Privacy Policy or our data practices, please contact us at [helpdesk@newsworldapp.org]",
fr: "Si vous avez des questions ou des préoccupations concernant cette politique de confidentialité ou nos pratiques de données, veuillez nous contacter à l'adresse [helpdesk@newsworldapp.org]",
de: "Wenn Sie Fragen oder Bedenken zu dieser Datenschutzrichtlinie oder unseren Datenpraktiken haben, kontaktieren Sie uns bitte unter [helpdesk@newsworldapp.org]",
es: "Si tiene alguna pregunta o inquietud sobre esta Política de Privacidad o nuestras prácticas de datos, contáctenos en [helpdesk@newsworldapp.org]",
it: "Se hai domande o dubbi su questa Informativa sulla privacy o sulle nostre pratiche relative ai dati, contattaci all'indirizzo [helpdesk@newsworldapp.org]",
nl: "Als u vragen of opmerkingen heeft over dit privacybeleid of onze gegevenspraktijken, neem dan contact met ons op via [helpdesk@newsworldapp.org]",
sv: "Om du har några frågor eller funderingar kring denna integritetspolicy eller vår hantering av data, vänligen kontakta oss på [helpdesk@newsworldapp.org]"

}

},





{

languages: [{ label:'English', value:'en' }, { label:'Arabic', value:"ar" }, { label:'Russian', value:"ru" }, 
{ label:'Japanese', value:"ja" }, { label:'Chinese', value:"zh" }],

region:"california",


title:{
en: "This section covers privacy policies for California residents under CCPA. This page informs users regarding our policies with the collection, use, and disclosure of Personal Information for those who choose to use our Service. At NEWSWORLD CORPORATION, the privacy of our customers, users of our services and their personal information is of importance to us.",


ar: "تغطي هذه القسم سياسات الخصوصية لسكان كاليفورنيا بموجب CCPA. تُبلغ هذه الصفحة المستخدمين بسياساتنا المتعلقة بجمع واستخدام والكشف عن المعلومات الشخصية لأولئك الذين يختارون استخدام خدمتنا. في شركة NEWSWORLD، خصوصية عملائنا ومستخدمي خدماتنا ومعلوماتهم الشخصية مهمة بالنسبة لنا.",


ru: "Этот раздел охватывает политику конфиденциальности для жителей Калифорнии в соответствии с CCPA. Эта страница информирует пользователей о нашей политике в отношении сбора, использования и раскрытия персональной информации для тех, кто выбирает использовать наши услуги. В компании NEWSWORLD конфиденциальность наших клиентов, пользователей наших услуг и их личной информации имеет важное значение.",


ja: "このセクションでは、CCPAに基づくカリフォルニア州の居住者向けのプライバシーポリシーについて説明します。 このページでは、当社のサービスを利用することを選択したユーザーの個人情報の収集、使用、開示に関するポリシーについてお知らせします。NEWSWORLD株式会社では、顧客、サービス利用者、その個人情報のプライバシーを重視しています。",


zh: "本部分涵盖加利福尼亚居民的隐私政策，根据CCPA规定。本页面告知用户有关我们在收集、使用和披露个人信息方面的政策，适用于选择使用我们服务的用户。在NEWSWORLD公司，我们重视客户、服务用户及其个人信息的隐私。"


},

dataCollect:{
en: "We collect personal information such as name, email, and browsing activity when you use our services.",
ar: "نجمع المعلومات الشخصية مثل الاسم والبريد الإلكتروني ونشاط التصفح عند استخدامك لخدماتنا.",
"ru": "Мы собираем личную информацию, такую как имя, адрес электронной почты и действия при просмотре, когда вы пользуетесь нашими услугами.",
"ja": "当社のサービスをご利用いただく際に、氏名、メールアドレス、閲覧履歴などの個人情報を収集します。",
"zh": "当您使用我们的服务时，我们会收集诸如姓名、电子邮件和浏览活动等个人信息。"
},

dataUsage:{
en: "Your information is used to enhance service quality, provide personalized content, and communicate updates.",
ar: "تُستخدم معلوماتك لتحسين جودة الخدمة، وتقديم محتوى مخصص، والتواصل بشأن التحديثات.",
ru: "Ваша информация используется для повышения качества услуг, предоставления персонализированного контента и уведомлений о обновлениях.",
ja: "お客様の情報は、サービスの品質向上、パーソナライズされたコンテンツの提供、更新情報の通知に使用されます。",
zh: "您的信息用于提升服务质量、提供个性化内容以及传达更新信息。"
},

dataSharing:{
en: "We do not sell your personal information. Third-party sharing occurs only when legally required or with your consent.",
ar: "نحن لا نبيع معلوماتك الشخصية. يتم مشاركة البيانات مع أطراف ثالثة فقط عند الضرورة القانونية أو بموافقتك.",
ru: "Мы не продаем вашу личную информацию. Передача третьим лицам происходит только в случае требования закона или с вашего согласия.",
ja: "当社はお客様の個人情報を販売しません。第三者への共有は、法的に必要な場合またはお客様の同意がある場合のみ行われます。",
zh: "我们不会出售您的个人信息。仅在法律要求或您同意的情况下与第三方共享。"
},


userRights:{
en: "Under CCPA, California residents can request access, deletion, or opt-out of the sale of personal information.",
ar: "بموجب قانون CCPA، يمكن لسكان كاليفورنيا طلب الوصول إلى المعلومات الشخصية أو حذفها أو رفض بيعها.",
ru: "В соответствии с CCPA жители Калифорнии могут запросить доступ к личной информации, ее удаление или отказ от продажи.",
ja: "CCPAに基づき、カリフォルニア州の居住者は個人情報へのアクセス、削除、販売のオプトアウトを要求できます。",
zh: "根据CCPA，加州居民可以请求访问、删除或选择不出售个人信息。"
},

cookies:{
en: "We use cookies and similar tracking technologies to enhance user experience, analyze usage, and deliver personalized content and advertisements.",
ar: "نستخدم ملفات تعريف الارتباط وتقنيات التتبع المشابهة لتحسين تجربة المستخدم، وتحليل الاستخدام، وتقديم محتوى وإعلانات مخصصة.",
ru: "Мы используем файлы cookie и аналогичные технологии отслеживания, чтобы улучшить пользовательский опыт, проанализировать использование и предоставлять персонализированный контент и рекламу.",
ja: "当社は、ユーザー体験を向上させ、利用状況を分析し、パーソナライズされたコンテンツや広告を提供するために、クッキーや類似の追跡技術を使用します。",
zh: "我们使用 Cookie 和类似的跟踪技术来提升用户体验、分析使用情况，并提供个性化的内容和广告。"

},


dataRetent:{

en: "We store personal information only as long as necessary for the purposes set out in this Privacy Policy, unless a longer retention period is required or permitted by law.",
ar: "نخزن المعلومات الشخصية فقط طالما كان ذلك ضروريًا للأغراض الموضحة في سياسة الخصوصية هذه، ما لم يتطلب القانون أو يسمح بفترة احتفاظ أطول.",
ru: "Мы храним личную информацию только столько времени, сколько необходимо для целей, изложенных в настоящей Политике конфиденциальности, если более длительный срок хранения не требуется или не разрешен законом.",
ja: "当社は、このプライバシーポリシーに記載された目的のために必要な期間のみ個人情報を保管し、法律でより長い保存期間が要求または許可されている場合を除きます。",
zh: "我们仅在本隐私政策所述目的所需的时间内存储个人信息，除非法律要求或允许更长的保存期限。"

},

security:{

en: "We implement technical, administrative, and physical safeguards designed to protect your information against unauthorized access, disclosure, alteration, or destruction.",
ar: "نطبق تدابير تقنية وإدارية ومادية مصممة لحماية معلوماتك من الوصول أو الكشف أو التعديل أو التدمير غير المصرح به.",
ru: "Мы внедряем технические, административные и физические меры защиты, предназначенные для защиты вашей информации от несанкционированного доступа, раскрытия, изменения или уничтожения.",
ja: "当社は、お客様の情報を不正アクセス、開示、改ざん、破壊から保護するために設計された技術的、管理的および物理的な保護策を実施しています。",
zh: "我们实施技术、管理和物理保护措施，以防止您的信息遭到未经授权的访问、披露、篡改或销毁。"

},

children:{
en: "Our Service is not directed to individuals under the age of 13 (or 16 in certain jurisdictions). We do not knowingly collect personal information from children.",
ar: "خدمتنا ليست موجهة للأفراد الذين تقل أعمارهم عن 13 عامًا (أو 16 عامًا في بعض الولايات القضائية). نحن لا نجمع عن قصد معلومات شخصية من الأطفال.",
ru: "Наш сервис не предназначен для лиц младше 13 лет (или 16 лет в некоторых юрисдикциях). Мы сознательно не собираем личную информацию от детей.",
ja: "当社のサービスは13歳未満（または一部の法域では16歳未満）の方を対象としていません。私たちは子供から個人情報を意図的に収集しません。",
zh: "我们的服务不针对13岁以下（或在某些司法管辖区为16岁以下）的个人。我们不会故意收集儿童的个人信息。"
},


change:{
en: "We may update our Privacy Policy from time to time. We will notify you of any changes by posting the new Privacy Policy on this page and updating the 'Last Updated' date.",
ar: "قد نقوم بتحديث سياسة الخصوصية الخاصة بنا من حين لآخر. سنخطرك بأي تغييرات عن طريق نشر سياسة الخصوصية الجديدة على هذه الصفحة وتحديث تاريخ 'آخر تحديث'.",
ru: "Мы можем время от времени обновлять нашу Политику конфиденциальности. Мы уведомим вас о любых изменениях, разместив новую Политику конфиденциальности на этой странице и обновив дату 'Последнее обновление'.",
ja: "当社はプライバシーポリシーを随時更新する場合があります。変更があった場合は、このページに新しいプライバシーポリシーを掲載し、「最終更新日」を更新することでお知らせします。",
zh: "我们可能会不时更新我们的隐私政策。我们会通过在此页面上发布新的隐私政策并更新“最后更新”日期来通知您任何更改。"
},

contact:{
en: "If you have any questions or concerns about this Privacy Policy or our data practices, please contact us at [helpdesk@newsworldapp.org].",
ar: "إذا كانت لديك أي أسئلة أو مخاوف بشأن سياسة الخصوصية هذه أو ممارساتنا المتعلقة بالبيانات، يرجى الاتصال بنا على [helpdesk@newsworldapp.org].",
ru: "Если у вас есть вопросы или опасения по поводу этой Политики конфиденциальности или наших методов обработки данных, пожалуйста, свяжитесь с нами по адресу [helpdesk@newsworldapp.org].",
ja: "本プライバシーポリシーまたは当社のデータ取り扱いに関してご質問やご懸念がある場合は、[helpdesk@newsworldapp.org] までご連絡ください。",
zh: "如果您对本隐私政策或我们的数据处理方式有任何疑问或担忧，请通过 [helpdesk@newsworldapp.org] 与我们联系。"
}

},




{

languages: [{ label:'Portugese', value:"pt"}, { label:'English', value:"en"}, { label:'Arabic', value:"ar"},{ label:'Russian',value:"ru" }, { label:"Japanese", value:"ja" }, { label:'Chinese', value:"zh" }],

region:"brazil",


title:{
pt: "Esta seção cobre políticas de privacidade para residentes do Brasil sob a LGPD. Esta página informa os usuários sobre nossas políticas de coleta, uso e divulgação de informações pessoais para aqueles que escolhem utilizar nosso serviço. Na NEWSWORLD CORPORATION, a privacidade de nossos clientes, usuários de nossos serviços e suas informações pessoais é importante para nós.",


en: "This section covers privacy policies for Brazilian residents under LGPD. This page informs users regarding our policies with the collection, use, and disclosure of Personal Information for those who choose to use our Service. At NEWSWORLD CORPORATION, the privacy of our customers, users of our services and their personal information is of importance to us.",


ar: "يغطي هذا القسم سياسات الخصوصية للمقيمين في البرازيل بموجب LGPD.تُبلغ هذه الصفحة المستخدمين بسياساتنا المتعلقة بجمع واستخدام والكشف عن المعلومات الشخصية لأولئك الذين يختارون استخدام خدمتنا. في شركة NEWSWORLD، خصوصية عملائنا ومستخدمي خدماتنا ومعلوماتهم الشخصية مهمة بالنسبة لنا.",


ru: "Этот раздел охватывает политику конфиденциальности для жителей Бразилии в соответствии с LGPD. Эта страница информирует пользователей о нашей политике в отношении сбора, использования и раскрытия персональной информации для тех, кто выбирает использовать наши услуги. В компании NEWSWORLD конфиденциальность наших клиентов, пользователей наших услуг и их личной информации имеет важное значение.",


ja: "このセクションでは、LGPDに基づくブラジル居住者向けのプライバシーポリシーについて説明します。このページでは、当社のサービスを利用することを選択したユーザーの個人情報の収集、使用、開示に関するポリシーについてお知らせします。NEWSWORLD株式会社では、顧客、サービス利用者、その個人情報のプライバシーを重視しています。",


zh: "本部分涵盖巴西居民的隐私政策，根据LGPD规定。本页面告知用户有关我们在收集、使用和披露个人信息方面的政策，适用于选择使用我们服务的用户。在NEWSWORLD公司，我们重视客户、服务用户及其个人信息的隐私。"
},

dataCollect:{
pt: "Coletamos informações pessoais que você fornece voluntariamente, como nome, e-mail e preferências.",
en: "We collect personal information that you provide voluntarily, such as name, email, and preferences.",
ar: "نجمع المعلومات الشخصية التي تقدمها طواعية، مثل الاسم والبريد الإلكتروني والتفضيلات.",
ru: "Мы собираем личную информацию, которую вы предоставляете добровольно, например имя, электронную почту и предпочтения.",
ja: "氏名、メールアドレス、設定など、お客様が自主的に提供する個人情報を収集します。",
zh: "我们收集您自愿提供的个人信息，例如姓名、电子邮件和偏好。"
},

dataUsage:{
pt: "Suas informações são usadas para melhorar nossos serviços, personalizar conteúdo e nos comunicar com você.",
en: "Your information is used to improve our services, personalize content, and communicate with you.",
ar: "تُستخدم معلوماتك لتحسين خدماتنا وتخصيص المحتوى والتواصل معك.",
ru: "Ваша информация используется для улучшения наших услуг, персонализации контента и связи с вами.",
ja: "お客様の情報は、サービスの向上、コンテンツのパーソナライズ、およびコミュニケーションに使用されます。",
zh: "您的信息用于改进我们的服务、个性化内容并与您沟通。"
},

dataSharing:{
pt: "Não compartilhamos seus dados pessoais com terceiros sem seu consentimento, exceto quando exigido por lei.",
en: "We do not share your personal data with third parties without your consent, except as required by law.",
ar: "لا نشارك بياناتك الشخصية مع أطراف ثالثة دون موافقتك، إلا إذا كان القانون يقتضي ذلك.",
ru: "Мы не передаем ваши персональные данные третьим лицам без вашего согласия, кроме случаев, когда это требуется законом.",
ja: "お客様の個人情報は、法的に必要な場合を除き、第三者と共有しません。",
zh: "我们不会在未经您同意的情况下与第三方共享您的个人数据，法律要求除外。"
},


userRights:{
pt: "De acordo com a LGPD, você tem direito de acessar, corrigir ou solicitar a exclusão de seus dados.",
en: "Under LGPD, you have the right to access, correct, or request deletion of your data.",
ar: "بموجب LGPD، لديك الحق في الوصول إلى بياناتك وتصحيحها أو طلب حذفها.",
ru: "В соответствии с LGPD вы имеете право на доступ, исправление или удаление своих данных.",
ja: "LGPDに基づき、お客様はデータへのアクセス、修正、削除を要求する権利があります。",
zh: "根据LGPD，您有权访问、更正或请求删除您的数据。"
},


cookies:{
pt: "Utilizamos cookies e tecnologias de rastreamento semelhantes para melhorar a experiência do usuário, analisar o uso e fornecer conteúdo e anúncios personalizados.",
en: "We use cookies and similar tracking technologies to enhance user experience, analyze usage, and deliver personalized content and advertisements.",
ar: "نستخدم ملفات تعريف الارتباط وتقنيات التتبع المشابهة لتحسين تجربة المستخدم، وتحليل الاستخدام، وتقديم محتوى وإعلانات مخصصة.",
ru: "Мы используем файлы cookie и аналогичные технологии отслеживания, чтобы улучшить пользовательский опыт, проанализировать использование и предоставлять персонализированный контент и рекламу.",
ja: "当社は、ユーザー体験を向上させ、利用状況を分析し、パーソナライズされたコンテンツや広告を提供するために、クッキーや類似の追跡技術を使用します。",
zh: "我们使用 Cookie 和类似的跟踪技术来提升用户体验、分析使用情况，并提供个性化的内容和广告。"

},

dataRetent:{
pt: "Armazenamos informações pessoais apenas pelo tempo necessário para os fins estabelecidos nesta Política de Privacidade, a menos que um período de retenção mais longo seja exigido ou permitido por lei.",
en: "We store personal information only as long as necessary for the purposes set out in this Privacy Policy, unless a longer retention period is required or permitted by law.",
ar: "نخزن المعلومات الشخصية فقط طالما كان ذلك ضروريًا للأغراض الموضحة في سياسة الخصوصية هذه، ما لم يتطلب القانون أو يسمح بفترة احتفاظ أطول.",
ru: "Мы храним личную информацию только столько времени, сколько необходимо для целей, изложенных в настоящей Политике конфиденциальности, если более длительный срок хранения не требуется или не разрешен законом.",
ja: "当社は、このプライバシーポリシーに記載された目的のために必要な期間のみ個人情報を保管し、法律でより長い保存期間が要求または許可されている場合を除きます。",
zh: "我们仅在本隐私政策所述目的所需的时间内存储个人信息，除非法律要求或允许更长的保存期限。"
},

security:{
pt: "Implementamos medidas técnicas, administrativas e físicas projetadas para proteger suas informações contra acesso, divulgação, alteração ou destruição não autorizados.",
en: "We implement technical, administrative, and physical safeguards designed to protect your information against unauthorized access, disclosure, alteration, or destruction.",
ar: "نطبق تدابير تقنية وإدارية ومادية مصممة لحماية معلوماتك من الوصول أو الكشف أو التعديل أو التدمير غير المصرح به.",
ru: "Мы внедряем технические, административные и физические меры защиты, предназначенные для защиты вашей информации от несанкционированного доступа, раскрытия, изменения или уничтожения.",
ja: "当社は、お客様の情報を不正アクセス、開示、改ざん、破壊から保護するために設計された技術的、管理的および物理的な保護策を実施しています。",
zh: "我们实施技术、管理和物理保护措施，以防止您的信息遭到未经授权的访问、披露、篡改或销毁。"

},

children:{
pt: "Nosso serviço não se destina a indivíduos com menos de 13 anos (ou 16 em determinadas jurisdições). Não coletamos intencionalmente informações pessoais de crianças.",
en: "Our Service is not directed to individuals under the age of 13 (or 16 in certain jurisdictions). We do not knowingly collect personal information from children.",
ar: "خدمتنا ليست موجهة للأفراد الذين تقل أعمارهم عن 13 عامًا (أو 16 عامًا في بعض الولايات القضائية). نحن لا نجمع عن قصد معلومات شخصية من الأطفال.",
ru: "Наш сервис не предназначен для лиц младше 13 лет (или 16 лет в некоторых юрисдикциях). Мы сознательно не собираем личную информацию от детей.",
ja: "当社のサービスは13歳未満（または一部の法域では16岁未満）の方を対象としていません。私たちは子供から個人情報を意図的に収集しません。",
zh: "我们的服务不针对13岁以下（或在某些司法管辖区为16岁以下）的个人。我们不会故意收集儿童的个人信息。"
},

change:{
pt: "Podemos atualizar nossa Política de Privacidade de tempos em tempos. Notificaremos você sobre quaisquer alterações publicando a nova Política de Privacidade nesta página e atualizando a data da 'Última atualização'.",
en: "We may update our Privacy Policy from time to time. We will notify you of any changes by posting the new Privacy Policy on this page and updating the 'Last Updated' date.",
ar: "قد نقوم بتحديث سياسة الخصوصية الخاصة بنا من حين لآخر. سنخطرك بأي تغييرات عن طريق نشر سياسة الخصوصية الجديدة على هذه الصفحة وتحديث تاريخ 'آخر تحديث'.",
ru: "Мы можем время от времени обновлять нашу Политику конфиденциальности. Мы уведомим вас о любых изменениях, разместив новую Политику конфиденциальности на этой странице и обновив дату 'Последнее обновление'.",
ja: "当社はプライバシーポリシーを随時更新する場合があります。変更があった場合は、このページに新しいプライバシーポリシーを掲載し、「最終更新日」を更新することでお知らせします。",
zh: "我们可能会不时更新我们的隐私政策。我们会通过在此页面上发布新的隐私政策并更新“最后更新”日期来通知您任何更改。"
},

contact:{
pt: "Se você tiver alguma dúvida ou preocupação sobre esta Política de Privacidade ou nossas práticas de dados, entre em contato conosco pelo e-mail [helpdesk@newsworldapp.org].",
en: "If you have any questions or concerns about this Privacy Policy or our data practices, please contact us at [helpdesk@newsworldapp.org].",
ar: "إذا كانت لديك أي أسئلة أو مخاوف بشأن سياسة الخصوصية هذه أو ممارساتنا المتعلقة بالبيانات، يرجى الاتصال بنا على [helpdesk@newsworldapp.org].",
ru: "Если у вас есть вопросы или опасения по поводу этой Политики конфиденциальности или наших методов обработки данных, пожалуйста, свяжитесь с нами по адресу [helpdesk@newsworldapp.org].",
ja: "本プライバシーポリシーまたは当社のデータ取り扱いに関してご質問やご懸念がある場合は、[helpdesk@newsworldapp.org] までご連絡ください。",
zh: "如果您对本隐私政策或我们的数据处理方式有任何疑问或担忧，请通过 [helpdesk@newsworldapp.org] 与我们联系。"
}




},



{

languages: [{ label:'English', value:"en" }, { label:'Hausa', value:"ha" }, { label:'Yoruba', value:"yo" }, { label:'Igbo', value:"ig" }, { label:'Arabic', value:"ar" }],

region:"nigeria",


title:{
en: "This section covers privacy policies for Nigerian residents and users. This page informs users regarding our policies with the collection, use, and disclosure of Personal Information for those who choose to use our Service. At NEWSWORLD CORPORATION, the privacy of our customers, users of our services and their personal information is of importance to us.",


ha: "Wannan sashe yana rufe manufofin sirri ga mazauna Najeriya da masu amfani.Wannan shafi yana sanar da masu amfani game da manufofinmu na tattara, amfani da kuma bayyana Bayanan Sirri ga waɗanda suka zaɓi amfani da Ayyukanmu. A NEWSWORLD CORPORATION, sirrin abokan cinikinmu, masu amfani da ayyukanmu da bayanan su yana da muhimmanci a gare mu.",


yo: "Ẹ̀ka yìí bo àwọn ìlànà àṣírí fún àwọn olùgbé Nàìjíríà àti àwọn oníṣe. Ojú-ìwé yìí sọ fún àwọn oníṣe nípa àwọn ìlànà wa nípa ìkójọ, lílo àti ìṣàfihàn Alaye Àtọkànwá fún àwọn tí wọ́n bá yan láti lo Iṣẹ́ wa. Ní NEWSWORLD CORPORATION, ìpamọ́ àwọn oníbàárà wa, àwọn olùlo iṣẹ́ wa àti àwọn alaye tiwọn jẹ́ ohun pataki fún wa.",


ig: "Nke a ngalaba na-ekpuchi iwu nzuzo maka ndị bi na Nigeria na ndị ọrụ.Ihe a na-akpọ ibe a na-agwa ndị ọrụ gbasara iwu anyị na nchịkọta, ojiji na nkwupụta nke Ozi Nkeonwe maka ndị họrọ iji Ọrụ anyị. Na NEWSWORLD CORPORATION, nzuzo nke ndị ahịa anyị, ndị ọrụ ọrụ anyị na ozi nkeonwe ha bụ ihe dị mkpa nye anyị.",


ar: "يغطي هذا القسم سياسات الخصوصية للمقيمين والمستخدمين في نيجيريا.تُبلغ هذه الصفحة المستخدمين بسياساتنا المتعلقة بجمع واستخدام والكشف عن المعلومات الشخصية لأولئك الذين يختارون استخدام خدمتنا. في شركة NEWSWORLD، خصوصية عملائنا ومستخدمي خدماتنا ومعلوماتهم الشخصية مهمة بالنسبة لنا."
},

dataCollect:{
en: "We collect personal information such as name, email, and preferences when you use our services.",
ha: "Muna tattara bayanan mutum kamar suna, imel, da abubuwan da kuka fi so lokacin da kuke amfani da ayyukanmu.",
yo: "A a gba alaye ti ara ẹni gẹgẹbi orukọ, imeeli, ati awọn ayanfẹ nigbati o ba nlo awọn iṣẹ wa.",
ig: "Anyị na-anakọta ozi nkeonwe dị ka aha, email, na ihe masịrị gị mgbe ị na-eji ọrụ anyị.",
ar: "نجمع المعلومات الشخصية مثل الاسم والبريد الإلكتروني والتفضيلات عند استخدامك لخدماتنا."
},

dataUsage:{
en: "Your information is used to improve our services, provide personalized content, and communicate with you.",
ha: "Bayanan ku ana amfani da su don inganta ayyukanmu, samar da abun ciki na musamman, da sadarwa da ku.",
yo: "Alaye rẹ ni a lo lati mu iṣẹ wa dara, pese akoonu ti ara ẹni, ati ba ọ sọrọ.",
ig: "A na-eji ozi gị eme ka ọrụ anyị ka mma, nye ọdịnaya ahaziri iche, ma kọọrọ gị.",
ar: "تُستخدم معلوماتك لتحسين خدماتنا وتقديم محتوى مخصص والتواصل معك."
},

dataSharing:{
en: "We do not share your personal information with third parties without your consent, except when required by law.",
ha: "Ba mu raba bayanan ku na sirri da wasu ba tare da izinin ku ba, sai dai idan doka ta bukata.",
yo: "A ko pin alaye ti ara ẹni rẹ pẹlu awọn ẹni-kẹta laisi igbanilaaye rẹ, ayafi nigbati ofin ba nilo.",
ig: "Anyị anaghị ekekọrịta ozi nkeonwe gị na ndị ọzọ na-enweghị ikike gị, ma e wezụga mgbe iwu chọrọ ya.",
ar: "لا نشارك معلوماتك الشخصية مع أطراف ثالثة دون موافقتك، إلا إذا كانت هناك متطلبات قانونية."
},


userRights:{
en: "You have the right to access, correct, or request deletion of your personal information.",
ha: "Kuna da hakkin samun damar bayanan ku, gyara su, ko neman goge su.",
yo: "O ni ẹtọ lati wọle si alaye rẹ, ṣe atunṣe rẹ, tabi beere fun piparẹ rẹ.",
ig: "Ị nwere ikike ịnweta ozi nkeonwe gị, dozie ya, ma ọ bụ rịọ ka ewepụ ya.",
ar: "لديك الحق في الوصول إلى معلوماتك الشخصية وتصحيحها أو طلب حذفها."
},

cookies:{
en: "We use cookies and similar tracking technologies to enhance user experience, analyze usage, and deliver personalized content and advertisements.",
ha: "Muna amfani da kukis da makamancin fasahar bin diddigi don ƙara ingancin ƙwarewar mai amfani, bincika amfani, da samar da abun ciki da tallace-tallace na musamman.",
yo: "A nlo àwọn kuki àti imọ̀ ẹrọ ìtẹ̀lé tí ó jọra láti mú ìrírí oníṣe dára síi, ṣe ìtúpalẹ̀ ìlò, àti pèsè àkóónú àti ìpolówó ti adani.",
ig: "Anyị na-eji kuki na teknụzụ ịchụso ndị yiri ya eme ka ahụmịhe onye ọrụ dịkwuo mma, nyochaa ojiji, ma nyefee ọdịnaya na mgbasa ozi ahaziri iche.",
ar: "نستخدم ملفات تعريف الارتباط وتقنيات التتبع المشابهة لتحسين تجربة المستخدم، وتحليل الاستخدام، وتقديم محتوى وإعلانات مخصصة."
},


dataRetent:{
en: "We store personal information only as long as necessary for the purposes set out in this Privacy Policy, unless a longer retention period is required or permitted by law.",
ha: "Muna adana bayanan sirri ne kawai muddin ya zama dole don dalilan da aka bayyana a cikin wannan Manufar Sirri, sai dai idan doka ta buƙaci ko ta yarda da tsawon lokaci.",
yo: "A n tọju alaye ti ara ẹni nikan fun igba to ba wulo fun awọn idi ti a ṣalaye ninu Ilana Asiri yii, ayafi ti ofin ba nilo tabi gba akoko ipamọ gigun sii.",
ig: "Anyị na-echekwa ozi nkeonwe naanị oge achọrọ maka ebumnuche e depụtara na Iwu Nzuzo a, belụsọ ma iwu chọrọ ma ọ bụ kwere ka echekwa ya ogologo oge.",
ar: "نخزن المعلومات الشخصية فقط طالما كان ذلك ضروريًا للأغراض الموضحة في سياسة الخصوصية هذه، ما لم يتطلب القانون أو يسمح بفترة احتفاظ أطول."
},


security:{
en: "We implement technical, administrative, and physical safeguards designed to protect your information against unauthorized access, disclosure, alteration, or destruction.",
ha: "Muna aiwatar da kariyar fasaha, gudanarwa da jiki don kare bayananka daga samun dama ba tare da izini ba, bayyana, canzawa ko rushewa.",
yo: "A n ṣe imuse awọn aabo imọ-ẹrọ, iṣakoso ati ti ara lati daabobo alaye rẹ lodi si wiwọle, ifihan, iyipada tabi iparun ti ko ni aṣẹ.",
ig: "Anyị na-eme atụmatụ nchedo teknụzụ, nchịkwa na anụ ahụ e mere iji chebe ozi gị pụọ na nnweta na-enweghị ikike, mkpughe, mgbanwe ma ọ bụ mbibi.",
ar: "نطبق تدابير تقنية وإدارية ومادية مصممة لحماية معلوماتك من الوصول أو الكشف أو التعديل أو التدمير غير المصرح به."

},

children:{
en: "Our Service is not directed to individuals under the age of 13 (or 16 in certain jurisdictions). We do not knowingly collect personal information from children.",
ha: "Ayyukanmu ba ya nufin yara ƙasa da shekara 13 (ko 16 a wasu wurare). Ba mu taɓa tara bayanan sirri daga yara da gangan ba.",
yo: "Iṣẹ wa ko ni itọka si awọn ọmọde labẹ ọdun 13 (tabi 16 ni diẹ ninu awọn agbegbe). A ko mọọmọ gba alaye ti ara ẹni lati ọdọ awọn ọmọde.",
ig: "Ọrụ anyị adịghị maka ụmụaka n'okpuru afọ 13 (ma ọ bụ 16 n'ọtụtụ ebe iwu). Anyị anaghị anakọta ozi nkeonwe sitere n'aka ụmụaka n'echiche.",
ar: "خدمتنا ليست موجهة للأفراد الذين تقل أعمارهم عن 13 عامًا (أو 16 عامًا في بعض الولايات القضائية). نحن لا نجمع عن قصد معلومات شخصية من الأطفال."
},

change:{
en: "We may update our Privacy Policy from time to time. We will notify you of any changes by posting the new Privacy Policy on this page and updating the 'Last Updated' date.",
ha: "Zai yiwu mu sabunta Manufar Sirrinmu lokaci zuwa lokaci. Za mu sanar da ku duk wani canji ta hanyar sanya sabon Manufar Sirri a wannan shafi da sabunta ranar 'An Sabunta Karshe'.",
yo: "A le ṣe imudojuiwọn Ilana Asiri wa lẹ́ẹ̀kọọ̀kan. A ó sọ fún ọ nípa àwọn ayipada kankan nipa fífi Ilana Asiri tuntun sí ojúewé yìí àti imudojúìwọ̀n ọjọ́ 'Imudojuiwọn Tó Kẹhin'.",
ig: "Anyị nwere ike imelite Atụmatụ Nzuzo anyị oge ụfọdụ. Anyị ga-agwa gị maka mgbanwe ọ bụla site n'ịtinye Atụmatụ Nzuzo ọhụrụ a na ibe a ma melite ụbọchị 'Emelitere Ikpeazụ'.",
ar: "قد نقوم بتحديث سياسة الخصوصية الخاصة بنا من حين لآخر. سنخطرك بأي تغييرات عن طريق نشر سياسة الخصوصية الجديدة على هذه الصفحة وتحديث تاريخ 'آخر تحديث'."

},

contact:{
en: "If you have any questions or concerns about this Privacy Policy or our data practices, please contact us at [helpdesk@newsworldapp.org].",
ha: "Idan kuna da wasu tambayoyi ko damuwa game da wannan Manufar Sirri ko hanyoyinmu na sarrafa bayanai, da fatan za ku tuntube mu a [helpdesk@newsworldapp.org].",
yo: "Ti o ba ni ibeere tabi aniyan nipa Ilana Asiri yii tabi bi a ṣe n lo data rẹ, jọwọ kan si wa ni [helpdesk@newsworldapp.org].",
ig: "Ọ bụrụ na ịnwere ajụjụ ma ọ bụ nchegbu gbasara Atụmatụ Nzuzo a ma ọ bụ omume anyị banyere data, biko kpọtụrụ anyị na [helpdesk@newsworldapp.org].",
ar: "إذا كانت لديك أي أسئلة أو مخاوف بشأن سياسة الخصوصية هذه أو ممارساتنا المتعلقة بالبيانات، يرجى الاتصال بنا على [helpdesk@newsworldapp.org]."
}

},



{

languages: [{ label:'English', value:"en" }, { label:'Arabic', value:"ar" }, { label:'Russian', value:"ru" }, { label:'Japanese', value:"ja" }, { label:'Chinese', value:"zh" }],

region:"others",


title:{
en: "This section covers privacy policies for all other regions not explicitly listed. This page informs users regarding our policies with the collection, use, and disclosure of Personal Information for those who choose to use our Service. At NEWSWORLD CORPORATION, the privacy of our customers, users of our services and their personal information is of importance to us.",


ar: "يغطي هذا القسم سياسات الخصوصية لجميع المناطق الأخرى غير المدرجة صراحة.تُبلغ هذه الصفحة المستخدمين بسياساتنا المتعلقة بجمع واستخدام والكشف عن المعلومات الشخصية لأولئك الذين يختارون استخدام خدمتنا. في شركة NEWSWORLD، خصوصية عملائنا ومستخدمي خدماتنا ومعلوماتهم الشخصية مهمة بالنسبة لنا.",


ru: "Этот раздел охватывает политику конфиденциальности для всех остальных регионов, не указанных явно.Эта страница информирует пользователей о нашей политике в отношении сбора, использования и раскрытия персональной информации для тех, кто выбирает использовать наши услуги. В компании NEWSWORLD конфиденциальность наших клиентов, пользователей наших услуг и их личной информации имеет важное значение.",


ja: "このセクションでは、明示的に記載されていないその他の地域のプライバシーポリシーについて説明します。このページでは、当社のサービスを利用することを選択したユーザーの個人情報の収集、使用、開示に関するポリシーについてお知らせします。NEWSWORLD株式会社では、顧客、サービス利用者、その個人情報のプライバシーを重視しています。",


zh: "本部分涵盖未明确列出的其他地区的隐私政策。本页面告知用户有关我们在收集、使用和披露个人信息方面的政策，适用于选择使用我们服务的用户。在NEWSWORLD公司，我们重视客户、服务用户及其个人信息的隐私。"
},

dataCollect:{
en: "We collect personal information such as name, email, and usage preferences when you interact with our services.",
ar: "نجمع المعلومات الشخصية مثل الاسم والبريد الإلكتروني وتفضيلات الاستخدام عند تفاعلك مع خدماتنا.",
ru: "Мы собираем личную информацию, такую как имя, адрес электронной почты и предпочтения использования, когда вы взаимодействуете с нашими услугами.",
ja: "当社のサービスとやり取りする際に、氏名、メールアドレス、使用状況の設定などの個人情報を収集します。",
zh: "当您使用我们的服务时，我们会收集诸如姓名、电子邮件和使用偏好等个人信息。"
},

dataUsage:{
en: "Your information is used to improve our platform, provide customized content, and communicate important updates.",
ar: "تُستخدم معلوماتك لتحسين منصتنا، وتقديم محتوى مخصص، والتواصل بشأن التحديثات المهمة.",
ru: "Ваша информация используется для улучшения нашей платформы, предоставления персонализированного контента и передачи важных обновлений.",
ja: "お客様の情報は、当社のプラットフォームを改善し、カスタマイズされたコンテンツを提供し、重要な更新情報をお知らせするために使用されます。",
zh: "您的信息用于改进我们的平台、提供个性化内容并传达重要更新。"
},

dataSharing:{
en: "We do not share personal information with third parties without consent, except as required by law or for service purposes.",
ar: "نحن لا نشارك المعلومات الشخصية مع أطراف ثالثة دون موافقة، إلا عند الحاجة القانونية أو لأغراض الخدمة.",
ru: "Мы не передаем личную информацию третьим лицам без согласия, кроме случаев, предусмотренных законом или для обслуживания.",
ja: "お客様の個人情報は、法的に必要な場合またはサービス提供の目的を除き、第三者と共有しません。",
zh: "我们不会在未经同意的情况下与第三方共享个人信息，法律要求或服务目的除外。"
},


userRights:{
en: "You have the right to access, correct, or request deletion of your personal data at any time.",
ar: "لديك الحق في الوصول إلى بياناتك الشخصية وتصحيحها أو طلب حذفها في أي وقت.",
ru: "Вы имеете право в любое время получить доступ, исправить или запросить удаление ваших персональных данных.",
ja: "お客様は、個人データへのアクセス、修正、削除をいつでも要求する権利があります。",
zh: "您有权随时访问、更正或请求删除您的个人数据。"
},

cookies:{
en: "We use cookies and similar tracking technologies to enhance user experience, analyze usage, and deliver personalized content and advertisements.",
ar: "نستخدم ملفات تعريف الارتباط وتقنيات التتبع المشابهة لتحسين تجربة المستخدم، وتحليل الاستخدام، وتقديم محتوى وإعلانات مخصصة.",
ru: "Мы используем файлы cookie и аналогичные технологии отслеживания, чтобы улучшить пользовательский опыт, проанализировать использование и предоставлять персонализированный контент и рекламу.",
ja: "当社は、ユーザー体験を向上させ、利用状況を分析し、パーソナライズされたコンテンツや広告を提供するために、クッキーや類似の追跡技術を使用します。",
zh: "我们使用 Cookie 和类似的跟踪技术来提升用户体验、分析使用情况，并提供个性化的内容和广告。"
},


dataRetent:{
en: "We store personal information only as long as necessary for the purposes set out in this Privacy Policy, unless a longer retention period is required or permitted by law.",
ar: "نخزن المعلومات الشخصية فقط طالما كان ذلك ضروريًا للأغراض الموضحة في سياسة الخصوصية هذه، ما لم يتطلب القانون أو يسمح بفترة احتفاظ أطول.",
ru: "Мы храним личную информацию только столько времени, сколько необходимо для целей, изложенных в настоящей Политике конфиденциальности, если более длительный срок хранения не требуется или не разрешен законом.",
ja: "当社は、このプライバシーポリシーに記載された目的のために必要な期間のみ個人情報を保管し、法律でより長い保存期間が要求または許可されている場合を除きます。",
zh: "我们仅在本隐私政策所述目的所需的时间内存储个人信息，除非法律要求或允许更长的保存期限。"
},


security:{
en: "We implement technical, administrative, and physical safeguards designed to protect your information against unauthorized access, disclosure, alteration, or destruction.",
ar: "نطبق تدابير تقنية وإدارية ومادية مصممة لحماية معلوماتك من الوصول أو الكشف أو التعديل أو التدمير غير المصرح به.",
ru: "Мы внедряем технические, административные и физические меры защиты, предназначенные для защиты вашей информации от несанкционированного доступа, раскрытия, изменения или уничтожения.",
ja: "当社は、お客様の情報を不正アクセス、開示、改ざん、破壊から保護するために設計された技術的、管理的および物理的な保護策を実施しています。",
zh: "我们实施技术、管理和物理保护措施，以防止您的信息遭到未经授权的访问、披露、篡改或销毁。"
},

children:{
en: "Our Service is not directed to individuals under the age of 13 (or 16 in certain jurisdictions). We do not knowingly collect personal information from children.",
ar: "خدمتنا ليست موجهة للأفراد الذين تقل أعمارهم عن 13 عامًا (أو 16 عامًا في بعض الولايات القضائية). نحن لا نجمع عن قصد معلومات شخصية من الأطفال.",
ru: "Наш сервис не предназначен для лиц младше 13 лет (или 16 лет в некоторых юрисдикциях). Мы сознательно не собираем личную информацию от детей.",
ja: "当社のサービスは13歳未満（または一部の法域では16歳未満）の方を対象としていません。私たちは子供から個人情報を意図的に収集しません。",
zh: "我们的服务不针对13岁以下（或在某些司法管辖区为16岁以下）的个人。我们不会故意收集儿童的个人信息。"
},


change:{
en: "We may update our Privacy Policy from time to time. We will notify you of any changes by posting the new Privacy Policy on this page and updating the 'Last Updated' date.",
ar: "قد نقوم بتحديث سياسة الخصوصية الخاصة بنا من حين لآخر. سنخطرك بأي تغييرات عن طريق نشر سياسة الخصوصية الجديدة على هذه الصفحة وتحديث تاريخ 'آخر تحديث'.",
ru: "Мы можем время от времени обновлять нашу Политику конфиденциальности. Мы уведомим вас о любых изменениях, разместив новую Политику конфиденциальности на этой странице и обновив дату 'Последнее обновление'.",
ja: "当社はプライバシーポリシーを随時更新する場合があります。変更があった場合は、このページに新しいプライバシーポリシーを掲載し、「最終更新日」を更新することでお知らせします。",
zh: "我们可能会不时更新我们的隐私政策。我们会通过在此页面上发布新的隐私政策并更新“最后更新”日期来通知您任何更改。"
},

contact:{
en: "If you have any questions or concerns about this Privacy Policy or our data practices, please contact us at [helpdesk@newsworldapp.org].",
ar: "إذا كانت لديك أي أسئلة أو مخاوف بشأن سياسة الخصوصية هذه أو ممارساتنا المتعلقة بالبيانات، يرجى الاتصال بنا على [helpdesk@newsworldapp.org].",
ru: "Если у вас есть вопросы или опасения по поводу этой Политики конфиденциальности или наших методов обработки данных, пожалуйста, свяжитесь с нами по адресу [helpdesk@newsworldapp.org].",
ja: "本プライバシーポリシーまたは当社のデータ取り扱いに関してご質問やご懸念がある場合は、[helpdesk@newsworldapp.org] までご連絡ください。",
zh: "如果您对本隐私政策或我们的数据处理方式有任何疑问或担忧，请通过 [helpdesk@newsworldapp.org] 与我们联系。"
}

}


]



export const EU = [ 'austria','belgium','bulgaria','croatia','cyprus','czech republic','denmark','estonia','finland','france','germany','greece','hungary','ireland','italy','latvia','lithuania','luxembourg','malta','netherlands','poland','portugal','romania','slovakia','slovenia','spain','sweden']



export const french_speak = [
"FR", // France
"BE", // Belgium
"CH", // Switzerland
"CA", // Canada (Quebec)
"LU", // Luxembourg
"MC", // Monaco
"HT", // Haiti
"SN", // Senegal
"CI", // Côte d’Ivoire
"TG", // Togo
"BJ", // Benin
"ML", // Mali
"BF", // Burkina Faso
"NE", // Niger
"GN", // Guinea
"CF", // Central African Republic
"CD", // Democratic Republic of the Congo
"CG", // Republic of the Congo
"GA", // Gabon
"CM", // Cameroon
"MG", // Madagascar
"DJ", // Djibouti
"KM", // Comoros
"VU"  // Vanuatu
];


export const spanish_speak = [
"ES", // Spain
"MX", // Mexico
"AR", // Argentina
"CO", // Colombia
"CL", // Chile
"PE", // Peru
"VE", // Venezuela
"EC", // Ecuador
"BO", // Bolivia
"PY", // Paraguay
"UY", // Uruguay
"CU", // Cuba
"DO", // Dominican Republic
"GT", // Guatemala
"HN", // Honduras
"SV", // El Salvador
"NI", // Nicaragua
"CR", // Costa Rica
"PA"  // Panama
];

export const portuguese_speak = [
"PT", // Portugal
"BR", // Brazil
"AO", // Angola
"MZ", // Mozambique
"CV", // Cape Verde
"GW", // Guinea-Bissau
"ST"  // São Tomé and Príncipe
];

export const arabic_speak = [
"EG", "SD", "DZ", "MA", "TN", "LY", "MR", // North Africa
"SA", "AE", "QA", "KW", "BH", "OM", "YE", // Arabian Peninsula
"IQ", "SY", "JO", "LB", "PS"             // Levant
];

export const russian_speak = [
"RU", // Russia
"BY", // Belarus
"KZ", // Kazakhstan
"KG", // Kyrgyzstan
"UA"  // Ukraine (partially Russian-speak)
];

export const japanese_speak = [
"JP" // Japan
];

export const chinese_speak = [
"CN", // China
"TW", // Taiwan
"SG"  // Sapore (official Mandarin use)
];

export const english_speak = [
"US", // United States
"GB", // United Kdom
"CA", // Canada
"AU", // Australia
"NZ", // New Zealand
"IE", // Ireland
"ZA", // South Africa
"NG", // Nigeria
"GH", // Ghana
"KE", // Kenya
"UG", // Uganda
"TZ", // Tanzania
"ZW", // Zimbabwe
"BW", // Botswana
"ZM", // Zambia
"MW", // Malawi
"SL", // Sierra Leone
"LR", // Liberia
"GM"  // Gambia
];

export const german_speak = [
"DE", // Germany
"AT", // Austria
"CH", // Switzerland
"LI", // Liechtenstein
"LU"  // Luxembourg (also French)
];

export const italian_speak = [
"IT", // Italy
"CH", // Switzerland (part)
"SM", // San Marino
"VA"  // Vatican City
];

export const dutch_speak = [
"NL", // Netherlands
"BE", // Belgium (Flemish)
"SR"  // Suriname
];


export const polish_speak = [
"PL" // Poland
];

export const hindi_speak = [
"IN", // India (Hindi is official but not exclusive)
"FJ", // Fiji (Fiji Hindi)
"MU", // Mauritius (part of population)
"NP"  // Nepal (close relation, but official is Nepali)
];

export const urdu_speak = [
"PK", // Pakistan
"IN"  // India (large Urdu-speak population)
];

export const persian_speak = [
"IR", // Iran (Farsi)
"AF", // Afghanistan (Dari, a Persian variant)
"TJ"  // Tajikistan (Tajik, Persian variant)
];

export const turkish_speak = [
"TR", // Turkey
"CY"  // Cyprus (Northern Cyprus, recognized partially)
];

export const korean_speak = [
"KR", // South Korea
"KP"  // North Korea
];

export const swedish_speak = [
"SE", // Sweden
"FI"  // Finland (Swedish is co-official)
];

export const danish_speak = [
"DK", // Denmark
"GL", // Greenland
"FO"  // Faroe Islands
];

export const norwegian_speak = [
"NO" // Norway
];

export const finnish_speak = [
"FI" // Finland
];

export const czech_speak = [
"CZ" // Czech Republic
];

export const ukrainian_speak = [
"UA" // Ukraine
];

export const greek_speak = [
"GR", // Greece
"CY"  // Cyprus
];

export const bulgarian_speak = [
"BG" // Bulgaria
];

export const romanian_speak = [
"RO", // Romania
"MD"  // Moldova
];

export const slovak_speak = [
"SK" // Slovakia
];

export const hungarian_speak = [
"HU" // Hungary
];





export const category = [

  {
    "item":{"en":"All","fr":"Tout","de":"Alle","ar":"الكل","es":"Todo","tr":"Tümü","nl":"Alles","it":"Tutto","ja":"すべて","zh":"全部","ko":"전체","hi":"सभी","pt":"Tudo","ru":"Все","sw":"Yote","pl":"Wszystko","id":"Semua","fa":"همه","pa":"ਸਭ","uk":"Усе","ro":"Toate","tl":"Lahat"},

    "id":"a"
  },
  {
    "item": {"en":"Business","fr":"Affaires","de":"Wirtschaft","ar":"الأعمال","es":"Negocios","tr":"İş Dünyası","nl":"Zakelijk","it":"Affari","ja":"ビジネス","zh":"商业","ko":"비즈니스","hi":"व्यापार","pt":"Negócios","ru":"Бизнес","sw":"Biashara","pl":"Biznes","id":"Bisnis","fa":"کسب‌وکار","pa":"ਵਪਾਰ","uk":"Бізнес","ro":"Afaceri","tl":"Negosyo"},

    "id": "b"
  },
  {
    "item": {"en":"Crime","fr":"Criminalité","de":"Kriminalität","ar":"الجريمة","es":"Crimen","tr":"Suç","nl":"Misdaad","it":"Crimine","ja":"犯罪","zh":"犯罪","ko":"범죄","hi":"अपराध","pt":"Crime","ru":"Преступность","sw":"Uhalifu","pl":"Przestępczość","id":"Kejahatan","fa":"جرم","pa":"ਅਪਰਾਧ","uk":"Злочин","ro":"Crimă","tl":"Krimen"},

    "id": "c"
  },
  {
    "item": {"en":"Domestic","fr":"National","de":"Inland","ar":"محلي","es":"Nacional","tr":"Yurt İçi","nl":"Binnenland","it":"Nazionale","ja":"国内","zh":"国内","ko":"국내","hi":"घरेलू","pt":"Nacional","ru":"Внутренние","sw":"Ndani ya nchi","pl":"Krajowe","id":"Domestik","fa":"داخلی","pa":"ਦੇਸ਼ੀ","uk":"Внутрішні","ro":"Național","tl":"Pambansa"},

    "id": "d"
  },
  {
    "item": {"en":"Education","fr":"Éducation","de":"Bildung","ar":"التعليم","es":"Educación","tr":"Eğitim","nl":"Onderwijs","it":"Istruzione","ja":"教育","zh":"教育","ko":"교육","hi":"शिक्षा","pt":"Educação","ru":"Образование","sw":"Elimu","pl":"Edukacja","id":"Pendidikan","fa":"آموزش","pa":"ਸਿੱਖਿਆ","uk":"Освіта","ro":"Educație","tl":"Edukasyon"},

    "id": "e"
  },
  {
    "item": {"en":"Entertainment","fr":"Divertissement","de":"Unterhaltung","ar":"الترفيه","es":"Entretenimiento","tr":"Eğlence","nl":"Entertainment","it":"Intrattenimento","ja":"エンタメ","zh":"娱乐","ko":"엔터테인먼트","hi":"मनोरंजन","pt":"Entretenimento","ru":"Развлечения","sw":"Burudani","pl":"Rozrywka","id":"Hiburan","fa":"سرگرمی","pa":"ਮਨੋਰੰਜਨ","uk":"Розваги","ro":"Divertisment","tl":"Aliwan"},

    "id": "f"
  },
  {
    "item": {"en":"Environment","fr":"Environnement","de":"Umwelt","ar":"البيئة","es":"Medio ambiente","tr":"Çevre","nl":"Milieu","it":"Ambiente","ja":"環境","zh":"环境","ko":"환경","hi":"पर्यावरण","pt":"Meio ambiente","ru":"Окружающая среда","sw":"Mazingira","pl":"Środowisko","id":"Lingkungan","fa":"محیط زیست","pa":"ਵਾਤਾਵਰਣ","uk":"Довкілля","ro":"Mediu","tl":"Kapaligiran"},

    "id": "g"
  },
  {
    "item": {"en":"Food","fr":"Nourriture","de":"Essen","ar":"الطعام","es":"Comida","tr":"Yemek","nl":"Eten","it":"Cibo","ja":"食べ物","zh":"食品","ko":"음식","hi":"भोजन","pt":"Comida","ru":"Еда","sw":"Chakula","pl":"Jedzenie","id":"Makanan","fa":"غذا","pa":"ਭੋਜਨ","uk":"Їжа","ro":"Mâncare","tl":"Pagkain"},

    "id": "h"
  },
  {
    "item": {"en":"Health","fr":"Santé","de":"Gesundheit","ar":"الصحة","es":"Salud","tr":"Sağlık","nl":"Gezondheid","it":"Salute","ja":"健康","zh":"健康","ko":"건강","hi":"स्वास्थ्य","pt":"Saúde","ru":"Здоровье","sw":"Afya","pl":"Zdrowie","id":"Kesehatan","fa":"سلامت","pa":"ਸਿਹਤ","uk":"Здоров’я","ro":"Sănătate","tl":"Kalusugan"},

    "id": "i"
  },
  {
    "item": {"en":"Lifestyle","fr":"Mode de vie","de":"Lebensstil","ar":"نمط الحياة","es":"Estilo de vida","tr":"Yaşam tarzı","nl":"Levensstijl","it":"Stile di vita","ja":"ライフスタイル","zh":"生活方式","ko":"라이프스타일","hi":"जीवनशैली","pt":"Estilo de vida","ru":"Образ жизни","sw":"Mtindo wa maisha","pl":"Styl życia","id":"Gaya hidup","fa":"سبک زندگی","pa":"ਜੀਵਨ ਸ਼ੈਲੀ","uk":"Стиль життя","ro":"Stil de viață","tl":"Pamumuhay"},

    "id": "j"
  },
  {
    "item": {"en":"Politics","fr":"Politique","de":"Politik","ar":"السياسة","es":"Política","tr":"Siyaset","nl":"Politiek","it":"Politica","ja":"政治","zh":"政治","ko":"정치","hi":"राजनीति","pt":"Política","ru":"Политика","sw":"Siasa","pl":"Polityka","id":"Politik","fa":"سیاست","pa":"ਰਾਜਨੀਤੀ","uk":"Політика","ro":"Politică","tl":"Pulitika"},

    "id": "k"
  },
  {
    "item": {"en":"Science","fr":"Science","de":"Wissenschaft","ar":"العلوم","es":"Ciencia","tr":"Bilim","nl":"Wetenschap","it":"Scienza","ja":"科学","zh":"科学","ko":"과학","hi":"विज्ञान","pt":"Ciência","ru":"Наука","sw":"Sayansi","pl":"Nauka","id":"Sains","fa":"علم","pa":"ਵਿਗਿਆਨ","uk":"Наука","ro":"Știință","tl":"Agham"},

    "id": "l"
  },
  {
    "item": {"en":"Sports","fr":"Sports","de":"Sport","ar":"الرياضة","es":"Deportes","tr":"Spor","nl":"Sport","it":"Sport","ja":"スポーツ","zh":"体育","ko":"스포츠","hi":"खेल","pt":"Esportes","ru":"Спорт","sw":"Michezo","pl":"Sport","id":"Olahraga","fa":"ورزش","pa":"ਖੇਡਾਂ","uk":"Спорт","ro":"Sport","tl":"Palakasan"},

    "id": "m"
  },
  {
    "item": {"en":"Technology","fr":"Technologie","de":"Technologie","ar":"التكنولوجيا","es":"Tecnología","tr":"Teknoloji","nl":"Technologie","it":"Tecnologia","ja":"テクノロジー","zh":"科技","ko":"기술","hi":"प्रौद्योगिकी","pt":"Tecnologia","ru":"Технологии","sw":"Teknolojia","pl":"Technologia","id":"Teknologi","fa":"فناوری","pa":"ਤਕਨਾਲੋਜੀ","uk":"Технології","ro":"Tehnologie","tl":"Teknolohiya"},
    
    "id": "n"
  },
  {
    "item": {"en":"Tourism","fr":"Tourisme","de":"Tourismus","ar":"السياحة","es":"Turismo","tr":"Turizm","nl":"Toerisme","it":"Turismo","ja":"観光","zh":"旅游","ko":"관광","hi":"पर्यटन","pt":"Turismo","ru":"Туризм","sw":"Utalii","pl":"Turystyka","id":"Pariwisata","fa":"گردشگری","pa":"ਸੈਰ-ਸਪਾਟਾ","uk":"Туризм","ro":"Turism","tl":"Turismo"},

    "id": "o"
  }
];



export const filterList = [
{
label:{
  "en": "All",
  "fr": "Tout",
  "de": "Alle",
  "ar": "الكل",
  "es": "Todo",
  "tr": "Tümü",
  "nl": "Alles",
  "it": "Tutto",
  "ja": "すべて",
  "zh": "全部",
  "ko": "전체",
  "hi": "सभी",
  "pt": "Tudo",
  "ru": "Все",
  "sw": "Zote",
  "pl": "Wszystko",
  "id": "Semua",
  "fa": "همه",
  "pa": "ਸਭ",
  "uk": "Усі",
  "ro": "Toate",
  "tl": "Lahat"
},
value:"all"
},

{
  label:{
  "en": "Reactions",
  "fr": "Réactions",
  "de": "Reaktionen",
  "ar": "التفاعلات",
  "es": "Reacciones",
  "tr": "Tepkiler",
  "nl": "Reacties",
  "it": "Reazioni",
  "ja": "リアクション",
  "zh": "反应",
  "ko": "반응",
  "hi": "प्रतिक्रियाएँ",
  "pt": "Reações",
  "ru": "Реакции",
  "sw": "Miitikio",
  "pl": "Reakcje",
  "id": "Reaksi",
  "fa": "واکنش‌ها",
  "pa": "ਪ੍ਰਤੀਕਿਰਿਆਵਾਂ",
  "uk": "Реакції",
  "ro": "Reacții",
  "tl": "Mga Reaksyon"
},
value:"reaction"
},


{
label:{
  "en": "Replies",
  "fr": "Réponses",
  "de": "Antworten",
  "ar": "الردود",
  "es": "Respuestas",
  "tr": "Yanıtlar",
  "nl": "Antwoorden",
  "it": "Risposte",
  "ja": "返信",
  "zh": "回复",
  "ko": "답글",
  "hi": "उत्तर",
  "pt": "Respostas",
  "ru": "Ответы",
  "sw": "Majibu",
  "pl": "Odpowiedzi",
  "id": "Balasan",
  "fa": "پاسخ‌ها",
  "pa": "ਜਵਾਬ",
  "uk": "Відповіді",
  "ro": "Răspunsuri",
  "tl": "Mga Sagot"
},
value:"reply"
},


{
label:{
  "en": "For you",
  "fr": "Pour vous",
  "de": "Für dich",
  "ar": "من أجلك",
  "es": "Para ti",
  "tr": "Senin için",
  "nl": "Voor jou",
  "it": "Per te",
  "ja": "あなたへのおすすめ",
  "zh": "为你推荐",
  "ko": "회원님을 위한",
  "hi": "आपके लिए",
  "pt": "Para você",
  "ru": "Для вас",
  "sw": "Kwa ajili yako",
  "pl": "Dla Ciebie",
  "id": "Untuk Anda",
  "fa": "برای شما",
  "pa": "ਤੁਹਾਡੇ ਲਈ",
  "uk": "Для вас",
  "ro": "Pentru tine",
  "tl": "Para sa iyo"
},
value:"news"
},


{
label:{
  "en": "Updates",
  "fr": "Mises à jour",
  "de": "Aktualisierungen",
  "ar": "التحديثات",
  "es": "Actualizaciones",
  "tr": "Güncellemeler",
  "nl": "Updates",
  "it": "Aggiornamenti",
  "ja": "更新情報",
  "zh": "更新",
  "ko": "업데이트",
  "hi": "अपडेट्स",
  "pt": "Atualizações",
  "ru": "Обновления",
  "sw": "Masasisho",
  "pl": "Aktualizacje",
  "id": "Pembaruan",
  "fa": "به‌روزرسانی‌ها",
  "pa": "ਅਪਡੇਟਾਂ",
  "uk": "Оновлення",
  "ro": "Actualizări",
  "tl": "Mga Update"
},
value:"update"
},
]




export const app_data = [
  {label:"English", value:"en", icon:'gb', lcode:'en-GB'},
  {label:"French", value:"fr", icon:'fr', lcode:'fr-FR'},
  {label:"German", value:"de", icon:'de', lcode:'de-DE'},
  {label:"Arabic", value:"ar", icon:'sa', lcode:'ar-SA'},
  {label:"Spanish", value:"es", icon:'es', lcode:'es-ES'},
  {label:"Turkish", value:"tr", icon:'tr', lcode:'tr-TR'},
  {label:"Dutch", value:"nl", icon:'nl', lcode:'nl-NL'},
  {label:"Italian", value:"it", icon:'it', lcode:'it-IT'},
  {label:"Japanese", value:"ja", icon:'jp', lcode:'ja-JP'},
  {label:"Chinese", value:"zh", icon:'cn', lcode:'zh-CN'},
  {label:"Korean", value:"ko", icon:'kr', lcode:'ko-KR'},
  {label:"Hindi", value:"hi", icon:'in', lcode:'hi-IN'},
  {label:"Portuguese", value:"pt", icon:'pt', lcode:'pt-PT'},
  {label:"Russian", value:"ru", icon:'ru', lcode:'ru-RU'},
  {label:"Swahili", value:"sw", icon:'tz', lcode:'sw-TZ'}, 
  {label:"Polish", value:"pl", icon:'pl', lcode:'pl-PL'},
  {label:"Indonesian", value:"id", icon:'id', lcode:'id-ID'},
  {label:"Persian/Farsi", value:"fa", icon:'ir', lcode:'fa-IR'},
  {label:"Punjabi", value:"pa", icon:'in', lcode:'pa-IN'}, 
  {label:"Ukrainian", value:"uk", icon:'ua', lcode:'uk-UA'}, 
  {label:"Romanian", value:"ro", icon:'ro', lcode:'ro-RO'},
  {label:"Filipino/Tagalog", value:"tl", icon:'ph', lcode:'fil-PH'},
];





export const suggest = [
{ 
type:{
"en": "Functional Issues",
"fr": "Problèmes fonctionnels",
"de": "Funktionale Probleme",
"ar": "مشاكل وظيفية",
"es": "Problemas funcionales",
"tr": "Fonksiyonel Sorunlar",
"nl": "Functionele problemen",
"it": "Problemi funzionali",
"ja": "機能上の問題",
"zh": "功能问题",
"ko": "기능 문제",
"hi": "कार्यात्मक समस्याएँ",
"pt": "Problemas funcionais",
"ru": "Функциональные проблемы",
"sw": "Shida za Kazi",
"pl": "Problemy funkcjonalne",
"id": "Masalah Fungsional",
"fa": "مشکلات عملکردی",
"pa": "ਕਾਰਜਕਾਰੀ ਸਮੱਸਿਆਵਾਂ",
"uk": "Функціональні проблеми",
"ro": "Probleme funcționale",
"tl": "Mga Isyu sa Paggana"
}, 

more:{
"en": "App malfunction/unavailable features",
"fr": "Dysfonctionnement de l'application/fonctionnalités indisponibles",
"de": "App-Fehlfunktion/nicht verfügbare Funktionen",
"ar": "عطل في التطبيق/ميزات غير متاحة",
"es": "Mal funcionamiento de la aplicación/características no disponibles",
"tr": "Uygulama arızası/kullanılamayan özellikler",
"nl": "App-storing/niet-beschikbare functies",
"it": "Malfunzionamento dell'app/funzionalità non disponibili",
"ja": "アプリの不具合/利用できない機能",
"zh": "应用故障/功能不可用",
"ko": "앱 오작동/사용할 수 없는 기능",
"hi": "ऐप खराबी/अनुपलब्ध विशेषताएँ",
"pt": "Mau funcionamento do aplicativo/recursos indisponíveis",
"ru": "Сбой приложения/недоступные функции",
"sw": "Hitilafu ya programu/vipengele visivyopatikana",
"pl": "Awaria aplikacji/niedostępne funkcje",
"id": "Kerusakan aplikasi/fitur tidak tersedia",
"fa": "خرابی برنامه/ویژگی‌های در دسترس نیست",
"pa": "ਐਪ ਦੀ ਖਰਾਬੀ/ਉਪਲਬਧ ਨਾ ਹੋਣ ਵਾਲੀਆਂ ਵਿਸ਼ੇਸ਼ਤਾਵਾਂ",
"uk": "Збій програми/недоступні функції",
"ro": "Defecțiune aplicație/funcții indisponibile",
"tl": "Sira ang app/mga hindi magagamit na tampok"
},
},


{
type:{
"en": "Suggestions",
"fr": "Suggestions",
"de": "Vorschläge",
"ar": "اقتراحات",
"es": "Sugerencias",
"tr": "Öneriler",
"nl": "Suggesties",
"it": "Suggerimenti",
"ja": "提案",
"zh": "建议",
"ko": "제안",
"hi": "सुझाव",
"pt": "Sugestões",
"ru": "Предложения",
"sw": "Mapendekezo",
"pl": "Sugestie",
"id": "Saran",
"fa": "پیشنهادات",
"pa": "ਸੁਝਾਵ",
"uk": "Пропозиції",
"ro": "Sugestii",
"tl": "Mga Mungkahi"
},

more:{
"en": "Feedback/Improvement Ideas",
"fr": "Retour/Idées d'amélioration",
"de": "Feedback/Verbesserungsideen",
"ar": "ملاحظات/أفكار للتحسين",
"es": "Comentarios/Ideas de mejora",
"tr": "Geri Bildirim/İyileştirme Fikirleri",
"nl": "Feedback/Verbeterideeën",
"it": "Feedback/Idee di miglioramento",
"ja": "フィードバック/改善アイデア",
"zh": "反馈/改进建议",
"ko": "피드백/개선 아이디어",
"hi": "प्रतिक्रिया/सुधार विचार",
"pt": "Feedback/Ideias de melhoria",
"ru": "Отзывы/Идеи по улучшению",
"sw": "Mrejesho/Maoni ya Maboresho",
"pl": "Opinie/Pomysły na ulepszenia",
"id": "Masukan/Ide Perbaikan",
"fa": "بازخورد/ایده‌های بهبود",
"pa": "ਫੀਡਬੈਕ/ਸੁਧਾਰ ਲਈ ਵਿਚਾਰ",
"uk": "Відгуки/Ідеї для покращення",
"ro": "Feedback/Idei de îmbunătățire",
"tl": "Feedback/Mga Ideya sa Pagpapabuti"
},
},


{
type:{
"en": "Security Issues",
"fr": "Problèmes de sécurité",
"de": "Sicherheitsprobleme",
"ar": "مشاكل أمنية",
"es": "Problemas de seguridad",
"tr": "Güvenlik Sorunları",
"nl": "Beveiligingsproblemen",
"it": "Problemi di sicurezza",
"ja": "セキュリティの問題",
"zh": "安全问题",
"ko": "보안 문제",
"hi": "सुरक्षा समस्याएँ",
"pt": "Problemas de segurança",
"ru": "Проблемы безопасности",
"sw": "Shida za Usalama",
"pl": "Problemy z bezpieczeństwem",
"id": "Masalah Keamanan",
"fa": "مشکلات امنیتی",
"pa": "ਸੁਰੱਖਿਆ ਸਮੱਸਿਆਵਾਂ",
"uk": "Проблеми безпеки",
"ro": "Probleme de securitate",
"tl": "Mga Isyu sa Seguridad"
},


more:{
"en": "Password/Privacy/Fraud",
"fr": "Mot de passe/Confidentialité/Fraude",
"de": "Passwort/Datenschutz/Betrug",
"ar": "كلمة المرور/الخصوصية/الاحتيال",
"es": "Contraseña/Privacidad/Fraude",
"tr": "Şifre/Gizlilik/Dolandırıcılık",
"nl": "Wachtwoord/Privacy/Fraude",
"it": "Password/Privacy/Frode",
"ja": "パスワード/プライバシー/詐欺",
"zh": "密码/隐私/欺诈",
"ko": "비밀번호/개인정보/사기",
"hi": "पासवर्ड/गोपनीयता/धोखाधड़ी",
"pt": "Senha/Privacidade/Fraude",
"ru": "Пароль/Конфиденциальность/Мошенничество",
"sw": "Nenosiri/Usiri/Ulaghai",
"pl": "Hasło/Prywatność/Oszustwo",
"id": "Kata Sandi/Privasi/Penipuan",
"fa": "رمز عبور/حریم خصوصی/کلاهبرداری",
"pa": "ਪਾਸਵਰਡ/ਗੋਪਨੀਯਤਾ/ਧੋਖਾਧੜੀ",
"uk": "Пароль/Конфіденційність/Шахрайство",
"ro": "Parolă/Confidențialitate/Fraudă",
"tl": "Password/Privacy/Pandaraya"
},
},


{
type:{
"en": "Network Issues",
"fr": "Problèmes de réseau",
"de": "Netzwerkprobleme",
"ar": "مشاكل الشبكة",
"es": "Problemas de red",
"tr": "Ağ Sorunları",
"nl": "Netwerkproblemen",
"it": "Problemi di rete",
"ja": "ネットワークの問題",
"zh": "网络问题",
"ko": "네트워크 문제",
"hi": "नेटवर्क समस्याएँ",
"pt": "Problemas de rede",
"ru": "Проблемы с сетью",
"sw": "Shida za Mtandao",
"pl": "Problemy sieciowe",
"id": "Masalah Jaringan",
"fa": "مشکلات شبکه",
"pa": "ਨੈੱਟਵਰਕ ਸਮੱਸਿਆਵਾਂ",
"uk": "Проблеми мережі",
"ro": "Probleme de rețea",
"tl": "Mga Isyu sa Network"
}, 


more:{
"en": "Payment Failure/Slow Processing",
"fr": "Échec du paiement/traitement lent",
"de": "Zahlungsfehler/langsame Verarbeitung",
"ar": "فشل الدفع/المعالجة البطيئة",
"es": "Fallo en el pago/procesamiento lento",
"tr": "Ödeme Hatası/Yavaş İşlem",
"nl": "Betalingsfout/trage verwerking",
"it": "Errore di pagamento/Elaborazione lenta",
"ja": "支払い失敗/処理の遅延",
"zh": "支付失败/处理缓慢",
"ko": "결제 실패/처리 지연",
"hi": "भुगतान विफल/धीमी प्रक्रिया",
"pt": "Falha no pagamento/Processamento lento",
"ru": "Ошибка платежа/Медленная обработка",
"sw": "Kushindwa kwa Malipo/Usindikaji Polepole",
"pl": "Nieudana płatność/Wolne przetwarzanie",
"id": "Kegagalan Pembayaran/Pemrosesan Lambat",
"fa": "شکست پرداخت/پردازش کند",
"pa": "ਭੁਗਤਾਨ ਅਸਫਲ/ਧੀਮੀ ਪ੍ਰਕਿਰਿਆ",
"uk": "Помилка платежу/повільна обробка",
"ro": "Eroare plată/Procesare lentă",
"tl": "Nabigong Bayad/Mabagal na Pagproseso"
},
},



{
type:{
"en": "Wording Issues",
"fr": "Problèmes de formulation",
"de": "Formulierungsprobleme",
"ar": "مشاكل الصياغة",
"es": "Problemas de redacción",
"tr": "İfade Sorunları",
"nl": "Formuleringsproblemen",
"it": "Problemi di formulazione",
"ja": "表現の問題",
"zh": "措辞问题",
"ko": "표현 문제",
"hi": "शब्दांकन की समस्याएँ",
"pt": "Problemas de redação",
"ru": "Проблемы формулировки",
"sw": "Shida za Ufasiri",
"pl": "Problemy z sformułowaniem",
"id": "Masalah Kata-kata",
"fa": "مشکلات نگارشی",
"pa": "ਲਿਖਤ ਸੰਬੰਧੀ ਸਮੱਸਿਆਵਾਂ",
"uk": "Проблеми формулювання",
"ro": "Probleme de formulare",
"tl": "Mga Isyu sa Pagkakasulat"
},

more:{
"en": "Unclear Instructions/Confusing steps",
"fr": "Instructions peu claires/étapes confuses",
"de": "Unklare Anweisungen/verwirrende Schritte",
"ar": "تعليمات غير واضحة/خطوات مربكة",
"es": "Instrucciones poco claras/pasos confusos",
"tr": "Belirsiz Talimatlar/Kafa karıştırıcı adımlar",
"nl": "Onduidelijke instructies/verwarde stappen",
"it": "Istruzioni poco chiare/passaggi confusi",
"ja": "不明確な指示/混乱する手順",
"zh": "不清楚的说明/令人困惑的步骤",
"ko": "불명확한 지시/혼란스러운 단계",
"hi": "अस्पष्ट निर्देश/भ्रमित कदम",
"pt": "Instruções pouco claras/etapas confusas",
"ru": "Неясные инструкции/Запутанные шаги",
"sw": "Maagizo Yasiyo Wazi/Hatua Zinazochanganya",
"pl": "Niejasne instrukcje/Zamieszane kroki",
"id": "Instruksi Tidak Jelas/Langkah Membingungkan",
"fa": "دستورالعمل‌های نامشخص/مراحل گیج‌کننده",
"pa": "ਅਸਪਸ਼ਟ ਹੁਕਮ/ਉਲਝਣ ਵਾਲੇ ਕਦਮ",
"uk": "Нечіткі інструкції/заплутані кроки",
"ro": "Instrucțiuni neclare/Pași confuzi",
"tl": "Hindi malinaw na tagubilin/Nakakalitong hakbang"
}
},



{
type: {
  "en": "Report Inappropriate Comments",
  "fr": "Signaler des commentaires inappropriés",
  "de": "Unangemessene Kommentare melden",
  "ar": "الإبلاغ عن التعليقات غير المناسبة",
  "es": "Reportar comentarios inapropiados",
  "tr": "Uygunsuz yorumları bildir",
  "nl": "Ongepaste reacties melden",
  "it": "Segnala commenti inappropriati",
  "ja": "不適切なコメントを報告",
  "zh": "举报不当评论",
  "ko": "부적절한 댓글 신고",
  "hi": "अनुचित टिप्पणियों की रिपोर्ट करें",
  "pt": "Denunciar comentários inadequados",
  "ru": "Сообщить о неподобающих комментариях",
  "sw": "Ripoti maoni yasiyofaa",
  "pl": "Zgłoś nieodpowiednie komentarze",
  "id": "Laporkan komentar yang tidak pantas",
  "fa": "گزارش نظرات نامناسب",
  "pa": "ਗੈਰ-ਉਚਿਤ ਟਿੱਪਣੀਆਂ ਦੀ ਰਿਪੋਰਟ ਕਰੋ",
  "uk": "Повідомити про неприйнятні коментарі",
  "ro": "Raportează comentarii necorespunzătoare",
  "tl": "I-report ang hindi angkop na komento"
},


more:{
  "en": "Include a screenshot of such comment",
  "fr": "Incluez une capture d’écran du commentaire",
  "de": "Fügen Sie einen Screenshot des Kommentars hinzu",
  "ar": "أرفق لقطة شاشة لهذا التعليق",
  "es": "Incluye una captura de pantalla del comentario",
  "tr": "Yorumun ekran görüntüsünü ekleyin",
  "nl": "Voeg een screenshot van de reactie toe",
  "it": "Includi uno screenshot del commento",
  "ja": "該当コメントのスクリーンショットを添付してください",
  "zh": "请附上该评论的截图",
  "ko": "해당 댓글의 스크린샷을 첨부하세요",
  "hi": "कृपया उस टिप्पणी का स्क्रीनशॉट शामिल करें",
  "pt": "Inclua uma captura de tela do comentário",
  "ru": "Добавьте скриншот комментария",
  "sw": "Jumuisha picha ya skrini ya maoni hayo",
  "pl": "Dołącz zrzut ekranu komentarza",
  "id": "Sertakan tangkapan layar komentar tersebut",
  "fa": "لطفاً اسکرین‌شات از این نظر را ضمیمه کنید",
  "pa": "ਕਿਰਪਾ ਕਰਕੇ ਇਸ ਟਿੱਪਣੀ ਦਾ ਸਕ੍ਰੀਨਸ਼ਾਟ ਸ਼ਾਮਲ ਕਰੋ",
  "uk": "Додайте скріншот цього коментаря",
  "ro": "Include un screenshot al comentariului",
  "tl": "Isama ang screenshot ng komentong iyon"
}
},
]







export const lingual = {

"onboardiib":{
  "en": "Translate, listen, and explore global perspectives on news, react, comment and save to read later.",
  "fr": "Traduisez, écoutez et explorez des perspectives mondiales sur l’actualité, réagissez, commentez et enregistrez pour plus tard.",
  "de": "Übersetzen, anhören und globale Perspektiven auf Nachrichten entdecken, reagieren, kommentieren und für später speichern.",
  "ar": "ترجم واستمع واستكشف وجهات نظر عالمية للأخبار، وتفاعل وعلّق واحفظ للقراءة لاحقًا.",
  "es": "Traduce, escucha y explora perspectivas globales sobre las noticias, reacciona, comenta y guarda para leer más tarde.",
  "tr": "Haberleri çevirin, dinleyin ve küresel bakış açılarını keşfedin; tepki verin, yorum yapın ve sonra okumak için kaydedin.",
  "nl": "Vertaal, luister en ontdek wereldwijde perspectieven op nieuws, reageer, reageer en sla op om later te lezen.",
  "it": "Traduci, ascolta ed esplora prospettive globali sulle notizie, reagisci, commenta e salva per dopo.",
  "ja": "ニュースを翻訳・音声で聞き、世界の 視点を探索し、リアクションやコメント、 後で読むために保存できます。",
  "zh": "翻译、收听并探索全球 新闻视角，参与互动、发表评论并 保存稍后阅读。",
  "ko": "뉴스를 번역하고 듣고 글로벌 관점을 탐색하며 반응하고 댓글을 남기고 나중에 읽도록 저장하세요.",
  "hi": "खबरों पर वैश्विक दृष्टिकोण का अनुवाद करें, सुनें और जानें, प्रतिक्रिया दें और बाद में पढ़ने के लिए सहेजें।",
  "pt": "Traduza, ouça e explore perspectivas globais sobre notícias, reaja, comente e salve para ler depois.",
  "ru": "Переводите, слушайте и изучайте мировые взгляды на новости, реагируйте, комментируйте и сохраняйте на потом.",
  "sw": "Tafsiri, sikiliza na chunguza mitazamo ya kimataifa ya habari, toa maoni na uhifadhi kwa baadaye.",
  "pl": "Tłumacz, słuchaj i odkrywaj globalne perspektywy wiadomości, reaguj, komentuj i zapisuj na później.",
  "id": "Terjemahkan, dengarkan, dan jelajahi perspektif global tentang berita, beri reaksi dan simpan untuk dibaca nanti.",
  "fa": "خبرها را ترجمه کنید، گوش دهید و دیدگاه‌های جهانی را بررسی کنید، واکنش نشان دهید و برای بعد ذخیره کنید.",
  "pa": "ਖ਼ਬਰਾਂ ਦੇ ਗਲੋਬਲ ਨਜ਼ਰੀਏ ਅਨੁਵਾਦ ਕਰੋ, ਸੁਣੋ, ਪ੍ਰਤੀਕਿਰਿਆ ਦਿਓ ਅਤੇ ਬਾਅਦ ਲਈ ਸੰਭਾਲੋ।",
  "uk": "Перекладайте, слухайте та досліджуйте глобальні погляди на новини, реагуйте та зберігайте на потім.",
  "ro": "Traduceți, ascultați și explorați perspective globale asupra știrilor, reacționați și salvați pentru mai târziu.",
  "tl": "Isalin, pakinggan at tuklasin ang pandaigdigang pananaw sa balita, mag-react at mag-save para basahin mamaya."
},


"onboardib":{
  "en": "Access global curated news from every part of the world. Niche down to any local location.",
  "fr": "Accédez à des actualités mondiales soigneusement sélectionnées provenant de toutes les régions du monde. Ciblez n’importe quel lieu local.",
  "de": "Greifen Sie auf kuratierte globale Nachrichten aus allen Teilen der Welt zu. Fokussieren Sie sich auf jeden lokalen Ort.",
  "ar": "اطّلع على أخبار عالمية مختارة من جميع أنحاء العالم، وركّز على أي موقع محلي.",
  "es": "Accede a noticias globales seleccionadas de todas las partes del mundo y enfócate en cualquier ubicación local.",
  "tr": "Dünyanın her yerinden derlenmiş küresel haberlere erişin. İstediğiniz yerel konuma odaklanın.",
  "nl": "Krijg toegang tot zorgvuldig samengestelde wereldwijde nieuwsberichten van over de hele wereld. Zoom in op elke lokale locatie.",
  "it": "Accedi a notizie globali curate da ogni parte del mondo. Approfondisci qualsiasi area locale.",
  "ja": "世界中から厳選されたニュースにアクセスし あらゆる地域のローカルニュースを深掘りできます。",
  "zh": "获取来自世界各地的精选全球新闻 并深入了解任何本地地区。",
  "ko": "전 세계에서 엄선된 글로벌 뉴스를 확인하고 원하는 지역 소식에 집중하세요.",
  "hi": "दुनिया के हर हिस्से से चुनी गई वैश्विक खबरों तक पहुँचें और किसी भी स्थानीय क्षेत्र पर ध्यान केंद्रित करें।",
  "pt": "Acesse notícias globais selecionadas de todas as partes do mundo e foque em qualquer localidade.",
  "ru": "Получайте отобранные мировые новости со всего мира и углубляйтесь в любой локальный регион.",
  "sw": "Pata habari za kimataifa zilizochaguliwa kutoka kila pembe ya dunia na zingatia eneo lolote la karibu.",
  "pl": "Uzyskaj dostęp do starannie dobranych wiadomości z całego świata i skup się na dowolnej lokalizacji.",
  "id": "Akses berita global pilihan dari seluruh dunia dan fokuskan ke lokasi lokal mana pun.",
  "fa": "به اخبار جهانی گزینش‌شده از سراسر دنیا دسترسی داشته باشید و روی هر منطقه محلی تمرکز کنید.",
  "pa": "ਦੁਨੀਆ ਭਰ ਤੋਂ ਚੁਣੀਆਂ ਗਈਆਂ ਗਲੋਬਲ ਖ਼ਬਰਾਂ ਤੱਕ ਪਹੁੰਚ ਕਰੋ ਅਤੇ ਕਿਸੇ ਵੀ ਸਥਾਨਕ ਥਾਂ ’ਤੇ ਧਿਆਨ ਕੇਂਦ੍ਰਿਤ ਕਰੋ।",
  "uk": "Отримуйте добірні глобальні новини з усього світу та зосереджуйтеся на будь-якому локальному регіоні.",
  "ro": "Accesați știri globale atent selectate din întreaga lume și concentrați-vă pe orice locație locală.",
  "tl": "I-access ang piling pandaigdigang balita mula sa iba’t ibang panig ng mundo at magpokus sa anumang lokal na lugar."
},



"confirmPass":{
  "en": "Confirm Password",
  "fr": "Confirmer le mot de passe",
  "de": "Passwort bestätigen",
  "ar": "تأكيد كلمة المرور",
  "es": "Confirmar contraseña",
  "tr": "Şifreyi onayla",
  "nl": "Bevestig wachtwoord",
  "it": "Conferma password",
  "ja": "パスワードを確認",
  "zh": "确认密码",
  "ko": "비밀번호 확인",
  "hi": "पासवर्ड की पुष्टि करें",
  "pt": "Confirmar senha",
  "ru": "Подтвердить пароль",
  "sw": "Thibitisha nenosiri",
  "pl": "Potwierdź hasło",
  "id": "Konfirmasi kata sandi",
  "fa": "تأیید رمز عبور",
  "pa": "ਪਾਸਵਰਡ ਦੀ ਪੁਸ਼ਟੀ ਕਰੋ",
  "uk": "Підтвердити пароль",
  "ro": "Confirmă parola",
  "tl": "Kumpirmahin ang password"
},


"enterNPass":{
  "en": "Enter New Password",
  "fr": "Entrez un nouveau mot de passe",
  "de": "Neues Passwort eingeben",
  "ar": "أدخل كلمة مرور جديدة",
  "es": "Introduce una nueva contraseña",
  "tr": "Yeni şifre girin",
  "nl": "Nieuw wachtwoord invoeren",
  "it": "Inserisci nuova password",
  "ja": "新しいパスワードを入力",
  "zh": "输入新密码",
  "ko": "새 비밀번호 입력",
  "hi": "नया पासवर्ड दर्ज करें",
  "pt": "Digite a nova senha",
  "ru": "Введите новый пароль",
  "sw": "Weka nenosiri jipya",
  "pl": "Wpisz nowe hasło",
  "id": "Masukkan kata sandi baru",
  "fa": "رمز عبور جدید را وارد کنید",
  "pa": "ਨਵਾਂ ਪਾਸਵਰਡ ਦਰਜ ਕਰੋ",
  "uk": "Введіть новий пароль",
  "ro": "Introdu parola nouă",
  "tl": "Ilagay ang bagong password"
},


"chnagePass":{
  "en": "Change Password",
  "fr": "Changer le mot de passe",
  "de": "Passwort ändern",
  "ar": "تغيير كلمة المرور",
  "es": "Cambiar contraseña",
  "tr": "Şifreyi değiştir",
  "nl": "Wachtwoord wijzigen",
  "it": "Cambia password",
  "ja": "パスワードを変更",
  "zh": "更改密码",
  "ko": "비밀번호 변경",
  "hi": "पासवर्ड बदलें",
  "pt": "Alterar senha",
  "ru": "Изменить пароль",
  "sw": "Badilisha nenosiri",
  "pl": "Zmień hasło",
  "id": "Ubah kata sandi",
  "fa": "تغییر رمز عبور",
  "pa": "ਪਾਸਵਰਡ ਬਦਲੋ",
  "uk": "Змінити пароль",
  "ro": "Schimbă parola",
  "tl": "Palitan ang password"
},



"resend":{
  "en": "Resend",
  "fr": "Renvoyer",
  "de": "Erneut senden",
  "ar": "إعادة الإرسال",
  "es": "Reenviar",
  "tr": "Yeniden gönder",
  "nl": "Opnieuw verzenden",
  "it": "Invia di nuovo",
  "ja": "再送信",
  "zh": "重新发送",
  "ko": "다시 보내기",
  "hi": "पुनः भेजें",
  "pt": "Reenviar",
  "ru": "Отправить снова",
  "sw": "Tuma tena",
  "pl": "Wyślij ponownie",
  "id": "Kirim ulang",
  "fa": "ارسال مجدد",
  "pa": "ਮੁੜ ਭੇਜੋ",
  "uk": "Надіслати знову",
  "ro": "Retrimite",
  "tl": "Ipadala muli"
},


"byTapping":{
  "en": "By tapping “Send Code” we’ll send a code to your email address.",
  "fr": "En appuyant sur « Envoyer le code », nous enverrons un code à votre adresse e-mail.",
  "de": "Durch Tippen auf “Code senden“ senden wir einen Code an Ihre E-Mail-Adresse.",
  "ar": "بالنقر على «إرسال الرمز» سنرسل رمزًا إلى بريدك الإلكتروني.",
  "es": "Al tocar “Enviar código”, enviaremos un código a tu correo electrónico.",
  "tr": "“Kodu Gönder”e dokunarak e-posta adresinize bir kod göndereceğiz.",
  "nl": "Door op ‘Code verzenden’ te tikken sturen we een code naar uw e-mailadres.",
  "it": "Toccando “Invia codice” invieremo un codice al tuo indirizzo email.",
  "ja": "「コードを送信」をタップすると、メールアドレスにコードが送信されます。",
  "zh": "点击“发送验证码”，我们将向您的电子邮箱发送验证码。",
  "ko": "‘코드 보내기’를 누르면 이메일 주소로 코드가 전송됩니다.",
  "hi": "“कोड भेजें” पर टैप करने से हम आपके ईमेल पर एक कोड भेजेंगे।",
  "pt": "Ao tocar em “Enviar código”, enviaremos um código para seu e-mail.",
  "ru": "Нажав «Отправить код», мы отправим код на ваш адрес электронной почты.",
  "sw": "Kwa kubofya “Tuma nambari”, tutatuma nambari kwenye barua pepe yako.",
  "pl": "Po naciśnięciu “Wyślij kod” wyślemy kod na Twój adres e-mail.",
  "id": "Dengan menekan “Kirim kode”, kami akan mengirimkan kode ke email Anda.",
  "fa": "با انتخاب «ارسال کد»، یک کد به ایمیل شما ارسال می‌شود.",
  "pa": "“ਕੋਡ ਭੇਜੋ” ਨੂੰ ਦਬਾ ਕੇ ਅਸੀਂ ਤੁਹਾਡੇ ਈਮੇਲ ’ਤੇ ਕੋਡ ਭੇਜਾਂਗੇ।",
  "uk": "Натиснувши «Надіслати код», ми надішлемо код на вашу електронну пошту.",
  "ro": "Apăsând “Trimite codul”, vom trimite un cod la adresa ta de e-mail.",
  "tl": "Sa pag-tap sa “Ipadala ang code”, magpapadala kami ng code sa iyong email."
},


"change":{
  "en": "Change",
  "fr": "Changer",
  "de": "Ändern",
  "ar": "تغيير",
  "es": "Cambiar",
  "tr": "Değiştir",
  "nl": "Wijzigen",
  "it": "Modifica",
  "ja": "変更",
  "zh": "更改",
  "ko": "변경",
  "hi": "बदलें",
  "pt": "Alterar",
  "ru": "Изменить",
  "sw": "Badilisha",
  "pl": "Zmień",
  "id": "Ubah",
  "fa": "تغییر",
  "pa": "ਬਦਲੋ",
  "uk": "Змінити",
  "ro": "Schimbă",
  "tl": "Baguhin"
},




"welcmCreate":{
"en": "Welcome! Fill up the information",
  "fr": "Bienvenue ! Remplissez les informations",
  "de": "Willkommen! Füllen Sie die Informationen aus",
  "ar": "مرحبًا! يرجى ملء المعلومات",
  "es": "¡Bienvenido! Complete la información",
  "tr": "Hoş geldiniz! Bilgileri doldurun",
  "nl": "Welkom! Vul de gegevens in",
  "it": "Benvenuto! Compila le informazioni",
  "ja": "ようこそ！情報を入力してください",
  "zh": "欢迎！请填写信息",
  "ko": "환영합니다! 정보를 입력하세요",
  "hi": "स्वागत है! जानकारी भरें",
  "pt": "Bem-vindo! Preencha as informações",
  "ru": "Добро пожаловать! Заполните информацию",
  "sw": "Karibu! Jaza taarifa",
  "pl": "Witamy! Wypełnij informacje",
  "id": "Selamat datang! Isi informasi",
  "fa": "خوش آمدید! لطفاً اطلاعات را وارد کنید",
  "pa": "ਜੀ ਆਇਆਂ ਨੂੰ! ਜਾਣਕਾਰੀ ਭਰੋ",
  "uk": "Ласкаво просимо! Заповніть інформацію",
  "ro": "Bine ați venit! Completați informațiile",
  "tl": "Maligayang pagdating! Punan ang impormasyon"
},


"belowCreate":{
  "en": "below to create an account.",
  "fr": "ci-dessous pour créer un compte.",
  "de": "unten, um ein Konto zu erstellen.",
  "ar": "أدناه لإنشاء حساب.",
  "es": "a continuación para crear una cuenta.",
  "tr": "aşağıdan bir hesap oluşturmak için.",
  "nl": "hieronder om een account aan te maken.",
  "it": "qui sotto per creare un account.",
  "ja": "以下からアカウントを作成します",
  "zh": "在下方创建账户",
  "ko": "아래에서 계정을 생성하세요",
  "hi": "नीचे खाता बनाने के लिए",
  "pt": "abaixo para criar uma conta",
  "ru": "ниже, чтобы создать аккаунт",
  "sw": "chini ili kuunda akaunti",
  "pl": "poniżej, aby utworzyć konto.",
  "id": "di bawah untuk membuat akun.",
  "fa": "در زیر برای ایجاد حساب",
  "pa": "ਹੇਠਾਂ ਖਾਤਾ ਬਣਾਉਣ ਲਈ",
  "uk": "нижче, щоб створити обліковий запис",
  "ro": "mai jos pentru a crea un cont.",
  "tl": "sa ibaba upang gumawa ng account."
},



"enterEmail":{
  "en": "Enter email address",
  "fr": "Entrez l’adresse e-mail",
  "de": "E-Mail-Adresse eingeben",
  "ar": "أدخل عنوان البريد الإلكتروني",
  "es": "Introduce el correo electrónico",
  "tr": "E-posta adresini gir",
  "nl": "Voer e-mailadres in",
  "it": "Inserisci l’indirizzo email",
  "ja": "メールアドレスを入力",
  "zh": "输入电子邮件地址",
  "ko": "이메일 주소 입력",
  "hi": "ईमेल पता दर्ज करें",
  "pt": "Digite o endereço de e-mail",
  "ru": "Введите адрес электронной почты",
  "sw": "Weka anwani ya barua pepe",
  "pl": "Wpisz adres e-mail",
  "id": "Masukkan alamat email",
  "fa": "آدرس ایمیل را وارد کنید",
  "pa": "ਈਮੇਲ ਪਤਾ ਦਰਜ ਕਰੋ",
  "uk": "Введіть адресу електронної пошти",
  "ro": "Introduceți adresa de e-mail",
  "tl": "Ilagay ang email address"
},



"resetPass":{
  "en": "Reset Password",
  "fr": "Réinitialiser le mot de passe",
  "de": "Passwort zurücksetzen",
  "ar": "إعادة تعيين كلمة المرور",
  "es": "Restablecer contraseña",
  "tr": "Şifreyi sıfırla",
  "nl": "Wachtwoord opnieuw instellen",
  "it": "Reimposta password",
  "ja": "パスワードをリセット",
  "zh": "重置密码",
  "ko": "비밀번호 재설정",
  "hi": "पासवर्ड रीसेट करें",
  "pt": "Redefinir senha",
  "ru": "Сбросить пароль",
  "sw": "Weka upya nenosiri",
  "pl": "Zresetuj hasło",
  "id": "Atur ulang kata sandi",
  "fa": "بازنشانی رمز عبور",
  "pa": "ਪਾਸਵਰਡ ਰੀਸੈਟ ਕਰੋ",
  "uk": "Скинути пароль",
  "ro": "Resetare parolă",
  "tl": "I-reset ang password"
},




"sendCode":{
  "en": "Send Code",
  "fr": "Envoyer le code",
  "de": "Code senden",
  "ar": "إرسال الرمز",
  "es": "Enviar código",
  "tr": "Kodu gönder",
  "nl": "Code verzenden",
  "it": "Invia codice",
  "ja": "コードを送信",
  "zh": "发送验证码",
  "ko": "코드 보내기",
  "hi": "कोड भेजें",
  "pt": "Enviar código",
  "ru": "Отправить код",
  "sw": "Tuma nambari",
  "pl": "Wyślij kod",
  "id": "Kirim kode",
  "fa": "ارسال کد",
  "pa": "ਕੋਡ ਭੇਜੋ",
  "uk": "Надіслати код",
  "ro": "Trimite codul",
  "tl": "Ipadala ang code"
},



"forgotPass":{
  "en": "Forgot Password?",
  "fr": "Mot de passe oublié ?",
  "de": "Passwort vergessen?",
  "ar": "هل نسيت كلمة المرور؟",
  "es": "¿Olvidaste tu contraseña?",
  "tr": "Şifrenizi mi unuttunuz?",
  "nl": "Wachtwoord vergeten?",
  "it": "Password dimenticata?",
  "ja": "パスワードを忘れましたか？",
  "zh": "忘记密码？",
  "ko": "비밀번호를 잊으셨나요?",
  "hi": "पासवर्ड भूल गए?",
  "pt": "Esqueceu a senha?",
  "ru": "Забыли пароль?",
  "sw": "Umesahau nenosiri?",
  "pl": "Nie pamiętasz hasła?",
  "id": "Lupa kata sandi?",
  "fa": "رمز عبور را فراموش کرده‌اید؟",
  "pa": "ਪਾਸਵਰਡ ਭੁੱਲ ਗਏ?",
  "uk": "Забули пароль?",
  "ro": "Ai uitat parola?",
  "tl": "Nakalimutan ang password?"
},



"enterEBegin":{
  "en": "Enter your email address to begin",
  "fr": "Entrez votre adresse e-mail pour commencer",
  "de": "Geben Sie Ihre E-Mail-Adresse ein, um zu beginnen",
  "ar": "أدخل عنوان بريدك الإلكتروني للبدء",
  "es": "Introduce tu correo electrónico para comenzar",
  "tr": "Başlamak için e-posta adresinizi girin",
  "nl": "Voer uw e-mailadres in om te beginnen",
  "it": "Inserisci il tuo indirizzo email per iniziare",
  "ja": "開始するにはメールアドレスを入力してください",
  "zh": "输入您的电子邮件地址以开始",
  "ko": "시작하려면 이메일 주소를 입력하세요",
  "hi": "शुरू करने के लिए अपना ईमेल दर्ज करें",
  "pt": "Digite seu endereço de e-mail para começar",
  "ru": "Введите адрес электронной почты, чтобы начать",
  "sw": "Weka barua pepe yako ili kuanza",
  "pl": "Wpisz adres e-mail, aby rozpocząć",
  "id": "Masukkan alamat email Anda untuk memulai",
  "fa": "برای شروع آدرس ایمیل خود را وارد کنید",
  "pa": "ਸ਼ੁਰੂ ਕਰਨ ਲਈ ਆਪਣਾ ਈਮੇਲ ਦਰਜ ਕਰੋ",
  "uk": "Введіть адресу електронної пошти, щоб почати",
  "ro": "Introduceți adresa de e-mail pentru a începe",
  "tl": "Ilagay ang iyong email address upang magsimula"
},



"lgetStarted":{
  "en": "Let’s get started",
  "fr": "Commençons",
  "de": "Los geht’s",
  "ar": "لنبدأ",
  "es": "Comencemos",
  "tr": "Başlayalım",
  "nl": "Laten we beginnen",
  "it": "Iniziamo",
  "ja": "始めましょう",
  "zh": "开始吧",
  "ko": "시작해볼까요",
  "hi": "चलिए शुरू करें",
  "pt": "Vamos começar",
  "ru": "Начнём",
  "sw": "Tuanzie",
  "pl": "Zaczynajmy",
  "id": "Mari mulai",
  "fa": "بیایید شروع کنیم",
  "pa": "ਆਓ ਸ਼ੁਰੂ ਕਰੀਏ",
  "uk": "Почнімо",
  "ro": "Să începem",
  "tl": "Magsimula na tayo"
},



"signIn":{
  "en": "Sign In",
  "fr": "Se connecter",
  "de": "Anmelden",
  "ar": "تسجيل الدخول",
  "es": "Iniciar sesión",
  "tr": "Giriş yap",
  "nl": "Inloggen",
  "it": "Accedi",
  "ja": "サインイン",
  "zh": "登录",
  "ko": "로그인",
  "hi": "साइन इन",
  "pt": "Entrar",
  "ru": "Войти",
  "sw": "Ingia",
  "pl": "Zaloguj się",
  "id": "Masuk",
  "fa": "ورود",
  "pa": "ਸਾਈਨ ਇਨ",
  "uk": "Увійти",
  "ro": "Autentificare",
  "tl": "Mag-sign in"
},




"previous":{
  "en": "Previous",
  "fr": "Précédent",
  "de": "Zurück",
  "ar": "السابق",
  "es": "Anterior",
  "tr": "Geri",
  "nl": "Vorige",
  "it": "Precedente",
  "ja": "前へ",
  "zh": "上一步",
  "ko": "이전",
  "hi": "पिछला",
  "pt": "Anterior",
  "ru": "Назад",
  "sw": "Iliyopita",
  "pl": "Poprzedni",
  "id": "Sebelumnya",
  "fa": "قبلی",
  "pa": "ਪਿਛਲਾ",
  "uk": "Попередній",
  "ro": "Anterior",
  "tl": "Nakaraan"
},




"chooseLang":{
  "en": "Choose a language to continue",
  "fr": "Choisissez une langue pour continuer",
  "de": "Wählen Sie eine Sprache, um fortzufahren",
  "ar": "اختر لغة للمتابعة",
  "es": "Elige un idioma para continuar",
  "tr": "Devam etmek için bir dil seçin",
  "nl": "Kies een taal om door te gaan",
  "it": "Scegli una lingua per continuare",
  "ja": "続行する言語を選択してください",
  "zh": "选择一种语言以继续",
  "ko": "계속하려면 언어를 선택하세요",
  "hi": "जारी रखने के लिए भाषा चुनें",
  "pt": "Escolha um idioma para continuar",
  "ru": "Выберите язык, чтобы продолжить",
  "sw": "Chagua lugha ili kuendelea",
  "pl": "Wybierz język, aby kontynuować",
  "id": "Pilih bahasa untuk melanjutkan",
  "fa": "برای ادامه یک زبان انتخاب کنید",
  "pa": "ਜਾਰੀ ਰੱਖਣ ਲਈ ਭਾਸ਼ਾ ਚੁਣੋ",
  "uk": "Оберіть мову, щоб продовжити",
  "ro": "Alegeți o limbă pentru a continua",
  "tl": "Pumili ng wika upang magpatuloy"
},




"next":{
  "en": "Next",
  "fr": "Suivant",
  "de": "Weiter",
  "ar": "التالي",
  "es": "Siguiente",
  "tr": "İleri",
  "nl": "Volgende",
  "it": "Avanti",
  "ja": "次へ",
  "zh": "下一步",
  "ko": "다음",
  "hi": "आगे",
  "pt": "Próximo",
  "ru": "Далее",
  "sw": "Ifuatayo",
  "pl": "Dalej",
  "id": "Berikutnya",
  "fa": "بعدی",
  "pa": "ਅਗਲਾ",
  "uk": "Далі",
  "ro": "Următorul",
  "tl": "Susunod"
},




"skip":{
  "en": "Skip",
  "fr": "Ignorer",
  "de": "Überspringen",
  "ar": "تخطي",
  "es": "Omitir",
  "tr": "Atla",
  "nl": "Overslaan",
  "it": "Salta",
  "ja": "スキップ",
  "zh": "跳过",
  "ko": "건너뛰기",
  "hi": "छोड़ें",
  "pt": "Pular",
  "ru": "Пропустить",
  "sw": "Ruka",
  "pl": "Pomiń",
  "id": "Lewati",
  "fa": "رد کردن",
  "pa": "ਛੱਡੋ",
  "uk": "Пропустити",
  "ro": "Sari",
  "tl": "Laktawan"
},





"onboardia":{
  "en": 'Understand the world,',
  "fr": 'Comprendre le monde,',
  "de": 'Verstehe die Welt,',
  "ar": 'افهم العالم,',
  "es": 'Comprende el mundo',
  "tr": 'Dünyayı anla,',
  "nl": 'Begrijp de wereld,',
  "it": 'Comprendi il mondo,',
  "ja": '世界を 理解する,',
  "zh": '理解这个 世界,',
  "ko": '세상을 이해하세요,',
  "hi": 'दुनिया को समझें,',
  "pt": 'Entenda o mundo,',
  'ru': 'Понимай мир,',
  "sw": 'Elewa dunia,',
  "pl": 'Zrozum świat,',
  "id": 'Pahami dunia,',
  "fa": 'جهان را درک کن,',
  "pa": 'ਦੁਨੀਆ ਨੂੰ ਸਮਝੋ,',
  "uk": 'Зрозумій світ,',
  "ro": 'Înțelege lumea,',
  "tl": 'Unawain ang mundo,'
},


"oneStory":{
  "en": "One story at a time",
  "fr": "Une histoire à la fois",
  "de": "Eine Geschichte nach der anderen",
  "ar": "قصة واحدة في كل مرة",
  "es": "Una historia a la vez",
  "tr": "Her seferinde bir hikaye",
  "nl": "Eén verhaal tegelijk",
  "it": "Una storia alla volta",
  "ja": "一度に一つのストーリー",
  "zh": "一次一个故事",
  "ko": "한 번에 하나의 이야기",
  "hi": "एक समय में एक कहानी",
  "pt": "Uma história de cada vez",
  "ru": "Одна история за раз",
  "sw": "Hadithi moja kwa wakati mmoja",
  "pl": "Jedna historia na raz",
  "id": "Satu cerita dalam satu waktu",
  "fa": "یک داستان در هر بار",
  "pa": "ਇੱਕ ਵਾਰ ਵਿੱਚ ਇੱਕ ਕਹਾਣੀ",
  "uk": "Одна історія за раз",
  "ro": "O poveste pe rând",
  "tl": "Isang kwento sa bawat pagkakataon"
},


"emailSent":{
  'en': "Email sent!",
  'fr': "E-mail envoyé !",
  'de': "E-Mail gesendet!",
  'ar': "تم إرسال البريد الإلكتروني!",
  'es': "¡Correo enviado!",
  'tr': "E-posta gönderildi!",
  'nl': "E-mail verzonden!",
  'it': "Email inviata!",
  'ja': "メールを送信しました！",
  'zh': "邮件已发送！",
  'ko': "이메일이 전송되었습니다!",
  'hi': "ईमेल भेज दिया गया!",
  'pt': "E-mail enviado!",
  'ru': "Письмо отправлено!",
  'sw': "Barua pepe imetumwa!",
  'pl': "E-mail wysłany!",
  'id': "Email berhasil dikirim!",
  'fa': "ایمیل ارسال شد!",
  'pa': "ਈਮੇਲ ਭੇਜੀ ਗਈ!",
  'uk': "Електронний лист надіслано!",
  'ro': "E-mail trimis!",
  'tl': "Naipadala ang email!"
},



"error":{
  'en': "Error! Try again...",
  'fr': "Erreur ! Réessayez...",
  'de': "Fehler! Versuchen Sie es erneut...",
  'ar': "خطأ! حاول مرة أخرى...",
  'es': "¡Error! Inténtalo de nuevo...",
  'tr': "Hata! Tekrar deneyin...",
  'nl': "Fout! Probeer opnieuw...",
  'it': "Errore! Riprova...",
  'ja': "エラーです。もう一度お試しください…",
  'zh': "出错了！请再试一次…",
  'ko': "오류가 발생했습니다! 다시 시도하세요...",
  'hi': "त्रुटि! फिर से प्रयास करें...",
  'pt': "Erro! Tente novamente...",
  'ru': "Ошибка! Попробуйте снова...",
  'sw': "Hitilafu! Jaribu tena...",
  'pl': "Błąd! Spróbuj ponownie...",
  'id': "Kesalahan! Coba lagi...",
  'fa': "خطا! دوباره تلاش کنید...",
  'pa': "ਗਲਤੀ! ਮੁੜ ਕੋਸ਼ਿਸ਼ ਕਰੋ...",
  'uk': "Помилка! Спробуйте ще раз...",
  'ro': "Eroare! Încercați din nou...",
  'tl': "Error! Subukang muli..."
},




"invalidC":{
  'en': "Invalid code!",
  'fr': "Code invalide !",
  'de': "Ungültiger Code!",
  'ar': "رمز غير صالح!",
  'es': "¡Código inválido!",
  'tr': "Geçersiz kod!",
  'nl': "Ongeldige code!",
  'it': "Codice non valido!",
  'ja': "無効なコードです！",
  'zh': "验证码无效！",
  'ko': "잘못된 코드입니다!",
  'hi': "अमान्य कोड!",
  'pt': "Código inválido!",
  'ru': "Неверный код!",
  'sw': "Msimbo batili!",
  'pl': "Nieprawidłowy kod!",
  'id': "Kode tidak valid!",
  'fa': "کد نامعتبر!",
  'pa': "ਗਲਤ ਕੋਡ!",
  'uk': "Недійсний код!",
  'ro': "Cod invalid!",
  'tl': "Hindi wastong code!"
},




"expired":{
  'en': "Expired code",
  'fr': "Code expiré",
  'de': "Code abgelaufen",
  'ar': "انتهت صلاحية الرمز",
  'es': "Código expirado",
  'tr': "Kodu süresi doldu",
  'nl': "Code verlopen",
  'it': "Codice scaduto",
  'ja': "コードの有効期限が切れています",
  'zh': "验证码已过期",
  'ko': "코드가 만료되었습니다",
  'hi': "कोड की अवधि समाप्त हो गई है",
  'pt': "Código expirado",
  'ru': "Срок действия кода истёк",
  'sw': "Msimbo umeisha muda wake",
  'pl': "Kod wygasł",
  'id': "Kode kedaluwarsa",
  'fa': "کد منقضی شده است",
  'pa': "ਕੋਡ ਦੀ ਮਿਆਦ ਖਤਮ ਹੋ ਗਈ ਹੈ",
  'uk': "Термін дії коду закінчився",
  'ro': "Cod expirat",
  'tl': "Nag-expire ang code"
},



"passUpdated":{
  'en': "Password Updated",
  'fr': "Mot de passe mis à jour",
  'de': "Passwort aktualisiert",
  'ar': "تم تحديث كلمة المرور",
  'es': "Contraseña actualizada",
  'tr': "Şifre güncellendi",
  'nl': "Wachtwoord bijgewerkt",
  'it': "Password aggiornata",
  'ja': "パスワードが更新されました",
  'zh': "密码已更新",
  'ko': "비밀번호가 업데이트되었습니다",
  'hi': "पासवर्ड अपडेट किया गया",
  'pt': "Senha atualizada",
  'ru': "Пароль обновлён",
  'sw': "Nenosiri limesasishwa",
  'pl': "Hasło zaktualizowane",
  'id': "Kata sandi diperbarui",
  'fa': "رمز عبور به‌روزرسانی شد",
  'pa': "ਪਾਸਵਰਡ ਅਪਡੇਟ ਹੋ ਗਿਆ",
  'uk': "Пароль оновлено",
  'ro': "Parolă actualizată",
  'tl': "Na-update ang password"
},


"unameExists":{
  'en': "Username Exists!",
  'fr': "Le nom d’utilisateur existe déjà !",
  'de': "Benutzername existiert bereits!",
  'ar': "اسم المستخدم موجود بالفعل!",
  'es': "¡El nombre de usuario ya existe!",
  'tr': "Kullanıcı adı zaten mevcut!",
  'nl': "Gebruikersnaam bestaat al!",
  'it': "Nome utente già esistente!",
  'ja': "ユーザー名はすでに存在します！",
  'zh': "用户名已存在！",
  'ko': "이미 존재하는 사용자 이름입니다!",
  'hi': "उपयोगकर्ता नाम पहले से मौजूद है!",
  'pt': "Nome de usuário já existe!",
  'ru': "Имя пользователя уже существует!",
  'sw': "Jina la mtumiaji tayari lipo!",
  'pl': "Nazwa użytkownika już istnieje!",
  'id': "Nama pengguna sudah ada!",
  'fa': "نام کاربری از قبل وجود دارد!",
  'pa': "ਯੂਜ਼ਰਨੇਮ ਪਹਿਲਾਂ ਹੀ ਮੌਜੂਦ ਹੈ!",
  'uk': "Ім’я користувача вже існує!",
  'ro': "Numele de utilizator există deja!",
  'tl': "May umiiral nang username!"
},



"fromNEWSW":{
  'en': "From NEWSWORLD",
  'fr': "De NEWSWORLD",
  'de': "Von NEWSWORLD",
  'ar': "من NEWSWORLD",
  'es': "De NEWSWORLD",
  'tr': "NEWSWORLD’dan",
  'nl': "Van NEWSWORLD",
  'it': "Da NEWSWORLD",
  'ja': "NEWSWORLDより",
  'zh': "来自 NEWSWORLD",
  'ko': "NEWSWORLD에서",
  'hi': "NEWSWORLD से",
  'pt': "De NEWSWORLD",
  'ru': "От NEWSWORLD",
  'sw': "Kutoka NEWSWORLD",
  'pl': "Od NEWSWORLD",
  'id': "Dari NEWSWORLD",
  'fa': "از NEWSWORLD",
  'pa': "NEWSWORLD ਵੱਲੋਂ",
  'uk': "Від NEWSWORLD",
  'ro': "De la NEWSWORLD",
  'tl': "Mula sa NEWSWORLD"
},


"hello":{
  'en': "Hello",
  'fr': "Bonjour",
  'de': "Hallo",
  'ar': "مرحبًا",
  'es': "Hola",
  'tr': "Merhaba",
  'nl': "Hallo",
  'it': "Ciao",
  'ja': "こんにちは",
  'zh': "你好",
  'ko': "안녕하세요",
  'hi': "नमस्ते",
  'pt': "Olá",
  'ru': "Здравствуйте",
  'sw': "Habari",
  'pl': "Cześć",
  'id': "Halo",
  'fa': "سلام",
  'pa': "ਸਤ ਸ੍ਰੀ ਅਕਾਲ",
  'uk': "Вітаємо",
  'ro': "Salut",
  'tl': "Kumusta"
},




"invalidPass":{
  'en': "Invalid Password",
  'fr': "Mot de passe invalide",
  'de': "Ungültiges Passwort",
  'ar': "كلمة مرور غير صالحة",
  'es': "Contraseña inválida",
  'tr': "Geçersiz şifre",
  'nl': "Ongeldig wachtwoord",
  'it': "Password non valida",
  'ja': "無効なパスワード",
  'zh': "密码无效",
  'ko': "잘못된 비밀번호",
  'hi': "अमान्य पासवर्ड",
  'pt': "Senha inválida",
  'ru': "Неверный пароль",
  'sw': "Nenosiri batili",
  'pl': "Nieprawidłowe hasło",
  'id': "Kata sandi tidak valid",
  'fa': "رمز عبور نامعتبر",
  'pa': "ਗਲਤ ਪਾਸਵਰਡ",
  'uk': "Недійсний пароль",
  'ro': "Parolă invalidă",
  'tl': "Hindi wastong password"
},



"threeMore":{
  'en': "three or more characters",
  'fr': "trois caractères ou plus",
  'de': "drei oder mehr Zeichen",
  'ar': "ثلاثة أحرف أو أكثر",
  'es': "tres o más caracteres",
  'tr': "üç veya daha fazla karakter",
  'nl': "drie of meer tekens",
  'it': "tre o più caratteri",
  'ja': "3文字以上",
  'zh': "三个或更多字符",
  'ko': "세 글자 이상",
  'hi': "तीन या अधिक अक्षर",
  'pt': "três ou mais caracteres",
  'ru': "три или более символа",
  'sw': "herufi tatu au zaidi",
  'pl': "trzy lub więcej znaków",
  'id': "tiga karakter atau lebih",
  'fa': "سه کاراکتر یا بیشتر",
  'pa': "ਤਿੰਨ ਜਾਂ ਵੱਧ ਅੱਖਰ",
  'uk': "три або більше символів",
  'ro': "trei sau mai multe caractere",
  'tl': "tatlo o higit pang karakter"
},



"fiveMore":{
  'en': "five or more digits including a number and symbol like [@,&,%,*,$]",
  'fr': "cinq caractères ou plus incluant un chiffre et un symbole comme [@,&,%,*,$]",
  'de': "fünf oder mehr Zeichen einschließlich einer Zahl und eines Symbols wie [@,&,%,*,$]",
  'ar': "خمسة أحرف أو أكثر تشمل رقمًا ورمزًا مثل [@,&,%,*,$]",
  'es': "cinco o más caracteres que incluyan un número y un símbolo como [@,&,%,*,$]",
  'tr': "[@,&,%,*,$] gibi bir sayı ve sembol içeren beş veya daha fazla karakter",
  'nl': "vijf of meer tekens inclusief een cijfer en een symbool zoals [@,&,%,*,$]",
  'it': "cinque o più caratteri inclusi un numero e un simbolo come [@,&,%,*,$]",
  'ja': "数字と[@,&,%,*,$]などの記号を含む5文字以上",
  'zh': "至少五个字符，包含数字和符号（如 [@,&,%,*,$]）",
  'ko': "숫자와 [@,&,%,*,$] 같은 기호를 포함한 다섯 자 이상",
  'hi': "पाँच या अधिक अक्षर जिनमें संख्या और [@,&,%,*,$] जैसे चिन्ह शामिल हों",
  'pt': "cinco ou mais caracteres incluindo um número e um símbolo como [@,&,%,*,$]",
  'ru': "пять или более символов, включая цифру и символ, например [@,&,%,*,$]",
  'sw': "herufi tano au zaidi zikiwemo namba na alama kama [@,&,%,*,$]",
  'pl': "pięć lub więcej znaków zawierających cyfrę i symbol, np. [@,&,%,*,$]",
  'id': "lima karakter atau lebih termasuk angka dan simbol seperti [@,&,%,*,$]",
  'fa': "پنج کاراکتر یا بیشتر شامل عدد و نمادی مانند [@,&,%,*,$]",
  'pa': "ਪੰਜ ਜਾਂ ਵੱਧ ਅੱਖਰ, ਜਿਸ ਵਿੱਚ ਅੰਕ ਅਤੇ [@,&,%,*,$] ਵਰਗਾ ਚਿੰਨ੍ਹ ਸ਼ਾਮਲ ਹੋਵੇ",
  'uk': "п’ять або більше символів, включаючи цифру та символ, як-от [@,&,%,*,$]",
  'ro': "cinci sau mai multe caractere, inclusiv un număr și un simbol precum [@,&,%,*,$]",
  'tl': "limang o higit pang karakter na may numero at simbolo tulad ng [@,&,%,*,$]"
},





"passwordDont":{
  'en': "Password do not match",
  'fr': "Les mots de passe ne correspondent pas",
  'de': "Passwörter stimmen nicht überein",
  'ar': "كلمتا المرور غير متطابقتين",
  'es': "Las contraseñas no coinciden",
  'tr': "Şifreler eşleşmiyor",
  'nl': "Wachtwoorden komen niet overeen",
  'it': "Le password non corrispondono",
  'ja': "パスワードが一致しません",
  'zh': "密码不匹配",
  'ko': "비밀번호가 일치하지 않습니다",
  'hi': "पासवर्ड मेल नहीं खाते",
  'pt': "As senhas não coincidem",
  'ru': "Пароли не совпадают",
  'sw': "Nenosiri hazilingani",
  'pl': "Hasła nie są zgodne",
  'id': "Kata sandi tidak cocok",
  'fa': "رمزهای عبور مطابقت ندارند",
  'pa': "ਪਾਸਵਰਡ ਮਿਲਦੇ ਨਹੀਂ ਹਨ",
  'uk': "Паролі не збігаються",
  'ro': "Parolele nu se potrivesc",
  'tl': "Hindi magkatugma ang mga password"
},


"yliveLocation":{
  'en': "Your Live Location is mandatory to give you the best experience in sourcing news",
  'fr': "Votre localisation en direct est obligatoire pour vous offrir la meilleure expérience dans la recherche d’actualités",
  'de': "Ihr Live-Standort ist erforderlich, um Ihnen das beste Erlebnis bei der Nachrichtenbeschaffung zu bieten",
  'ar': "موقعك المباشر إلزامي لمنحك أفضل تجربة في الحصول على الأخبار",
  'es': "Tu ubicación en tiempo real es obligatoria para ofrecerte la mejor experiencia al acceder a noticias",
  'tr': "Haberleri en iyi şekilde sunabilmek için canlı konumunuz gereklidir",
  'nl': "Uw live locatie is vereist om u de beste ervaring te bieden bij het verkrijgen van nieuws",
  'it': "La tua posizione in tempo reale è necessaria per offrirti la migliore esperienza nella consultazione delle notizie",
  'ja': "最適なニュース体験を提供するために、現在地の利用が必要です",
  'zh': "为了为您提供最佳的新闻体验，需要启用实时位置",
  'ko': "최상의 뉴스 경험을 제공하기 위해 실시간 위치가 필요합니다",
  'hi': "समाचार प्राप्त करने का सर्वोत्तम अनुभव देने के लिए आपकी लाइव लोकेशन आवश्यक है",
  'pt': "Sua localização em tempo real é obrigatória para oferecer a melhor experiência na busca por notícias",
  'ru': "Для наилучшего новостного опыта требуется доступ к вашему текущему местоположению",
  'sw': "Mahali ulipo moja kwa moja ni muhimu ili kukupa uzoefu bora wa kupata habari",
  'pl': "Twoja lokalizacja na żywo jest wymagana, aby zapewnić najlepsze doświadczenie w dostępie do wiadomości",
  'id': "Lokasi langsung Anda diperlukan untuk memberikan pengalaman terbaik dalam mengakses berita",
  'fa': "برای ارائه بهترین تجربه در دریافت اخبار، موقعیت مکانی زنده شما الزامی است",
  'pa': "ਖ਼ਬਰਾਂ ਦਾ ਸਭ ਤੋਂ ਵਧੀਆ ਤਜਰਬਾ ਦੇਣ ਲਈ ਤੁਹਾਡਾ ਲਾਈਵ ਟਿਕਾਣਾ ਲਾਜ਼ਮੀ ਹੈ",
  'uk': "Для найкращого досвіду отримання новин необхідний доступ до вашого поточного місцезнаходження",
  'ro': "Localizarea ta în timp real este necesară pentru a-ți oferi cea mai bună experiență în accesarea știrilor",
  'tl': "Kinakailangan ang iyong live location upang maibigay ang pinakamahusay na karanasan sa pagkuha ng balita"
},



"Name": {
    'en': "Name",
    'fr': "Nom",
    'de': "Name",
    'ar': "الاسم",
    'es': "Nombre",
    'tr': "İsim",
    'nl': "Naam",
    'it': "Nome",
    'ja': "名前",
    'zh': "姓名",
    'ko': "이름",
    'hi': "नाम",
    'pt': "Nome",
    'ru': "Имя",
    'sw': "Jina",
    'pl': "Imię",
    'id': "Nama",
    'fa': "نام",
    'pa': "ਨਾਮ",
    'uk': "Ім’я",
    'ro': "Nume",
    'tl': "Pangalan"
  },

  "DateofBirth": {
    'en': "Date of Birth",
    'fr': "Date de naissance",
    'de': "Geburtsdatum",
    'ar': "تاريخ الميلاد",
    'es': "Fecha de nacimiento",
    'tr': "Doğum tarihi",
    'nl': "Geboortedatum",
    'it': "Data di nascita",
    'ja': "生年月日",
    'zh': "出生日期",
    'ko': "생년월일",
    'hi': "जन्म तिथि",
    'pt': "Data de nascimento",
    'ru': "Дата рождения",
    'sw': "Tarehe ya kuzaliwa",
    'pl': "Data urodzenia",
    'id': "Tanggal lahir",
    'fa': "تاریخ تولد",
    'pa': "ਜਨਮ ਤਾਰੀਖ",
    'uk': "Дата народження",
    'ro': "Data nașterii",
    'tl': "Petsa ng kapanganakan"
  },

  "SelectDate": {
    'en': "Select Date of Birth",
    'fr': "Sélectionnez la date de naissance",
    'de': "Geburtsdatum auswählen",
    'ar': "اختر تاريخ الميلاد",
    'es': "Selecciona la fecha de nacimiento",
    'tr': "Doğum tarihini seç",
    'nl': "Selecteer geboortedatum",
    'it': "Seleziona la data di nascita",
    'ja': "生年月日を選択",
    'zh': "选择出生日期",
    'ko': "생년월일 선택",
    'hi': "जन्म तिथि चुनें",
    'pt': "Selecione a data de nascimento",
    'ru': "Выберите дату рождения",
    'sw': "Chagua tarehe ya kuzaliwa",
    'pl': "Wybierz datę urodzenia",
    'id': "Pilih tanggal lahir",
    'fa': "تاریخ تولد را انتخاب کنید",
    'pa': "ਜਨਮ ਤਾਰੀਖ ਚੁਣੋ",
    'uk': "Оберіть дату народження",
    'ro': "Selectează data nașterii",
    'tl': "Piliin ang petsa ng kapanganakan"
  },

  "Gender": {
    'en': "Gender",
    'fr': "Genre",
    'de': "Geschlecht",
    'ar': "الجنس",
    'es': "Género",
    'tr': "Cinsiyet",
    'nl': "Geslacht",
    'it': "Genere",
    'ja': "性別",
    'zh': "性别",
    'ko': "성별",
    'hi': "लिंग",
    'pt': "Gênero",
    'ru': "Пол",
    'sw': "Jinsia",
    'pl': "Płeć",
    'id': "Jenis kelamin",
    'fa': "جنسیت",
    'pa': "ਲਿੰਗ",
    'uk': "Стать",
    'ro': "Gen",
    'tl': "Kasarian"
  },

  "SelectGender": {
    'en': "Select Gender",
    'fr': "Sélectionnez le genre",
    'de': "Geschlecht auswählen",
    'ar': "اختر الجنس",
    'es': "Selecciona el género",
    'tr': "Cinsiyet seç",
    'nl': "Selecteer geslacht",
    'it': "Seleziona genere",
    'ja': "性別を選択",
    'zh': "选择性别",
    'ko': "성별 선택",
    'hi': "लिंग चुनें",
    'pt': "Selecione o gênero",
    'ru': "Выберите пол",
    'sw': "Chagua jinsia",
    'pl': "Wybierz płeć",
    'id': "Pilih jenis kelamin",
    'fa': "جنسیت را انتخاب کنید",
    'pa': "ਲਿੰਗ ਚੁਣੋ",
    'uk': "Оберіть стать",
    'ro': "Selectează genul",
    'tl': "Piliin ang kasarian"
  },

  "Location": {
    'en': "Location",
    'fr': "Localisation",
    'de': "Standort",
    'ar': "الموقع",
    'es': "Ubicación",
    'tr': "Konum",
    'nl': "Locatie",
    'it': "Posizione",
    'ja': "場所",
    'zh': "位置",
    'ko': "위치",
    'hi': "स्थान",
    'pt': "Localização",
    'ru': "Местоположение",
    'sw': "Eneo",
    'pl': "Lokalizacja",
    'id': "Lokasi",
    'fa': "موقعیت",
    'pa': "ਟਿਕਾਣਾ",
    'uk': "Місцезнаходження",
    'ro': "Locație",
    'tl': "Lokasyon"
  },

  "Enable": {
    'en': "Enable",
    'fr': "Activer",
    'de': "Aktivieren",
    'ar': "تمكين",
    'es': "Habilitar",
    'tr': "Etkinleştir",
    'nl': "Inschakelen",
    'it': "Abilita",
    'ja': "有効にする",
    'zh': "启用",
    'ko': "활성화",
    'hi': "सक्षम करें",
    'pt': "Ativar",
    'ru': "Включить",
    'sw': "Wezesha",
    'pl': "Włącz",
    'id': "Aktifkan",
    'fa': "فعال‌سازی",
    'pa': "ਚਾਲੂ ਕਰੋ",
    'uk': "Увімкнути",
    'ro': "Activează",
    'tl': "Paganahin"
  },

  "NotEnabled": {
    'en': "Not Enabled",
    'fr': "Non activé",
    'de': "Nicht aktiviert",
    'ar': "غير مفعّل",
    'es': "No habilitado",
    'tr': "Etkin değil",
    'nl': "Niet ingeschakeld",
    'it': "Non abilitato",
    'ja': "無効",
    'zh': "未启用",
    'ko': "비활성화됨",
    'hi': "सक्षम नहीं",
    'pt': "Não ativado",
    'ru': "Не включено",
    'sw': "Haijawezeshwa",
    'pl': "Nieaktywne",
    'id': "Tidak diaktifkan",
    'fa': "غیرفعال",
    'pa': "ਚਾਲੂ ਨਹੀਂ",
    'uk': "Вимкнено",
    'ro': "Dezactivat",
    'tl': "Hindi pinagana"
},



"fillingdetls":{
  'en': "Complete your profile by filling in the details",
  'fr': "Complétez votre profil en renseignant les informations",
  'de': "Vervollständigen Sie Ihr Profil, indem Sie die Details ausfüllen",
  'ar': "أكمل ملفك الشخصي بإدخال التفاصيل",
  'es': "Completa tu perfil rellenando los datos",
  'tr': "Bilgileri doldurarak profilinizi tamamlayın",
  'nl': "Vul uw profiel aan door de gegevens in te vullen",
  'it': "Completa il tuo profilo inserendo i dettagli",
  'ja': "詳細を入力してプロフィールを完成させてください",
  'zh': "填写信息以完成您的个人资料",
  'ko': "정보를 입력하여 프로필을 완성하세요",
  'hi': "विवरण भरकर अपनी प्रोफ़ाइल पूरी करें",
  'pt': "Complete seu perfil preenchendo os dados",
  'ru': "Заполните данные, чтобы завершить профиль",
  'sw': "Kamilisha wasifu wako kwa kujaza maelezo",
  'pl': "Uzupełnij profil, wpisując wymagane dane",
  'id': "Lengkapi profil dengan mengisi detail",
  'fa': "با وارد کردن جزئیات، پروفایل خود را تکمیل کنید",
  'pa': "ਵੇਰਵੇ ਭਰ ਕੇ ਆਪਣੀ ਪ੍ਰੋਫ਼ਾਈਲ ਪੂਰੀ ਕਰੋ",
  'uk': "Заповніть дані, щоб завершити профіль",
  'ro': "Completează-ți profilul completând detaliile",
  'tl': "Kumpletuhin ang iyong profile sa pamamagitan ng paglalagay ng detalye"
},



"complteProfile":{
  'en': "Complete Profile",
  'fr': "Compléter le profil",
  'de': "Profil vervollständigen",
  'ar': "إكمال الملف الشخصي",
  'es': "Completar perfil",
  'tr': "Profili tamamla",
  'nl': "Profiel voltooien",
  'it': "Completa profilo",
  'ja': "プロフィールを完成させる",
  'zh': "完善个人资料",
  'ko': "프로필 완료",
  'hi': "प्रोफ़ाइल पूरा करें",
  'pt': "Completar perfil",
  'ru': "Заполнить профиль",
  'sw': "Kamilisha wasifu",
  'pl': "Uzupełnij profil",
  'id': "Lengkapi profil",
  'fa': "تکمیل پروفایل",
  'pa': "ਪ੍ਰੋਫ਼ਾਈਲ ਪੂਰਾ ਕਰੋ",
  'uk': "Заповнити профіль",
  'ro': "Completează profilul",
  'tl': "Kumpletuhin ang profile"
},


"verify":{
  'en': "Verify",
  'fr': "Vérifier",
  'de': "Bestätigen",
  'ar': "تحقق",
  'es': "Verificar",
  'tr': "Doğrula",
  'nl': "Verifiëren",
  'it': "Verifica",
  'ja': "確認",
  'zh': "验证",
  'ko': "확인",
  'hi': "सत्यापित करें",
  'pt': "Verificar",
  'ru': "Подтвердить",
  'sw': "Thibitisha",
  'pl': "Zweryfikuj",
  'id': "Verifikasi",
  'fa': "تأیید",
  'pa': "ਤਸਦੀਕ ਕਰੋ",
  'uk': "Підтвердити",
  'ro': "Verifică",
  'tl': "I-verify"
},



"pleaseVerify":{
  "en": "Please verify email by entering the code",
  "fr": "Veuillez vérifier votre e-mail en entrant le code",
  "de": "Bitte bestätigen Sie Ihre E-Mail, indem Sie den Code eingeben",
  "ar": "يرجى تأكيد البريد الإلكتروني بإدخال الرمز",
  "es": "Verifica el correo ingresando el código",
  "tr": "Lütfen kodu girerek e-postanızı doğrulayın",
  "nl": "Verifieer uw e-mail door de code in te voeren",
  "it": "Verifica l’email inserendo il codice",
  "ja": "コードを入力してメールを確認してください",
  "zh": "请输入验证码以验证邮箱",
  "ko": "코드를 입력하여 이메일을 인증하세요",
  "hi": "कोड दर्ज करके ईमेल सत्यापित करें",
  "pt": "Verifique o e-mail inserindo o código",
  "ru": "Пожалуйста, подтвердите почту, введя код",
  "sw": "Tafadhali thibitisha barua pepe kwa kuingiza msimbo",
  "pl": "Zweryfikuj e-mail, wpisując kod",
  "id": "Silakan verifikasi email dengan memasukkan kode",
  "fa": "لطفاً ایمیل را با وارد کردن کد تأیید کنید",
  "pa": "ਕੋਡ ਦਰਜ ਕਰਕੇ ਈਮੇਲ ਦੀ ਪੁਸ਼ਟੀ ਕਰੋ",
  "uk": "Підтвердьте електронну пошту, ввівши код",
  "ro": "Verifică e-mailul introducând codul",
  "tl": "I-verify ang email sa pamamagitan ng paglalagay ng code"
},


"eprovided":{
  "en": "sent to the email provided",
  "fr": "envoyé à l’adresse e-mail fournie",
  "de": "an die angegebene E-Mail-Adresse gesendet",
  "ar": "المرسل إلى البريد الإلكتروني المُقدّم",
  "es": "enviado al correo proporcionado",
  "tr": "sağlanan e-posta adresine gönderildi",
  "nl": "verzonden naar het opgegeven e-mailadres",
  "it": "inviato all’indirizzo email fornito",
  "ja": "入力されたメールアドレスに送信されました",
  "zh": "已发送至所提供的邮箱",
  "ko": "입력한 이메일로 전송되었습니다",
  "hi": "प्रदान किए गए ईमेल पर भेजा गया",
  "pt": "enviado para o e-mail fornecido",
  "ru": "отправлено на указанный адрес электронной почты",
  "sw": "imetumwa kwenye barua pepe uliyoitoa",
  "pl": "wysłany na podany adres e-mail",
  "id": "dikirim ke email yang diberikan",
  "fa": "به ایمیل ارائه‌شده ارسال شد",
  "pa": "ਦਿੱਤੇ ਗਏ ਈਮੇਲ ’ਤੇ ਭੇਜਿਆ ਗਿਆ",
  "uk": "надіслано на вказану електронну пошту",
  "ro": "trimis la adresa de e-mail furnizată",
  "tl": "ipinadala sa ibinigay na email"
},



"termsService":{
  'en': "Terms of service",
  'fr': "Conditions d’utilisation",
  'de': "Nutzungsbedingungen",
  'ar': "شروط الخدمة",
  'es': "Términos de servicio",
  'tr': "Hizmet şartları",
  'nl': "Servicevoorwaarden",
  'it': "Termini di servizio",
  'ja': "利用規約",
  'zh': "服务条款",
  'ko': "서비스 약관",
  'hi': "सेवा की शर्तें",
  'pt': "Termos de serviço",
  'ru': "Условия использования",
  'sw': "Masharti ya huduma",
  'pl': "Warunki korzystania",
  'id': "Ketentuan layanan",
  'fa': "شرایط استفاده",
  'pa': "ਸੇਵਾ ਦੀਆਂ ਸ਼ਰਤਾਂ",
  'uk': "Умови користування",
  'ro': "Termeni și condiții",
  'tl': "Mga tuntunin ng serbisyo"
},



"bySigning":{
  'en': "By signing up you agree to our",
  'fr': "En vous inscrivant, vous acceptez nos",
  'de': "Mit der Registrierung stimmen Sie unseren",
  'ar': "بالتسجيل فإنك توافق على",
  'es': "Al registrarte aceptas nuestros",
  'tr': "Kayıt olarak şartlarımızı kabul etmiş olursunuz",
  'nl': "Door je aan te melden ga je akkoord met onze",
  'it': "Iscrivendoti accetti i nostri",
  'ja': "登録することで、次の内容に同意したことになります：",
  'zh': "注册即表示您同意我们的",
  'ko': "가입함으로써 귀하는 다음에 동의하게 됩니다",
  'hi': "साइन अप करने पर आप हमारी शर्तों से सहमत होते हैं",
  'pt': "Ao se cadastrar, você concorda com nossos",
  'ru': "Регистрируясь, вы соглашаетесь с нашими",
  'sw': "Kwa kujisajili unakubali",
  'pl': "Rejestrując się, zgadzasz się z naszymi",
  'id': "Dengan mendaftar, Anda menyetujui",
  'fa': "با ثبت‌نام، شما با شرایط ما موافقت می‌کنید",
  'pa': "ਸਾਈਨ ਅੱਪ ਕਰਕੇ ਤੁਸੀਂ ਸਾਡੀਆਂ ਸ਼ਰਤਾਂ ਨਾਲ ਸਹਿਮਤ ਹੋ",
  'uk': "Реєструючись, ви погоджуєтеся з нашими",
  'ro': "Prin înregistrare, ești de acord cu",
  'tl': "Sa pag-sign up, sumasang-ayon ka sa aming"
},




"username":{
  'en': "Username",
  'fr': "Nom d’utilisateur",
  'de': "Benutzername",
  'ar': "اسم المستخدم",
  'es': "Nombre de usuario",
  'tr': "Kullanıcı adı",
  'nl': "Gebruikersnaam",
  'it': "Nome utente",
  'ja': "ユーザー名",
  'zh': "用户名",
  'ko': "사용자 이름",
  'hi': "उपयोगकर्ता नाम",
  'pt': "Nome de usuário",
  'ru': "Имя пользователя",
  'sw': "Jina la mtumiaji",
  'pl': "Nazwa użytkownika",
  'id': "Nama pengguna",
  'fa': "نام کاربری",
  'pa': "ਯੂਜ਼ਰਨੇਮ",
  'uk': "Ім’я користувача",
  'ro': "Nume de utilizator",
  'tl': "Username"
},



"password":{
  'en': "Password",
  'fr': "Mot de passe",
  'de': "Passwort",
  'ar': "كلمة المرور",
  'es': "Contraseña",
  'tr': "Şifre",
  'nl': "Wachtwoord",
  'it': "Password",
  'ja': "パスワード",
  'zh': "密码",
  'ko': "비밀번호",
  'hi': "पासवर्ड",
  'pt': "Senha",
  'ru': "Пароль",
  'sw': "Nenosiri",
  'pl': "Hasło",
  'id': "Kata sandi",
  'fa': "رمز عبور",
  'pa': "ਪਾਸਵਰਡ",
  'uk': "Пароль",
  'ro': "Parolă",
  'tl': "Password"
},




"verifyEmail":{
  'en': "Verify Email",
  'fr': "Vérifier l’e-mail",
  'de': "E-Mail bestätigen",
  'ar': "تأكيد البريد الإلكتروني",
  'es': "Verificar correo electrónico",
  'tr': "E-postayı doğrula",
  'nl': "E-mail verifiëren",
  'it': "Verifica email",
  'ja': "メールを確認",
  'zh': "验证邮箱",
  'ko': "이메일 인증",
  'hi': "ईमेल सत्यापित करें",
  'pt': "Verificar e-mail",
  'ru': "Подтвердить электронную почту",
  'sw': "Thibitisha barua pepe",
  'pl': "Zweryfikuj e-mail",
  'id': "Verifikasi email",
  'fa': "تأیید ایمیل",
  'pa': "ਈਮੇਲ ਦੀ ਪੁਸ਼ਟੀ ਕਰੋ",
  'uk': "Підтвердити електронну пошту",
  'ro': "Verifică e-mailul",
  'tl': "I-verify ang email"
},



"signUP":{
  'en': "Sign Up",
  'fr': "S’inscrire",
  'de': "Registrieren",
  'ar': "إنشاء حساب",
  'es': "Registrarse",
  'tr': "Kayıt Ol",
  'nl': "Registreren",
  'it': "Registrati",
  'ja': "登録",
  'zh': "注册",
  'ko': "회원가입",
  'hi': "साइन अप",
  'pt': "Cadastrar-se",
  'ru': "Зарегистрироваться",
  'sw': "Jisajili",
  'pl': "Zarejestruj się",
  'id': "Daftar",
  'fa': "ثبت‌نام",
  'pa': "ਸਾਈਨ ਅੱਪ",
  'uk': "Зареєструватися",
  'ro': "Înregistrare",
  'tl': "Mag-sign up"
},


"yourNews":{
  "en": "Your news.",
  "fr": "Vos actualités.",
  "de": "Ihre Nachrichten.",
  "ar": "أخبارك.",
  "es": "Tus noticias.",
  "tr": "Haberlerin.",
  "nl": "Jouw nieuws.",
  "it": "Le tue notizie.",
  "ja": "あなたのニュース。",
  "zh": "你的新闻。",
  "ko": "당신의 뉴스.",
  "hi": "आपकी खबरें।",
  "pt": "Suas notícias.",
  "ru": "Ваши новости.",
  "sw": "Habari zako.",
  "pl": "Twoje wiadomości.",
  "id": "Berita Anda.",
  "fa": "اخبار شما.",
  "pa": "ਤੁਹਾਡੀਆਂ ਖ਼ਬਰਾਂ।",
  "uk": "Ваші новини.",
  "ro": "Știrile tale.",
  "tl": "Balita mo."
},



"yourLang":{
  "en": "Your language.",
  "fr": "Votre langue.",
  "de": "Ihre Sprache.",
  "ar": "لغتك.",
  "es": "Tu idioma.",
  "tr": "Diliniz.",
  "nl": "Jouw taal.",
  "it": "La tua lingua.",
  "ja": "あなたの言語。",
  "zh": "你的语言。",
  "ko": "당신의 언어.",
  "hi": "आपकी भाषा।",
  "pt": "Seu idioma.",
  "ru": "Ваш язык.",
  "sw": "Lugha yako.",
  "pl": "Twój język.",
  "id": "Bahasa Anda.",
  "fa": "زبان شما.",
  "pa": "ਤੁਹਾਡੀ ਭਾਸ਼ਾ।",
  "uk": "Ваша мова.",
  "ro": "Limba ta.",
  "tl": "Wika mo."
},


"yourWorld":{
  "en": "Your world.",
  "fr": "Votre monde.",
  "de": "Ihre Welt.",
  "ar": "عالمك.",
  "es": "Tu mundo.",
  "tr": "Dünyan.",
  "nl": "Jouw wereld.",
  "it": "Il tuo mondo.",
  "ja": "あなたの世界。",
  "zh": "你的世界。",
  "ko": "당신의 세계.",
  "hi": "आपकी दुनिया।",
  "pt": "Seu mundo.",
  "ru": "Ваш мир.",
  "sw": "Dunia yako.",
  "pl": "Twój świat.",
  "id": "Dunia Anda.",
  "fa": "دنیای شما.",
  "pa": "ਤੁਹਾਡੀ ਦੁਨੀਆ।",
  "uk": "Ваш світ.",
  "ro": "Lumea ta.",
  "tl": "Mundo mo."
},


"permDenied":{
  'en': "Permission Denied! Allow NEWSWORLD to use your location for adequate performance.",
  'fr': "Autorisation refusée ! Autorisez NEWSWORLD à utiliser votre localisation pour des performances optimales.",
  'de': "Berechtigung verweigert! Erlauben Sie NEWSWORLD, Ihren Standort für eine optimale Leistung zu nutzen.",
  'ar': "تم رفض الإذن! اسمح لـ NEWSWORLD باستخدام موقعك لضمان أداء مناسب.",
  'es': "¡Permiso denegado! Permite que NEWSWORLD use tu ubicación para un rendimiento adecuado.",
  'tr': "İzin reddedildi! NEWSWORLD’ün yeterli performans için konumunuzu kullanmasına izin verin.",
  'nl': "Toestemming geweigerd! Sta NEWSWORLD toe uw locatie te gebruiken voor optimale prestaties.",
  'it': "Autorizzazione negata! Consenti a NEWSWORLD di utilizzare la tua posizione per prestazioni adeguate.",
  'ja': "権限が拒否されました。最適なパフォーマンスのために、NEWSWORLDが位置情報を使用できるようにしてください。",
  'zh': "权限被拒绝！请允许 NEWSWORLD 使用您的位置以确保最佳性能。",
  'ko': "권한이 거부되었습니다! 최적의 성능을 위해 NEWSWORLD가 위치 정보를 사용할 수 있도록 허용하세요.",
  'hi': "अनुमति अस्वीकृत! बेहतर प्रदर्शन के लिए NEWSWORLD को आपकी लोकेशन उपयोग करने दें।",
  'pt': "Permissão negada! Permita que o NEWSWORLD use sua localização para um melhor desempenho.",
  'ru': "Доступ запрещён! Разрешите NEWSWORLD использовать ваше местоположение для корректной работы.",
  'sw': "Ruhusa imekataliwa! Ruhusu NEWSWORLD kutumia eneo lako kwa utendaji bora.",
  'pl': "Odmowa dostępu! Zezwól NEWSWORLD na użycie Twojej lokalizacji w celu lepszej wydajności.",
  'id': "Izin ditolak! Izinkan NEWSWORLD menggunakan lokasi Anda untuk kinerja yang optimal.",
  'fa': "دسترسی رد شد! برای عملکرد بهتر، به NEWSWORLD اجازه استفاده از موقعیت مکانی خود را بدهید.",
  'pa': "ਇਜਾਜ਼ਤ ਰੱਦ ਕੀਤੀ ਗਈ! ਵਧੀਆ ਕਾਰਗੁਜ਼ਾਰੀ ਲਈ NEWSWORLD ਨੂੰ ਤੁਹਾਡਾ ਟਿਕਾਣਾ ਵਰਤਣ ਦੀ ਆਗਿਆ ਦਿਓ।",
  'uk': "Доступ заборонено! Дозвольте NEWSWORLD використовувати ваше місцезнаходження для належної роботи.",
  'ro': "Permisiune refuzată! Permite NEWSWORLD să utilizeze locația ta pentru performanță optimă.",
  'tl': "Tinanggihan ang pahintulot! Payagan ang NEWSWORLD na gamitin ang iyong lokasyon para sa mas maayos na performance."
},



"validEmail":{
  'en': "Please provide a valid email address",
  'fr': "Veuillez fournir une adresse e-mail valide",
  'de': "Bitte geben Sie eine gültige E-Mail-Adresse an",
  'ar': "يرجى إدخال عنوان بريد إلكتروني صالح",
  'es': "Por favor proporciona una dirección de correo válida",
  'tr': "Lütfen geçerli bir e-posta adresi girin",
  'nl': "Geef een geldig e-mailadres op",
  'it': "Inserisci un indirizzo email valido",
  'ja': "有効なメールアドレスを入力してください",
  'zh': "请输入有效的电子邮件地址",
  'ko': "유효한 이메일 주소를 입력하세요",
  'hi': "कृपया एक मान्य ईमेल पता दर्ज करें",
  'pt': "Forneça um endereço de e-mail válido",
  'ru': "Пожалуйста, укажите действительный адрес электронной почты",
  'sw': "Tafadhali toa anwani halali ya barua pepe",
  'pl': "Podaj prawidłowy adres e-mail",
  'id': "Silakan masukkan alamat email yang valid",
  'fa': "لطفاً یک آدرس ایمیل معتبر وارد کنید",
  'pa': "ਕਿਰਪਾ ਕਰਕੇ ਵੈਧ ਈਮੇਲ ਪਤਾ ਦਿਓ",
  'uk': "Будь ласка, введіть дійсну адресу електронної пошти",
  'ro': "Te rugăm să introduci o adresă de e-mail validă",
  'tl': "Mangyaring magbigay ng wastong email address"
},





"locationReq":{
  'en': "Location Service required. Please enable your location and grant access when prompted to allow NEWSWORLD provide relevant information efficiently.",
  'fr': "Le service de localisation est requis. Veuillez activer votre localisation et accorder l’accès lorsque cela est demandé afin que NEWSWORLD fournisse des informations pertinentes efficacement.",
  'de': "Der Standortdienst ist erforderlich. Bitte aktivieren Sie Ihren Standort und gewähren Sie Zugriff, wenn Sie dazu aufgefordert werden, damit NEWSWORLD relevante Informationen effizient bereitstellen kann.",
  'ar': "خدمة الموقع مطلوبة. يرجى تفعيل موقعك ومنح الإذن عند الطلب لتمكين NEWSWORLD من تقديم معلومات ذات صلة بكفاءة.",
  'es': "Se requiere el servicio de ubicación. Activa tu ubicación y concede el acceso cuando se te solicite para que NEWSWORLD pueda proporcionar información relevante de manera eficiente.",
  'tr': "Konum hizmeti gereklidir. NEWSWORLD’ün ilgili bilgileri verimli şekilde sunabilmesi için lütfen konumunuzu etkinleştirin ve istendiğinde erişim izni verin.",
  'nl': "Locatieservice vereist. Schakel uw locatie in en verleen toegang wanneer daarom wordt gevraagd, zodat NEWSWORLD relevante informatie efficiënt kan aanbieden.",
  'it': "È richiesto il servizio di localizzazione. Attiva la posizione e concedi l’accesso quando richiesto per consentire a NEWSWORLD di fornire informazioni pertinenti in modo efficiente.",
  'ja': "位置情報サービスが必要です。NEWSWORLDが関連情報を効率的に提供できるよう、位置情報を有効にし、表示されたらアクセスを許可してください。",
  'zh': "需要定位服务。请启用您的位置并在提示时授予访问权限，以便 NEWSWORLD 高效地提供相关信息。",
  'ko': "위치 서비스가 필요합니다. NEWSWORLD가 관련 정보를 효율적으로 제공할 수 있도록 위치를 활성화하고 요청 시 접근을 허용하세요.",
  'hi': "लोकेशन सेवा आवश्यक है। NEWSWORLD को प्रासंगिक जानकारी प्रभावी ढंग से देने के लिए कृपया अपनी लोकेशन चालू करें और अनुरोध आने पर अनुमति दें।",
  'pt': "O serviço de localização é necessário. Ative sua localização e conceda acesso quando solicitado para que o NEWSWORLD forneça informações relevantes de forma eficiente.",
  'ru': "Требуется служба геолокации. Пожалуйста, включите определение местоположения и предоставьте доступ по запросу, чтобы NEWSWORLD мог эффективно предоставлять релевантную информацию.",
  'sw': "Huduma ya eneo inahitajika. Tafadhali washa eneo lako na ruhusu ufikiaji unapoulizwa ili NEWSWORLD iweze kutoa taarifa husika kwa ufanisi.",
  'pl': "Wymagana jest usługa lokalizacji. Włącz lokalizację i przyznaj dostęp po wyświetleniu monitu, aby NEWSWORLD mógł skutecznie dostarczać istotne informacje.",
  'id': "Layanan lokasi diperlukan. Aktifkan lokasi Anda dan berikan akses saat diminta agar NEWSWORLD dapat menyediakan informasi yang relevan secara efisien.",
  'fa': "سرویس موقعیت مکانی موردنیاز است. لطفاً موقعیت مکانی خود را فعال کرده و هنگام درخواست، دسترسی را تأیید کنید تا NEWSWORLD بتواند اطلاعات مرتبط را به‌صورت مؤثر ارائه دهد.",
  'pa': "ਲੋਕੇਸ਼ਨ ਸੇਵਾ ਲੋੜੀਂਦੀ ਹੈ। NEWSWORLD ਨੂੰ ਸੰਬੰਧਤ ਜਾਣਕਾਰੀ ਪ੍ਰਭਾਵਸ਼ਾਲੀ ਤਰੀਕੇ ਨਾਲ ਦੇਣ ਲਈ ਕਿਰਪਾ ਕਰਕੇ ਆਪਣਾ ਟਿਕਾਣਾ ਚਾਲੂ ਕਰੋ ਅਤੇ ਪੁੱਛੇ ਜਾਣ ’ਤੇ ਪਹੁੰਚ ਦੀ ਆਗਿਆ ਦਿਓ।",
  'uk': "Потрібна служба визначення місцезнаходження. Увімкніть локацію та надайте доступ за запитом, щоб NEWSWORLD міг ефективно надавати релевантну інформацію.",
  'ro': "Este necesar serviciul de localizare. Activează locația și acordă acces atunci când ți se solicită pentru ca NEWSWORLD să ofere informații relevante în mod eficient.",
  'tl': "Kailangan ang serbisyo ng lokasyon. Pakiactivate ang iyong lokasyon at payagan ang access kapag hiniling upang makapagbigay ang NEWSWORLD ng kaugnay na impormasyon nang mahusay."
},



"home":{
  "en": "Home",
  "fr": "Accueil",
  "de": "Startseite",
  "ar": "الرئيسية",
  "es": "Inicio",
  "tr": "Ana Sayfa",
  "nl": "Start",
  "it": "Home",
  "ja": "ホーム",
  "zh": "首页",
  "ko": "홈",
  "hi": "होम",
  "pt": "Início",
  "ru": "Главная",
  "sw": "Mwanzo",
  "pl": "Strona główna",
  "id": "Beranda",
  "fa": "خانه",
  "pa": "ਹੋਮ",
  "uk": "Головна",
  "ro": "Acasă",
  "tl": "Home"
},



"search":{
  "en": "Search",
  "fr": "Rechercher",
  "de": "Suchen",
  "ar": "بحث",
  "es": "Buscar",
  "tr": "Ara",
  "nl": "Zoeken",
  "it": "Cerca",
  "ja": "検索",
  "zh": "搜索",
  "ko": "검색",
  "hi": "खोजें",
  "pt": "Pesquisar",
  "ru": "Поиск",
  "sw": "Tafuta",
  "pl": "Szukaj",
  "id": "Cari",
  "fa": "جستجو",
  "pa": "ਖੋਜ",
  "uk": "Пошук",
  "ro": "Căutare",
  "tl": "Maghanap"
},




"watch":{
  "en": "Watch",
  "fr": "Regarder",
  "de": "Ansehen",
  "ar": "مشاهدة",
  "es": "Ver",
  "tr": "İzle",
  "nl": "Bekijken",
  "it": "Guarda",
  "ja": "視聴",
  "zh": "观看",
  "ko": "시청",
  "hi": "देखें",
  "pt": "Assistir",
  "ru": "Смотреть",
  "sw": "Tazama",
  "pl": "Oglądaj",
  "id": "Tonton",
  "fa": "تماشا",
  "pa": "ਵੇਖੋ",
  "uk": "Дивитися",
  "ro": "Urmărește",
  "tl": "Manood"
},




"profile":{
  "en": "Profile",
  "fr": "Profil",
  "de": "Profil",
  "ar": "الملف الشخصي",
  "es": "Perfil",
  "tr": "Profil",
  "nl": "Profiel",
  "it": "Profilo",
  "ja": "プロフィール",
  "zh": "个人资料",
  "ko": "프로필",
  "hi": "प्रोफ़ाइल",
  "pt": "Perfil",
  "ru": "Профиль",
  "sw": "Wasifu",
  "pl": "Profil",
  "id": "Profil",
  "fa": "پروفایل",
  "pa": "ਪ੍ਰੋਫ਼ਾਈਲ",
  "uk": "Профіль",
  "ro": "Profil",
  "tl": "Profile"
},



"livenews":{
  "en": "Live news",
  "fr": "Actualités en direct",
  "de": "Live-Nachrichten",
  "ar": "أخبار مباشرة",
  "es": "Noticias en vivo",
  "tr": "Canlı haberler",
  "nl": "Live nieuws",
  "it": "Notizie in diretta",
  "ja": "ライブニュース",
  "zh": "实时新闻",
  "ko": "실시간 뉴스",
  "hi": "लाइव समाचार",
  "pt": "Notícias ao vivo",
  "ru": "Новости в прямом эфире",
  "sw": "Habari mubashara",
  "pl": "Wiadomości na żywo",
  "id": "Berita langsung",
  "fa": "اخبار زنده",
  "pa": "ਲਾਈਵ ਖ਼ਬਰਾਂ",
  "uk": "Новини наживо",
  "ro": "Știri live",
  "tl": "Live na balita"
},




"sportsEve":{
  "en": "Sports event",
  "fr": "Événement sportif",
  "de": "Sportveranstaltung",
  "ar": "حدث رياضي",
  "es": "Evento deportivo",
  "tr": "Spor etkinliği",
  "nl": "Sportevenement",
  "it": "Evento sportivo",
  "ja": "スポーツイベント",
  "zh": "体育赛事",
  "ko": "스포츠 이벤트",
  "hi": "खेल आयोजन",
  "pt": "Evento esportivo",
  "ru": "Спортивное событие",
  "sw": "Tukio la michezo",
  "pl": "Wydarzenie sportowe",
  "id": "Acara olahraga",
  "fa": "رویداد ورزشی",
  "pa": "ਖੇਡ ਸਮਾਗਮ",
  "uk": "Спортивна подія",
  "ro": "Eveniment sportiv",
  "tl": "Kaganapang pampalakasan"
},


"Firstname": {
    "en": "First name",
    "fr": "Prénom",
    "de": "Vorname",
    "ar": "الاسم الأول",
    "es": "Nombre",
    "tr": "Ad",
    "nl": "Voornaam",
    "it": "Nome",
    "ja": "名",
    "zh": "名字",
    "ko": "이름",
    "hi": "पहला नाम",
    "pt": "Nome",
    "ru": "Имя",
    "sw": "Jina la kwanza",
    "pl": "Imię",
    "id": "Nama depan",
    "fa": "نام",
    "pa": "ਪਹਿਲਾ ਨਾਮ",
    "uk": "Ім’я",
    "ro": "Prenume",
    "tl": "Unang pangalan"
  },

  "Lastname": {
    "en": "Last name",
    "fr": "Nom",
    "de": "Nachname",
    "ar": "اسم العائلة",
    "es": "Apellido",
    "tr": "Soyad",
    "nl": "Achternaam",
    "it": "Cognome",
    "ja": "姓",
    "zh": "姓氏",
    "ko": "성",
    "hi": "अंतिम नाम",
    "pt": "Sobrenome",
    "ru": "Фамилия",
    "sw": "Jina la ukoo",
    "pl": "Nazwisko",
    "id": "Nama belakang",
    "fa": "نام خانوادگی",
    "pa": "ਆਖ਼ਰੀ ਨਾਮ",
    "uk": "Прізвище",
    "ro": "Nume",
    "tl": "Apelyido"
  },



  "Male": {
    "en": "Male",
    "fr": "Homme",
    "de": "Männlich",
    "ar": "ذكر",
    "es": "Masculino",
    "tr": "Erkek",
    "nl": "Man",
    "it": "Maschio",
    "ja": "男性",
    "zh": "男",
    "ko": "남성",
    "hi": "पुरुष",
    "pt": "Masculino",
    "ru": "Мужской",
    "sw": "Mwanaume",
    "pl": "Mężczyzna",
    "id": "Laki-laki",
    "fa": "مرد",
    "pa": "ਮਰਦ",
    "uk": "Чоловіча",
    "ro": "Masculin",
    "tl": "Lalaki"
  },

  "Female": {
    "en": "Female",
    "fr": "Femme",
    "de": "Weiblich",
    "ar": "أنثى",
    "es": "Femenino",
    "tr": "Kadın",
    "nl": "Vrouw",
    "it": "Femmina",
    "ja": "女性",
    "zh": "女",
    "ko": "여성",
    "hi": "महिला",
    "pt": "Feminino",
    "ru": "Женский",
    "sw": "Mwanamke",
    "pl": "Kobieta",
    "id": "Perempuan",
    "fa": "زن",
    "pa": "ਔਰਤ",
    "uk": "Жіноча",
    "ro": "Feminin",
    "tl": "Babae"
  },



  "enterPass":{
  "en": "Enter Password",
  "fr": "Entrez le mot de passe",
  "de": "Passwort eingeben",
  "ar": "أدخل كلمة المرور",
  "es": "Introduce la contraseña",
  "tr": "Şifreyi girin",
  "nl": "Wachtwoord invoeren",
  "it": "Inserisci password",
  "ja": "パスワードを入力",
  "zh": "输入密码",
  "ko": "비밀번호 입력",
  "hi": "पासवर्ड दर्ज करें",
  "pt": "Digite a senha",
  "ru": "Введите пароль",
  "sw": "Weka nenosiri",
  "pl": "Wpisz hasło",
  "id": "Masukkan kata sandi",
  "fa": "رمز عبور را وارد کنید",
  "pa": "ਪਾਸਵਰਡ ਦਰਜ ਕਰੋ",
  "uk": "Введіть пароль",
  "ro": "Introduce parola",
  "tl": "Ilagay ang password"
},




"welomback":{
  "en": "Welcome Back",
  "fr": "Bon retour",
  "de": "Willkommen zurück",
  "ar": "مرحبًا بعودتك",
  "es": "Bienvenido de nuevo",
  "tr": "Tekrar hoş geldiniz",
  "nl": "Welkom terug",
  "it": "Bentornato",
  "ja": "お帰りなさい",
  "zh": "欢迎回来",
  "ko": "다시 오신 것을 환영합니다",
  "hi": "वापसी पर स्वागत है",
  "pt": "Bem-vindo de volta",
  "ru": "С возвращением",
  "sw": "Karibu tena",
  "pl": "Witaj ponownie",
  "id": "Selamat datang kembali",
  "fa": "خوش آمدید",
  "pa": "ਵਾਪਸ ਸੁਆਗਤ ਹੈ",
  "uk": "З поверненням",
  "ro": "Bine ai revenit",
  "tl": "Maligayang pagbabalik"
},



"kindlyEnter":{
  "en": "Kindly enter your password for access",
  "fr": "Veuillez entrer votre mot de passe pour accéder",
  "de": "Bitte geben Sie Ihr Passwort für den Zugriff ein",
  "ar": "يرجى إدخال كلمة المرور للوصول",
  "es": "Por favor ingresa tu contraseña para acceder",
  "tr": "Erişim için lütfen şifrenizi girin",
  "nl": "Voer alstublieft uw wachtwoord in voor toegang",
  "it": "Inserisci gentilmente la tua password per accedere",
  "ja": "アクセスするにはパスワードを入力してください",
  "zh": "请输入您的密码以访问",
  "ko": "접근하려면 비밀번호를 입력하세요",
  "hi": "कृपया प्रवेश के लिए अपना पासवर्ड दर्ज करें",
  "pt": "Por favor, insira sua senha para acessar",
  "ru": "Пожалуйста, введите пароль для доступа",
  "sw": "Tafadhali ingiza nenosiri lako ili kupata ufikiaji",
  "pl": "Wprowadź hasło, aby uzyskać dostęp",
  "id": "Silakan masukkan kata sandi Anda untuk mengakses",
  "fa": "لطفاً برای دسترسی، رمز عبور خود را وارد کنید",
  "pa": "ਕਿਰਪਾ ਕਰਕੇ ਪਹੁੰਚ ਲਈ ਆਪਣਾ ਪਾਸਵਰਡ ਦਰਜ ਕਰੋ",
  "uk": "Будь ласка, введіть пароль для доступу",
  "ro": "Te rugăm să introduci parola pentru acces",
  "tl": "Mangyaring ilagay ang iyong password para sa access"
},


"Earlier": {
  "en": "Earlier •",
  "fr": "Plus tôt •",
  "de": "Früher •",
  "ar": "في وقت سابق •",
  "es": "Antes •",
  "tr": "Daha önce •",
  "nl": "Eerder •",
  "it": "Prima •",
  "ja": "先ほど •",
  "zh": "早些时候 •",
  "ko": "이전 •",
  "hi": "पहले •",
  "pt": "Mais cedo •",
  "ru": "Ранее •",
  "sw": "Mapema •",
  "pl": "Wcześniej •",
  "id": "Sebelumnya •",
  "fa": "زودتر •",
  "pa": "ਪਹਿਲਾਂ •",
  "uk": "Раніше •",
  "ro": "Mai devreme •",
  "tl": "Mas maaga •"
},


"ago":{
'en':"{time} ago" ,
'fr':"il y a {time}" ,
'de':"vor {time}",
'ar': "منذ {time}",
'es':"hace {time}",
'tr': "{time} önce",
'nl':"{time} geleden",
'it': "{time} fa",
'ja':"{time}前",
'zh': "{time}前",
'ko': "{time} 전",
'hi': "{time} पहले",
'pt':"há {time}",
'ru':"{time} назад",
'sw':"{time} iliyopita",
'pl':"{time} temu",
'id':"{time} yang lalu",
'fa':"{time} پیش",
'pa':"{time} ਪਹਿਲਾਂ",
'uk':"{time} тому",
'ro':"{time} în urmă",
'tl':"{time} nakalipas"
},


"Reply": {
  "en": "Reply",
  "fr": "Répondre",
  "de": "Antworten",
  "ar": "رد",
  "es": "Responder",
  "tr": "Yanıtla",
  "nl": "Beantwoorden",
  "it": "Rispondi",
  "ja": "返信",
  "zh": "回复",
  "ko": "답글",
  "hi": "उत्तर दें",
  "pt": "Responder",
  "ru": "Ответить",
  "sw": "Jibu",
  "pl": "Odpowiedz",
  "id": "Balas",
  "fa": "پاسخ",
  "pa": "ਜਵਾਬ",
  "uk": "Відповісти",
  "ro": "Răspunde",
  "tl": "Tumugon"
},


"Translate": {
  "en": "Translate",
  "fr": "Traduire",
  "de": "Übersetzen",
  "ar": "ترجمة",
  "es": "Traducir",
  "tr": "Çevir",
  "nl": "Vertalen",
  "it": "Traduci",
  "ja": "翻訳",
  "zh": "翻译",
  "ko": "번역",
  "hi": "अनुवाद करें",
  "pt": "Traduzir",
  "ru": "Перевести",
  "sw": "Tafsiri",
  "pl": "Przetłumacz",
  "id": "Terjemahkan",
  "fa": "ترجمه",
  "pa": "ਅਨੁਵਾਦ ਕਰੋ",
  "uk": "Перекласти",
  "ro": "Tradu",
  "tl": "Isalin"
},


"Original": {
  "en": "Original",
  "fr": "Original",
  "de": "Original",
  "ar": "الأصلي",
  "es": "Original",
  "tr": "Orijinal",
  "nl": "Origineel",
  "it": "Originale",
  "ja": "元の内容",
  "zh": "原文",
  "ko": "원문",
  "hi": "मूल",
  "pt": "Original",
  "ru": "Оригинал",
  "sw": "Asili",
  "pl": "Oryginał",
  "id": "Asli",
  "fa": "اصلی",
  "pa": "ਮੂਲ",
  "uk": "Оригінал",
  "ro": "Original",
  "tl": "Orihinal"
},

"selectIssue":{
  "en": "Select type of issue",
  "fr": "Sélectionnez le type de problème",
  "de": "Art des Problems auswählen",
  "ar": "اختر نوع المشكلة",
  "es": "Seleccione el tipo de problema",
  "tr": "Sorun türünü seçin",
  "nl": "Selecteer het type probleem",
  "it": "Seleziona il tipo di problema",
  "ja": "問題の種類を選択してください",
  "zh": "选择问题类型",
  "ko": "문제 유형을 선택하세요",
  "hi": "समस्या का प्रकार चुनें",
  "pt": "Selecione o tipo de problema",
  "ru": "Выберите тип проблемы",
  "sw": "Chagua aina ya tatizo",
  "pl": "Wybierz typ problemu",
  "id": "Pilih jenis masalah",
  "fa": "نوع مشکل را انتخاب کنید",
  "pa": "ਸਮੱਸਿਆ ਦੀ ਕਿਸਮ ਚੁਣੋ",
  "uk": "Виберіть тип проблеми",
  "ro": "Selectați tipul problemei",
  "tl": "Piliin ang uri ng isyu"
},

"assistance":{
  "en": "Please briefly describe your problem so our support team can understand it and provide assistance",
  "fr": "Veuillez décrire brièvement votre problème afin que notre équipe d'assistance puisse le comprendre et vous aider",
  "de": "Bitte beschreiben Sie Ihr Problem kurz, damit unser Support-Team es verstehen und Ihnen helfen kann",
  "ar": "يرجى وصف مشكلتك بإيجاز حتى يتمكن فريق الدعم من فهمها وتقديم المساعدة",
  "es": "Describa brevemente su problema para que nuestro equipo de soporte pueda entenderlo y brindarle asistencia",
  "tr": "Destek ekibimizin anlayıp yardımcı olabilmesi için lütfen sorununuzu kısaca açıklayın",
  "nl": "Beschrijf uw probleem kort zodat ons ondersteuningsteam het kan begrijpen en hulp kan bieden",
  "it": "Descrivi brevemente il tuo problema affinché il nostro team di supporto possa comprenderlo e fornirti assistenza",
  "ja": "サポートチームが理解し支援できるよう、問題を簡単に説明してください",
  "zh": "请简要描述您的问题，以便我们的支持团队理解并提供帮助",
  "ko": "지원팀이 이해하고 도움을 드릴 수 있도록 문제를 간단히 설명해 주세요",
  "hi": "कृपया अपनी समस्या का संक्षेप में वर्णन करें ताकि हमारी सहायता टीम उसे समझकर मदद कर सके",
  "pt": "Descreva brevemente seu problema para que nossa equipe de suporte possa entendê-lo e fornecer assistência",
  "ru": "Пожалуйста, кратко опишите вашу проблему, чтобы наша служба поддержки могла понять её и оказать помощь",
  "sw": "Tafadhali eleza tatizo lako kwa ufupi ili timu yetu ya msaada iweze kulielewa na kutoa usaidizi",
  "pl": "Proszę krótko opisać swój problem, aby nasz zespół wsparcia mógł go zrozumieć i udzielić pomocy",
  "id": "Silakan jelaskan masalah Anda secara singkat agar tim dukungan kami dapat memahaminya dan memberikan bantuan",
  "fa": "لطفاً مشکل خود را به‌طور مختصر توضیح دهید تا تیم پشتیبانی ما آن را درک کرده و کمک ارائه دهد",
  "pa": "ਕਿਰਪਾ ਕਰਕੇ ਆਪਣੀ ਸਮੱਸਿਆ ਨੂੰ ਸੰਖੇਪ ਵਿੱਚ ਵੇਰਵਾ ਕਰੋ ਤਾਂ ਜੋ ਸਾਡੀ ਸਹਾਇਤਾ ਟੀਮ ਇਸਨੂੰ ਸਮਝ ਕੇ ਮਦਦ ਕਰ ਸਕੇ",
  "uk": "Будь ласка, коротко опишіть свою проблему, щоб наша служба підтримки могла її зрозуміти та надати допомогу",
  "ro": "Vă rugăm să descrieți pe scurt problema pentru ca echipa noastră de suport să o poată înțelege și oferi asistență",
  "tl": "Mangyaring ilarawan nang maikli ang iyong problema upang maunawaan ito ng aming support team at makapagbigay ng tulong"
},


"supporting":{
  "en": "Upload Supporting Documents",
  "fr": "Télécharger des documents justificatifs",
  "de": "Unterstützende Dokumente hochladen",
  "ar": "تحميل مستندات داعمة",
  "es": "Subir documentos de respaldo",
  "tr": "Destekleyici belgeleri yükleyin",
  "nl": "Ondersteunende documenten uploaden",
  "it": "Carica documenti di supporto",
  "ja": "補足資料をアップロード",
  "zh": "上传支持文件",
  "ko": "지원 문서 업로드",
  "hi": "सहायक दस्तावेज़ अपलोड करें",
  "pt": "Enviar documentos de suporte",
  "ru": "Загрузить подтверждающие документы",
  "sw": "Pakia nyaraka za kusaidia",
  "pl": "Prześlij dokumenty pomocnicze",
  "id": "Unggah dokumen pendukung",
  "fa": "بارگذاری مدارک پشتیبان",
  "pa": "ਸਹਾਇਕ ਦਸਤਾਵੇਜ਼ ਅੱਪਲੋਡ ਕਰੋ",
  "uk": "Завантажити підтверджуючі документи",
  "ro": "Încărcați documente justificative",
  "tl": "Mag-upload ng mga sumusuportang dokumento"
},


"optional":{
  "en": "Optional",
  "fr": "Optionnel",
  "de": "Optional",
  "ar": "اختياري",
  "es": "Opcional",
  "tr": "İsteğe bağlı",
  "nl": "Optioneel",
  "it": "Opzionale",
  "ja": "任意",
  "zh": "可选",
  "ko": "선택 사항",
  "hi": "वैकल्पिक",
  "pt": "Opcional",
  "ru": "Необязательно",
  "sw": "Hiari",
  "pl": "Opcjonalne",
  "id": "Opsional",
  "fa": "اختیاری",
  "pa": "ਚੋਣਵਾਂ",
  "uk": "Необов'язково",
  "ro": "Opțional",
  "tl": "Opsyonal"
},


"submit":{
  "en": "Submit",
  "fr": "Soumettre",
  "de": "Einreichen",
  "ar": "إرسال",
  "es": "Enviar",
  "tr": "Gönder",
  "nl": "Indienen",
  "it": "Invia",
  "ja": "送信",
  "zh": "提交",
  "ko": "제출",
  "hi": "जमा करें",
  "pt": "Enviar",
  "ru": "Отправить",
  "sw": "Wasilisha",
  "pl": "Prześlij",
  "id": "Kirim",
  "fa": "ارسال",
  "pa": "ਜਮ੍ਹਾ ਕਰੋ",
  "uk": "Надіслати",
  "ro": "Trimite",
  "tl": "Isumite"
},


"Done":{
  "en": "Done",
  "fr": "Terminé",
  "de": "Fertig",
  "ar": "تم",
  "es": "Hecho",
  "tr": "Tamamlandı",
  "nl": "Klaar",
  "it": "Fatto",
  "ja": "完了",
  "zh": "完成",
  "ko": "완료",
  "hi": "पूर्ण",
  "pt": "Concluído",
  "ru": "Готово",
  "sw": "Imekamilika",
  "pl": "Gotowe",
  "id": "Selesai",
  "fa": "انجام شد",
  "pa": "ਮੁਕੰਮਲ",
  "uk": "Готово",
  "ro": "Finalizat",
  "tl": "Tapos"
},

"Reactions": {
    "en": "Reactions",
    "fr": "Réactions",
    "de": "Reaktionen",
    "ar": "التفاعلات",
    "es": "Reacciones",
    "tr": "Tepkiler",
    "nl": "Reacties",
    "it": "Reazioni",
    "ja": "リアクション",
    "zh": "反应",
    "ko": "반응",
    "hi": "प्रतिक्रियाएँ",
    "pt": "Reações",
    "ru": "Реакции",
    "sw": "Mwitikio",
    "pl": "Reakcje",
    "id": "Reaksi",
    "fa": "واکنش‌ها",
    "pa": "ਪ੍ਰਤੀਕਿਰਿਆਵਾਂ",
    "uk": "Реакції",
    "ro": "Reacții",
    "tl": "Mga Reaksyon"
  },

  "Comments": {
    "en": "Comments",
    "fr": "Commentaires",
    "de": "Kommentare",
    "ar": "التعليقات",
    "es": "Comentarios",
    "tr": "Yorumlar",
    "nl": "Reacties",
    "it": "Commenti",
    "ja": "コメント",
    "zh": "评论",
    "ko": "댓글",
    "hi": "टिप्पणियाँ",
    "pt": "Comentários",
    "ru": "Комментарии",
    "sw": "Maoni",
    "pl": "Komentarze",
    "id": "Komentar",
    "fa": "نظرات",
    "pa": "ਟਿੱਪਣੀਆਂ",
    "uk": "Коментарі",
    "ro": "Comentarii",
    "tl": "Mga Komento"
  },

  "Saved": {
    "en": "Saved",
    "fr": "Enregistré",
    "de": "Gespeichert",
    "ar": "المحفوظات",
    "es": "Guardado",
    "tr": "Kaydedildi",
    "nl": "Opgeslagen",
    "it": "Salvato",
    "ja": "保存済み",
    "zh": "已保存",
    "ko": "저장됨",
    "hi": "सहेजा गया",
    "pt": "Salvo",
    "ru": "Сохранено",
    "sw": "Imehifadhiwa",
    "pl": "Zapisane",
    "id": "Tersimpan",
    "fa": "ذخیره‌شده",
    "pa": "ਸੰਭਾਲਿਆ ਗਿਆ",
    "uk": "Збережено",
    "ro": "Salvat",
    "tl": "Naka-save"
  },

  "noReactionY": {
    "en": "No reactions yet",
    "fr": "Aucune réaction pour le moment",
    "de": "Noch keine Reaktionen",
    "ar": "لا توجد تفاعلات بعد",
    "es": "Aún no hay reacciones",
    "tr": "Henüz tepki yok",
    "nl": "Nog geen reacties",
    "it": "Nessuna reazione ancora",
    "ja": "まだリアクションはありません",
    "zh": "暂无反应",
    "ko": "아직 반응이 없습니다",
    "hi": "अभी तक कोई प्रतिक्रिया नहीं",
    "pt": "Ainda não há reações",
    "ru": "Пока нет реакций",
    "sw": "Bado hakuna mwitikio",
    "pl": "Brak reakcji",
    "id": "Belum ada reaksi",
    "fa": "هنوز واکنشی وجود ندارد",
    "pa": "ਹਾਲੇ ਕੋਈ ਪ੍ਰਤੀਕਿਰਿਆ ਨਹੀਂ",
    "uk": "Поки що немає реакцій",
    "ro": "Încă nu există reacții",
    "tl": "Wala pang reaksyon"
  },

  "noCommentY": {
    "en": "No comments yet",
    "fr": "Aucun commentaire pour le moment",
    "de": "Noch keine Kommentare",
    "ar": "لا توجد تعليقات بعد",
    "es": "Aún no hay comentarios",
    "tr": "Henüz yorum yok",
    "nl": "Nog geen reacties",
    "it": "Nessun commento ancora",
    "ja": "まだコメントはありません",
    "zh": "暂无评论",
    "ko": "아직 댓글이 없습니다",
    "hi": "अभी तक कोई टिप्पणी नहीं",
    "pt": "Ainda não há comentários",
    "ru": "Пока нет комментариев",
    "sw": "Bado hakuna maoni",
    "pl": "Brak komentarzy",
    "id": "Belum ada komentar",
    "fa": "هنوز نظری وجود ندارد",
    "pa": "ਹਾਲੇ ਕੋਈ ਟਿੱਪਣੀ ਨਹੀਂ",
    "uk": "Поки що немає коментарів",
    "ro": "Încă nu există comentarii",
    "tl": "Wala pang komento"
  },

  "noSavedY": {
    "en": "No saved items yet",
    "fr": "Aucun élément enregistré pour le moment",
    "de": "Noch keine gespeicherten Elemente",
    "ar": "لا توجد عناصر محفوظة بعد",
    "es": "Aún no hay elementos guardados",
    "tr": "Henüz kaydedilen öğe yok",
    "nl": "Nog geen opgeslagen items",
    "it": "Nessun elemento salvato",
    "ja": "まだ保存された項目はありません",
    "zh": "暂无已保存内容",
    "ko": "아직 저장된 항목이 없습니다",
    "hi": "अभी तक कोई सहेजा गया आइटम नहीं",
    "pt": "Ainda não há itens salvos",
    "ru": "Пока нет сохранённых элементов",
    "sw": "Bado hakuna vipengee vilivyohifadhiwa",
    "pl": "Brak zapisanych elementów",
    "id": "Belum ada item tersimpan",
    "fa": "هنوز موردی ذخیره نشده است",
    "pa": "ਹਾਲੇ ਕੋਈ ਸੰਭਾਲਿਆ ਆਈਟਮ ਨਹੀਂ",
    "uk": "Поки що немає збережених елементів",
    "ro": "Încă nu există elemente salvate",
    "tl": "Wala pang naka-save na item"
  },

  "Settings": {
    "en": "Settings",
    "fr": "Paramètres",
    "de": "Einstellungen",
    "ar": "الإعدادات",
    "es": "Configuración",
    "tr": "Ayarlar",
    "nl": "Instellingen",
    "it": "Impostazioni",
    "ja": "設定",
    "zh": "设置",
    "ko": "설정",
    "hi": "सेटिंग्स",
    "pt": "Configurações",
    "ru": "Настройки",
    "sw": "Mipangilio",
    "pl": "Ustawienia",
    "id": "Pengaturan",
    "fa": "تنظیمات",
    "pa": "ਸੈਟਿੰਗਾਂ",
    "uk": "Налаштування",
    "ro": "Setări",
    "tl": "Mga Setting"
  },

  "AccSettings": {
    "en": "Account Settings",
    "fr": "Paramètres du compte",
    "de": "Kontoeinstellungen",
    "ar": "إعدادات الحساب",
    "es": "Configuración de la cuenta",
    "tr": "Hesap Ayarları",
    "nl": "Accountinstellingen",
    "it": "Impostazioni account",
    "ja": "アカウント設定",
    "zh": "账户设置",
    "ko": "계정 설정",
    "hi": "खाता सेटिंग्स",
    "pt": "Configurações da conta",
    "ru": "Настройки аккаунта",
    "sw": "Mipangilio ya Akaunti",
    "pl": "Ustawienia konta",
    "id": "Pengaturan Akun",
    "fa": "تنظیمات حساب",
    "pa": "ਖਾਤਾ ਸੈਟਿੰਗਾਂ",
    "uk": "Налаштування облікового запису",
    "ro": "Setări cont",
    "tl": "Mga Setting ng Account"
  },

  "Theme": {
    "en": "Theme",
    "fr": "Thème",
    "de": "Design",
    "ar": "المظهر",
    "es": "Tema",
    "tr": "Tema",
    "nl": "Thema",
    "it": "Tema",
    "ja": "テーマ",
    "zh": "主题",
    "ko": "테마",
    "hi": "थीम",
    "pt": "Tema",
    "ru": "Тема",
    "sw": "Mandhari",
    "pl": "Motyw",
    "id": "Tema",
    "fa": "تم",
    "pa": "ਥੀਮ",
    "uk": "Тема",
    "ro": "Temă",
    "tl": "Tema"
  },

  "LangLocal": {
    "en": "Language & Localization",
    "fr": "Langue et localisation",
    "de": "Sprache & Lokalisierung",
    "ar": "اللغة والتوطين",
    "es": "Idioma y localización",
    "tr": "Dil ve Yerelleştirme",
    "nl": "Taal en lokalisatie",
    "it": "Lingua e localizzazione",
    "ja": "言語とローカライズ",
    "zh": "语言与本地化",
    "ko": "언어 및 현지화",
    "hi": "भाषा और स्थानीयकरण",
    "pt": "Idioma e localização",
    "ru": "Язык и локализация",
    "sw": "Lugha na Ujanibishaji",
    "pl": "Język i lokalizacja",
    "id": "Bahasa & Lokalisasi",
    "fa": "زبان و بومی‌سازی",
    "pa": "ਭਾਸ਼ਾ ਅਤੇ ਲੋਕਲਾਈਜ਼ੇਸ਼ਨ",
    "uk": "Мова та локалізація",
    "ro": "Limbă și localizare",
    "tl": "Wika at Lokalisasyon"
  },

  "NotifyPref": {
    "en": "Notification Preferences",
    "fr": "Préférences de notification",
    "de": "Benachrichtigungseinstellungen",
    "ar": "تفضيلات الإشعارات",
    "es": "Preferencias de notificación",
    "tr": "Bildirim Tercihleri",
    "nl": "Meldingsvoorkeuren",
    "it": "Preferenze di notifica",
    "ja": "通知設定",
    "zh": "通知偏好",
    "ko": "알림 설정",
    "hi": "सूचना प्राथमिकताएँ",
    "pt": "Preferências de notificação",
    "ru": "Настройки уведомлений",
    "sw": "Mapendeleo ya Arifa",
    "pl": "Preferencje powiadomień",
    "id": "Preferensi Notifikasi",
    "fa": "تنظیمات اعلان‌ها",
    "pa": "ਨੋਟੀਫਿਕੇਸ਼ਨ ਪਸੰਦਾਂ",
    "uk": "Налаштування сповіщень",
    "ro": "Preferințe notificări",
    "tl": "Mga Kagustuhan sa Notipikasyon"
  },

  "AISettings": {
    "en": "AI Settings",
    "fr": "Paramètres IA",
    "de": "KI-Einstellungen",
    "ar": "إعدادات الذكاء الاصطناعي",
    "es": "Configuración de IA",
    "tr": "Yapay Zeka Ayarları",
    "nl": "AI-instellingen",
    "it": "Impostazioni IA",
    "ja": "AI設定",
    "zh": "AI 设置",
    "ko": "AI 설정",
    "hi": "एआई सेटिंग्स",
    "pt": "Configurações de IA",
    "ru": "Настройки ИИ",
    "sw": "Mipangilio ya AI",
    "pl": "Ustawienia AI",
    "id": "Pengaturan AI",
    "fa": "تنظیمات هوش مصنوعی",
    "pa": "ਏਆਈ ਸੈਟਿੰਗਾਂ",
    "uk": "Налаштування ШІ",
    "ro": "Setări AI",
    "tl": "Mga Setting ng AI"
  },

  "PrivacyData": {
    "en": "Privacy & Data",
    "fr": "Confidentialité et données",
    "de": "Datenschutz & Daten",
    "ar": "الخصوصية والبيانات",
    "es": "Privacidad y datos",
    "tr": "Gizlilik ve Veri",
    "nl": "Privacy en gegevens",
    "it": "Privacy e dati",
    "ja": "プライバシーとデータ",
    "zh": "隐私与数据",
    "ko": "개인정보 및 데이터",
    "hi": "गोपनीयता और डेटा",
    "pt": "Privacidade e dados",
    "ru": "Конфиденциальность и данные",
    "sw": "Faragha na Data",
    "pl": "Prywatność i dane",
    "id": "Privasi & Data",
    "fa": "حریم خصوصی و داده‌ها",
    "pa": "ਗੋਪਨੀਯਤਾ ਅਤੇ ਡਾਟਾ",
    "uk": "Конфіденційність та дані",
    "ro": "Confidențialitate și date",
    "tl": "Privacy at Data"
  },

  "PlanBill": {
    "en": "Plan & Billing",
    "fr": "Forfait et facturation",
    "de": "Tarif & Abrechnung",
    "ar": "الخطة والفوترة",
    "es": "Plan y facturación",
    "tr": "Plan ve Faturalandırma",
    "nl": "Abonnement en facturering",
    "it": "Piano e fatturazione",
    "ja": "プランと請求",
    "zh": "套餐与账单",
    "ko": "요금제 및 결제",
    "hi": "प्लान और बिलिंग",
    "pt": "Plano e faturamento",
    "ru": "Тариф и оплата",
    "sw": "Mpango na Malipo",
    "pl": "Plan i rozliczenia",
    "id": "Paket & Penagihan",
    "fa": "طرح و صورتحساب",
    "pa": "ਯੋਜਨਾ ਅਤੇ ਬਿਲਿੰਗ",
    "uk": "Тариф і оплата",
    "ro": "Plan și facturare",
    "tl": "Plano at Pagsingil"
  },

  "Support": {
    "en": "Support",
    "fr": "Assistance",
    "de": "Support",
    "ar": "الدعم",
    "es": "Soporte",
    "tr": "Destek",
    "nl": "Ondersteuning",
    "it": "Supporto",
    "ja": "サポート",
    "zh": "支持",
    "ko": "지원",
    "hi": "सहायता",
    "pt": "Suporte",
    "ru": "Поддержка",
    "sw": "Msaada",
    "pl": "Wsparcie",
    "id": "Dukungan",
    "fa": "پشتیبانی",
    "pa": "ਸਹਾਇਤਾ",
    "uk": "Підтримка",
    "ro": "Asistență",
    "tl": "Suporta"
  },

  "LogOut": {
    "en": "Log out",
    "fr": "Se déconnecter",
    "de": "Abmelden",
    "ar": "تسجيل الخروج",
    "es": "Cerrar sesión",
    "tr": "Çıkış yap",
    "nl": "Uitloggen",
    "it": "Disconnetti",
    "ja": "ログアウト",
    "zh": "退出登录",
    "ko": "로그아웃",
    "hi": "लॉग आउट",
    "pt": "Sair",
    "ru": "Выйти",
    "sw": "Ondoka",
    "pl": "Wyloguj się",
    "id": "Keluar",
    "fa": "خروج",
    "pa": "ਲੌਗ ਆਉਟ",
    "uk": "Вийти",
    "ro": "Deconectare",
    "tl": "Mag-log out"
  },

  "EditProfile": {
    "en": "Edit Profile Details",
    "fr": "Modifier les informations du profil",
    "de": "Profildetails bearbeiten",
    "ar": "تعديل تفاصيل الملف الشخصي",
    "es": "Editar detalles del perfil",
    "tr": "Profil bilgilerini düzenle",
    "nl": "Profielgegevens bewerken",
    "it": "Modifica dettagli profilo",
    "ja": "プロフィール詳細を編集",
    "zh": "编辑个人资料",
    "ko": "프로필 정보 수정",
    "hi": "प्रोफ़ाइल विवरण संपादित करें",
    "pt": "Editar detalhes do perfil",
    "ru": "Редактировать профиль",
    "sw": "Hariri maelezo ya wasifu",
    "pl": "Edytuj dane profilu",
    "id": "Edit detail profil",
    "fa": "ویرایش اطلاعات پروفایل",
    "pa": "ਪ੍ਰੋਫਾਈਲ ਵੇਰਵੇ ਸੋਧੋ",
    "uk": "Редагувати профіль",
    "ro": "Editează detalii profil",
    "tl": "I-edit ang detalye ng profile"
  },

  "Light": {
    "en": "Light",
    "fr": "Clair",
    "de": "Hell",
    "ar": "فاتح",
    "es": "Claro",
    "tr": "Açık",
    "nl": "Licht",
    "it": "Chiaro",
    "ja": "ライト",
    "zh": "浅色",
    "ko": "라이트",
    "hi": "लाइट",
    "pt": "Claro",
    "ru": "Светлая",
    "sw": "Mwanga",
    "pl": "Jasny",
    "id": "Terang",
    "fa": "روشن",
    "pa": "ਹਲਕਾ",
    "uk": "Світла",
    "ro": "Luminos",
    "tl": "Light"
  },

  "System": {
    "en": "System",
    "fr": "Système",
    "de": "System",
    "ar": "النظام",
    "es": "Sistema",
    "tr": "Sistem",
    "nl": "Systeem",
    "it": "Sistema",
    "ja": "システム",
    "zh": "系统",
    "ko": "시스템",
    "hi": "सिस्टम",
    "pt": "Sistema",
    "ru": "Системная",
    "sw": "Mfumo",
    "pl": "Systemowy",
    "id": "Sistem",
    "fa": "سیستم",
    "pa": "ਸਿਸਟਮ",
    "uk": "Система",
    "ro": "Sistem",
    "tl": "System"
  },

  "Dark": {
    "en": "Dark",
    "fr": "Sombre",
    "de": "Dunkel",
    "ar": "داكن",
    "es": "Oscuro",
    "tr": "Koyu",
    "nl": "Donker",
    "it": "Scuro",
    "ja": "ダーク",
    "zh": "深色",
    "ko": "다크",
    "hi": "डार्क",
    "pt": "Escuro",
    "ru": "Тёмная",
    "sw": "Giza",
    "pl": "Ciemny",
    "id": "Gelap",
    "fa": "تیره",
    "pa": "ਗੂੜ੍ਹਾ",
    "uk": "Темна",
    "ro": "Întunecat",
    "tl": "Dark"
  },

  "FirstName": {
    "en": "First Name",
    "fr": "Prénom",
    "de": "Vorname",
    "ar": "الاسم الأول",
    "es": "Nombre",
    "tr": "Ad",
    "nl": "Voornaam",
    "it": "Nome",
    "ja": "名",
    "zh": "名字",
    "ko": "이름",
    "hi": "पहला नाम",
    "pt": "Nome",
    "ru": "Имя",
    "sw": "Jina la Kwanza",
    "pl": "Imię",
    "id": "Nama Depan",
    "fa": "نام",
    "pa": "ਪਹਿਲਾ ਨਾਮ",
    "uk": "Ім'я",
    "ro": "Prenume",
    "tl": "Pangalan"
  },

  "LastName": {
    "en": "Last Name",
    "fr": "Nom",
    "de": "Nachname",
    "ar": "اسم العائلة",
    "es": "Apellido",
    "tr": "Soyad",
    "nl": "Achternaam",
    "it": "Cognome",
    "ja": "姓",
    "zh": "姓氏",
    "ko": "성",
    "hi": "उपनाम",
    "pt": "Sobrenome",
    "ru": "Фамилия",
    "sw": "Jina la Mwisho",
    "pl": "Nazwisko",
    "id": "Nama Belakang",
    "fa": "نام خانوادگی",
    "pa": "ਆਖਰੀ ਨਾਮ",
    "uk": "Прізвище",
    "ro": "Nume",
    "tl": "Apelyido"
  },

  "Username": {
    "en": "Username",
    "fr": "Nom d'utilisateur",
    "de": "Benutzername",
    "ar": "اسم المستخدم",
    "es": "Nombre de usuario",
    "tr": "Kullanıcı adı",
    "nl": "Gebruikersnaam",
    "it": "Nome utente",
    "ja": "ユーザー名",
    "zh": "用户名",
    "ko": "사용자 이름",
    "hi": "उपयोगकर्ता नाम",
    "pt": "Nome de usuário",
    "ru": "Имя пользователя",
    "sw": "Jina la Mtumiaji",
    "pl": "Nazwa użytkownika",
    "id": "Nama pengguna",
    "fa": "نام کاربری",
    "pa": "ਯੂਜ਼ਰਨੇਮ",
    "uk": "Ім'я користувача",
    "ro": "Nume utilizator",
    "tl": "Username"
  },

  "Birthday": {
    "en": "Birthday",
    "fr": "Date de naissance",
    "de": "Geburtstag",
    "ar": "تاريخ الميلاد",
    "es": "Fecha de nacimiento",
    "tr": "Doğum günü",
    "nl": "Verjaardag",
    "it": "Data di nascita",
    "ja": "生年月日",
    "zh": "生日",
    "ko": "생일",
    "hi": "जन्मदिन",
    "pt": "Data de nascimento",
    "ru": "Дата рождения",
    "sw": "Siku ya kuzaliwa",
    "pl": "Data urodzenia",
    "id": "Tanggal lahir",
    "fa": "تاریخ تولد",
    "pa": "ਜਨਮਦਿਨ",
    "uk": "Дата народження",
    "ro": "Data nașterii",
    "tl": "Kaarawan"
  },


  "Premium": {
    "en": "Premium",
    "fr": "Premium",
    "de": "Premium",
    "ar": "بريميوم",
    "es": "Premium",
    "tr": "Premium",
    "nl": "Premium",
    "it": "Premium",
    "ja": "プレミアム",
    "zh": "高级版",
    "ko": "프리미엄",
    "hi": "प्रीमियम",
    "pt": "Premium",
    "ru": "Премиум",
    "sw": "Premium",
    "pl": "Premium",
    "id": "Premium",
    "fa": "پریمیوم",
    "pa": "ਪ੍ਰੀਮੀਅਮ",
    "uk": "Преміум",
    "ro": "Premium",
    "tl": "Premium"
  },

  "EnhanceExp": {
    "en": "Enhance your reading experience",
    "fr": "Améliorez votre expérience de lecture",
    "de": "Verbessern Sie Ihr Leseerlebnis",
    "ar": "حسّن تجربة القراءة الخاصة بك",
    "es": "Mejora tu experiencia de lectura",
    "tr": "Okuma deneyiminizi geliştirin",
    "nl": "Verbeter uw leeservaring",
    "it": "Migliora la tua esperienza di lettura",
    "ja": "読書体験を向上させる",
    "zh": "提升您的阅读体验",
    "ko": "읽기 경험을 향상시키세요",
    "hi": "अपने पढ़ने के अनुभव को बेहतर बनाएं",
    "pt": "Melhore sua experiência de leitura",
    "ru": "Улучшите свой опыт чтения",
    "sw": "Boresha uzoefu wako wa kusoma",
    "pl": "Popraw swoje doświadczenie czytania",
    "id": "Tingkatkan pengalaman membaca Anda",
    "fa": "تجربه مطالعه خود را ارتقا دهید",
    "pa": "ਆਪਣੇ ਪੜ੍ਹਨ ਦੇ ਅਨੁਭਵ ਨੂੰ ਬਿਹਤਰ ਬਣਾਓ",
    "uk": "Покращіть свій досвід читання",
    "ro": "Îmbunătățește experiența ta de lectură",
    "tl": "Pagandahin ang iyong karanasan sa pagbabasa"
  },

  "Monthly": {
    "en": "Monthly",
    "fr": "Mensuel",
    "de": "Monatlich",
    "ar": "شهري",
    "es": "Mensual",
    "tr": "Aylık",
    "nl": "Maandelijks",
    "it": "Mensile",
    "ja": "月額",
    "zh": "每月",
    "ko": "월간",
    "hi": "मासिक",
    "pt": "Mensal",
    "ru": "Ежемесячно",
    "sw": "Kila mwezi",
    "pl": "Miesięcznie",
    "id": "Bulanan",
    "fa": "ماهانه",
    "pa": "ਮਾਸਿਕ",
    "uk": "Щомісячно",
    "ro": "Lunar",
    "tl": "Buwanang"
  },

  "Yearly": {
    "en": "Yearly",
    "fr": "Annuel",
    "de": "Jährlich",
    "ar": "سنوي",
    "es": "Anual",
    "tr": "Yıllık",
    "nl": "Jaarlijks",
    "it": "Annuale",
    "ja": "年額",
    "zh": "每年",
    "ko": "연간",
    "hi": "वार्षिक",
    "pt": "Anual",
    "ru": "Ежегодно",
    "sw": "Kila mwaka",
    "pl": "Rocznie",
    "id": "Tahunan",
    "fa": "سالانه",
    "pa": "ਸਾਲਾਨਾ",
    "uk": "Щорічно",
    "ro": "Anual",
    "tl": "Taunan"
  },

  "Active": {
    "en": "Active",
    "fr": "Actif",
    "de": "Aktiv",
    "ar": "نشط",
    "es": "Activo",
    "tr": "Aktif",
    "nl": "Actief",
    "it": "Attivo",
    "ja": "有効",
    "zh": "已激活",
    "ko": "활성",
    "hi": "सक्रिय",
    "pt": "Ativo",
    "ru": "Активен",
    "sw": "Hai",
    "pl": "Aktywny",
    "id": "Aktif",
    "fa": "فعال",
    "pa": "ਸਕ੍ਰਿਯ",
    "uk": "Активний",
    "ro": "Activ",
    "tl": "Aktibo"
  },

  "GeneralNotify": {
    "en": "General Notification",
    "fr": "Notifications générales",
    "de": "Allgemeine Benachrichtigungen",
    "ar": "الإشعارات العامة",
    "es": "Notificaciones generales",
    "tr": "Genel Bildirimler",
    "nl": "Algemene meldingen",
    "it": "Notifiche generali",
    "ja": "一般通知",
    "zh": "常规通知",
    "ko": "일반 알림",
    "hi": "सामान्य सूचनाएँ",
    "pt": "Notificações gerais",
    "ru": "Общие уведомления",
    "sw": "Arifa za Jumla",
    "pl": "Powiadomienia ogólne",
    "id": "Notifikasi Umum",
    "fa": "اعلان‌های عمومی",
    "pa": "ਆਮ ਨੋਟੀਫਿਕੇਸ਼ਨ",
    "uk": "Загальні сповіщення",
    "ro": "Notificări generale",
    "tl": "Pangkalahatang Notipikasyon"
  },

  "EnablePush": {
    "en": "Enable Push Notification",
    "fr": "Activer les notifications push",
    "de": "Push-Benachrichtigungen aktivieren",
    "ar": "تمكين الإشعارات الفورية",
    "es": "Activar notificaciones push",
    "tr": "Anlık bildirimleri etkinleştir",
    "nl": "Pushmeldingen inschakelen",
    "it": "Abilita notifiche push",
    "ja": "プッシュ通知を有効にする",
    "zh": "启用推送通知",
    "ko": "푸시 알림 활성화",
    "hi": "पुश नोटिफिकेशन सक्षम करें",
    "pt": "Ativar notificações push",
    "ru": "Включить push-уведомления",
    "sw": "Washa arifa za push",
    "pl": "Włącz powiadomienia push",
    "id": "Aktifkan notifikasi push",
    "fa": "فعال‌سازی اعلان‌های پوش",
    "pa": "ਪੁਸ਼ ਨੋਟੀਫਿਕੇਸ਼ਨ ਚਾਲੂ ਕਰੋ",
    "uk": "Увімкнути push-сповіщення",
    "ro": "Activează notificările push",
    "tl": "I-enable ang push notification"
  },

  "NewsUpdate": {
    "en": "News Update",
    "fr": "Mises à jour des actualités",
    "de": "Nachrichten-Updates",
    "ar": "تحديثات الأخبار",
    "es": "Actualizaciones de noticias",
    "tr": "Haber Güncellemeleri",
    "nl": "Nieuwsupdates",
    "it": "Aggiornamenti notizie",
    "ja": "ニュース更新",
    "zh": "新闻更新",
    "ko": "뉴스 업데이트",
    "hi": "समाचार अपडेट",
    "pt": "Atualizações de notícias",
    "ru": "Обновления новостей",
    "sw": "Masasisho ya Habari",
    "pl": "Aktualizacje wiadomości",
    "id": "Pembaruan berita",
    "fa": "به‌روزرسانی اخبار",
    "pa": "ਖਬਰਾਂ ਅਪਡੇਟ",
    "uk": "Оновлення новин",
    "ro": "Actualizări știri",
    "tl": "Update ng Balita"
  },

  "CommRepl": {
    "en": "Comments & Replies",
    "fr": "Commentaires et réponses",
    "de": "Kommentare und Antworten",
    "ar": "التعليقات والردود",
    "es": "Comentarios y respuestas",
    "tr": "Yorumlar ve Yanıtlar",
    "nl": "Reacties en antwoorden",
    "it": "Commenti e risposte",
    "ja": "コメントと返信",
    "zh": "评论与回复",
    "ko": "댓글 및 답글",
    "hi": "टिप्पणियाँ और उत्तर",
    "pt": "Comentários e respostas",
    "ru": "Комментарии и ответы",
    "sw": "Maoni na Majibu",
    "pl": "Komentarze i odpowiedzi",
    "id": "Komentar & Balasan",
    "fa": "نظرات و پاسخ‌ها",
    "pa": "ਟਿੱਪਣੀਆਂ ਅਤੇ ਜਵਾਬ",
    "uk": "Коментарі та відповіді",
    "ro": "Comentarii și răspunsuri",
    "tl": "Mga Komento at Tugon"
  },


"Features": {
    "en": "Features",
    "fr": "Fonctionnalités",
    "de": "Funktionen",
    "ar": "الميزات",
    "es": "Funciones",
    "tr": "Özellikler",
    "nl": "Functies",
    "it": "Funzionalità",
    "ja": "機能",
    "zh": "功能",
    "ko": "기능",
    "hi": "विशेषताएँ",
    "pt": "Recursos",
    "ru": "Функции",
    "sw": "Vipengele",
    "pl": "Funkcje",
    "id": "Fitur",
    "fa": "ویژگی‌ها",
    "pa": "ਵਿਸ਼ੇਸ਼ਤਾਵਾਂ",
    "uk": "Функції",
    "ro": "Funcții",
    "tl": "Mga Feature"
  },

  "AccessHundred": {
    "en": "Access to News from 110+ countries",
    "fr": "Accès aux actualités de 110+ pays",
    "de": "Zugang zu Nachrichten aus über 110 Ländern",
    "ar": "الوصول إلى الأخبار من أكثر من 110 دولة",
    "es": "Acceso a noticias de más de 110 países",
    "tr": "110+ ülkeden haberlere erişim",
    "nl": "Toegang tot nieuws uit 110+ landen",
    "it": "Accesso a notizie da oltre 110 paesi",
    "ja": "110以上の国のニュースにアクセス",
    "zh": "访问来自110多个国家的新闻",
    "ko": "110개 이상의 국가 뉴스 접근",
    "hi": "110+ देशों की खबरों तक पहुंच",
    "pt": "Acesso a notícias de mais de 110 países",
    "ru": "Доступ к новостям из 110+ стран",
    "sw": "Ufikiaji wa habari kutoka zaidi ya nchi 110",
    "pl": "Dostęp do wiadomości z ponad 110 krajów",
    "id": "Akses berita dari 110+ negara",
    "fa": "دسترسی به اخبار از بیش از 110 کشور",
    "pa": "110+ ਦੇਸ਼ਾਂ ਦੀਆਂ ਖਬਰਾਂ ਤੱਕ ਪਹੁੰਚ",
    "uk": "Доступ до новин із понад 110 країн",
    "ro": "Acces la știri din peste 110 țări",
    "tl": "Access sa balita mula sa 110+ bansa"
  },

  "AIRead": {
    "en": "AI Read Aloud",
    "fr": "Lecture vocale IA",
    "de": "KI-Vorlesen",
    "ar": "القراءة الصوتية بالذكاء الاصطناعي",
    "es": "Lectura en voz alta con IA",
    "tr": "Yapay Zeka ile Sesli Okuma",
    "nl": "AI voorlezen",
    "it": "Lettura vocale IA",
    "ja": "AI音声読み上げ",
    "zh": "AI 朗读",
    "ko": "AI 음성 읽기",
    "hi": "एआई वाचन",
    "pt": "Leitura em voz alta com IA",
    "ru": "Озвучивание ИИ",
    "sw": "Usomaji wa sauti wa AI",
    "pl": "Odczyt AI",
    "id": "Baca dengan AI",
    "fa": "خواندن صوتی با هوش مصنوعی",
    "pa": "ਏਆਈ ਨਾਲ ਆਵਾਜ਼ੀ ਪਾਠ",
    "uk": "Озвучування ШІ",
    "ro": "Citire vocală AI",
    "tl": "AI Read Aloud"
  },

  "Billing": {
    "en": "Billing",
    "fr": "Facturation",
    "de": "Abrechnung",
    "ar": "الفوترة",
    "es": "Facturación",
    "tr": "Faturalandırma",
    "nl": "Facturering",
    "it": "Fatturazione",
    "ja": "請求",
    "zh": "账单",
    "ko": "결제",
    "hi": "बिलिंग",
    "pt": "Faturamento",
    "ru": "Оплата",
    "sw": "Malipo",
    "pl": "Rozliczenia",
    "id": "Penagihan",
    "fa": "صورتحساب",
    "pa": "ਬਿਲਿੰਗ",
    "uk": "Оплата",
    "ro": "Facturare",
    "tl": "Pagsingil"
  },

  "BillHistory": {
    "en": "Billing History",
    "fr": "Historique de facturation",
    "de": "Abrechnungsverlauf",
    "ar": "سجل الفوترة",
    "es": "Historial de facturación",
    "tr": "Fatura Geçmişi",
    "nl": "Factuurgeschiedenis",
    "it": "Cronologia fatturazione",
    "ja": "請求履歴",
    "zh": "账单历史",
    "ko": "결제 내역",
    "hi": "बिलिंग इतिहास",
    "pt": "Histórico de faturamento",
    "ru": "История платежей",
    "sw": "Historia ya malipo",
    "pl": "Historia rozliczeń",
    "id": "Riwayat penagihan",
    "fa": "سابقه صورتحساب",
    "pa": "ਬਿਲਿੰਗ ਇਤਿਹਾਸ",
    "uk": "Історія платежів",
    "ro": "Istoric facturare",
    "tl": "Kasaysayan ng Pagsingil"
  },

  "freePlan": {
    "en": "You are currently on a free plan",
    "fr": "Vous êtes actuellement sur un forfait gratuit",
    "de": "Sie nutzen derzeit den kostenlosen Plan",
    "ar": "أنت حاليًا على الخطة المجانية",
    "es": "Actualmente estás en un plan gratuito",
    "tr": "Şu anda ücretsiz plandasınız",
    "nl": "U gebruikt momenteel het gratis abonnement",
    "it": "Attualmente sei nel piano gratuito",
    "ja": "現在無料プランをご利用中です",
    "zh": "您当前使用的是免费套餐",
    "ko": "현재 무료 플랜을 사용 중입니다",
    "hi": "आप वर्तमान में मुफ्त प्लान पर हैं",
    "pt": "Você está atualmente no plano gratuito",
    "ru": "В настоящее время вы используете бесплатный план",
    "sw": "Kwa sasa uko kwenye mpango wa bure",
    "pl": "Obecnie korzystasz z planu bezpłatnego",
    "id": "Anda saat ini menggunakan paket gratis",
    "fa": "شما در حال حاضر در طرح رایگان هستید",
    "pa": "ਤੁਸੀਂ ਇਸ ਵੇਲੇ ਮੁਫ਼ਤ ਯੋਜਨਾ 'ਤੇ ਹੋ",
    "uk": "Наразі ви користуєтесь безкоштовним планом",
    "ro": "În prezent utilizați planul gratuit",
    "tl": "Kasalukuyan kang nasa libreng plano"
  },

  "Cards": {
    "en": "Cards",
    "fr": "Cartes",
    "de": "Karten",
    "ar": "البطاقات",
    "es": "Tarjetas",
    "tr": "Kartlar",
    "nl": "Kaarten",
    "it": "Carte",
    "ja": "カード",
    "zh": "卡片",
    "ko": "카드",
    "hi": "कार्ड",
    "pt": "Cartões",
    "ru": "Карты",
    "sw": "Kadi",
    "pl": "Karty",
    "id": "Kartu",
    "fa": "کارت‌ها",
    "pa": "ਕਾਰਡ",
    "uk": "Картки",
    "ro": "Carduri",
    "tl": "Mga Card"
  },


  "updateProfile":{
  "en": "Update Profile",
  "fr": "Mettre à jour le profil",
  "de": "Profil aktualisieren",
  "ar": "تحديث الملف الشخصي",
  "es": "Actualizar perfil",
  "tr": "Profili güncelle",
  "nl": "Profiel bijwerken",
  "it": "Aggiorna profilo",
  "ja": "プロフィールを更新",
  "zh": "更新个人资料",
  "ko": "프로필 업데이트",
  "hi": "प्रोफ़ाइल अपडेट करें",
  "pt": "Atualizar perfil",
  "ru": "Обновить профиль",
  "sw": "Sasisha wasifu",
  "pl": "Zaktualizuj profil",
  "id": "Perbarui profil",
  "fa": "به‌روزرسانی پروفایل",
  "pa": "ਪ੍ਰੋਫਾਈਲ ਅੱਪਡੇਟ ਕਰੋ",
  "uk": "Оновити профіль",
  "ro": "Actualizează profilul",
  "tl": "I-update ang profile"
  },

  "Applang":{
  "en": "App Language",
  "fr": "Langue de l'application",
  "de": "App-Sprache",
  "ar": "لغة التطبيق",
  "es": "Idioma de la aplicación",
  "tr": "Uygulama dili",
  "nl": "App-taal",
  "it": "Lingua dell'app",
  "ja": "アプリの言語",
  "zh": "应用语言",
  "ko": "앱 언어",
  "hi": "ऐप भाषा",
  "pt": "Idioma do aplicativo",
  "ru": "Язык приложения",
  "sw": "Lugha ya programu",
  "pl": "Język aplikacji",
  "id": "Bahasa aplikasi",
  "fa": "زبان برنامه",
  "pa": "ਐਪ ਭਾਸ਼ਾ",
  "uk": "Мова додатку",
  "ro": "Limba aplicației",
  "tl": "Wika ng app"
  },


  "ChooseAppL":{
  "en": "Choose App Language",
  "fr": "Choisir la langue de l'application",
  "de": "App-Sprache auswählen",
  "ar": "اختر لغة التطبيق",
  "es": "Elegir idioma de la aplicación",
  "tr": "Uygulama dilini seç",
  "nl": "Kies app-taal",
  "it": "Scegli lingua dell'app",
  "ja": "アプリの言語を選択",
  "zh": "选择应用语言",
  "ko": "앱 언어 선택",
  "hi": "ऐप भाषा चुनें",
  "pt": "Escolher idioma do aplicativo",
  "ru": "Выбрать язык приложения",
  "sw": "Chagua lugha ya programu",
  "pl": "Wybierz język aplikacji",
  "id": "Pilih bahasa aplikasi",
  "fa": "انتخاب زبان برنامه",
  "pa": "ਐਪ ਭਾਸ਼ਾ ਚੁਣੋ",
  "uk": "Оберіть мову додатку",
  "ro": "Alege limba aplicației",
  "tl": "Piliin ang wika ng app"
  },


  "TranslateLang":{
  "en": "Translate Language",
  "fr": "Langue de traduction",
  "de": "Übersetzungssprache",
  "ar": "لغة الترجمة",
  "es": "Idioma de traducción",
  "tr": "Çeviri dili",
  "nl": "Vertaaltaal",
  "it": "Lingua di traduzione",
  "ja": "翻訳言語",
  "zh": "翻译语言",
  "ko": "번역 언어",
  "hi": "अनुवाद भाषा",
  "pt": "Idioma de tradução",
  "ru": "Язык перевода",
  "sw": "Lugha ya tafsiri",
  "pl": "Język tłumaczenia",
  "id": "Bahasa terjemahan",
  "fa": "زبان ترجمه",
  "pa": "ਅਨੁਵਾਦ ਭਾਸ਼ਾ",
  "uk": "Мова перекладу",
  "ro": "Limba de traducere",
  "tl": "Wika ng pagsasalin"
  },


  "chooseTransL":{
  "en": "Choose Translate Language",
  "fr": "Choisir la langue de traduction",
  "de": "Übersetzungssprache auswählen",
  "ar": "اختر لغة الترجمة",
  "es": "Elegir idioma de traducción",
  "tr": "Çeviri dilini seç",
  "nl": "Kies vertaaltaal",
  "it": "Scegli lingua di traduzione",
  "ja": "翻訳言語を選択",
  "zh": "选择翻译语言",
  "ko": "번역 언어 선택",
  "hi": "अनुवाद भाषा चुनें",
  "pt": "Escolher idioma de tradução",
  "ru": "Выбрать язык перевода",
  "sw": "Chagua lugha ya tafsiri",
  "pl": "Wybierz język tłumaczenia",
  "id": "Pilih bahasa terjemahan",
  "fa": "انتخاب زبان ترجمه",
  "pa": "ਅਨੁਵਾਦ ਭਾਸ਼ਾ ਚੁਣੋ",
  "uk": "Оберіть мову перекладу",
  "ro": "Alege limba de traducere",
  "tl": "Piliin ang wika ng pagsasalin"
  },



  "Engage":{
  "en": "Engagement",
  "fr": "Engagement",
  "de": "Interaktion",
  "ar": "التفاعل",
  "es": "Interacción",
  "tr": "Etkileşim",
  "nl": "Betrokkenheid",
  "it": "Coinvolgimento",
  "ja": "エンゲージメント",
  "zh": "互动",
  "ko": "참여",
  "hi": "सहभागिता",
  "pt": "Engajamento",
  "ru": "Вовлеченность",
  "sw": "Ushiriki",
  "pl": "Zaangażowanie",
  "id": "Keterlibatan",
  "fa": "تعامل",
  "pa": "ਭਾਗੀਦਾਰੀ",
  "uk": "Залученість",
  "ro": "Implicare",
  "tl": "Pakikilahok"
  },


  "tofs":{
  "en": "Terms of Service",
  "fr": "Conditions d'utilisation",
  "de": "Nutzungsbedingungen",
  "ar": "شروط الخدمة",
  "es": "Términos de servicio",
  "tr": "Hizmet Şartları",
  "nl": "Servicevoorwaarden",
  "it": "Termini di servizio",
  "ja": "利用規約",
  "zh": "服务条款",
  "ko": "서비스 약관",
  "hi": "सेवा की शर्तें",
  "pt": "Termos de serviço",
  "ru": "Условия использования",
  "sw": "Masharti ya huduma",
  "pl": "Warunki korzystania",
  "id": "Syarat layanan",
  "fa": "شرایط استفاده",
  "pa": "ਸੇਵਾ ਦੀਆਂ ਸ਼ਰਤਾਂ",
  "uk": "Умови користування",
  "ro": "Termeni și condiții",
  "tl": "Mga tuntunin ng serbisyo"
  },


  "PrivacyPoly":{
  "en": "Privacy Policy",
  "fr": "Politique de confidentialité",
  "de": "Datenschutzrichtlinie",
  "ar": "سياسة الخصوصية",
  "es": "Política de privacidad",
  "tr": "Gizlilik Politikası",
  "nl": "Privacybeleid",
  "it": "Informativa sulla privacy",
  "ja": "プライバシーポリシー",
  "zh": "隐私政策",
  "ko": "개인정보 처리방침",
  "hi": "गोपनीयता नीति",
  "pt": "Política de privacidade",
  "ru": "Политика конфиденциальности",
  "sw": "Sera ya faragha",
  "pl": "Polityka prywatności",
  "id": "Kebijakan privasi",
  "fa": "سیاست حفظ حریم خصوصی",
  "pa": "ਗੋਪਨੀਯਤਾ ਨੀਤੀ",
  "uk": "Політика конфіденційності",
  "ro": "Politica de confidențialitate",
  "tl": "Patakaran sa privacy"
  },


  "changePass":{
  "en": "Change Password",
  "fr": "Changer le mot de passe",
  "de": "Passwort ändern",
  "ar": "تغيير كلمة المرور",
  "es": "Cambiar contraseña",
  "tr": "Şifre değiştir",
  "nl": "Wachtwoord wijzigen",
  "it": "Cambia password",
  "ja": "パスワードを変更",
  "zh": "更改密码",
  "ko": "비밀번호 변경",
  "hi": "पासवर्ड बदलें",
  "pt": "Alterar senha",
  "ru": "Сменить пароль",
  "sw": "Badilisha nenosiri",
  "pl": "Zmień hasło",
  "id": "Ubah kata sandi",
  "fa": "تغییر رمز عبور",
  "pa": "ਪਾਸਵਰਡ ਬਦਲੋ",
  "uk": "Змінити пароль",
  "ro": "Schimbă parola",
  "tl": "Palitan ang password"
  },


  "deleteMyAcc":{
  "en": "Delete my Account",
  "fr": "Supprimer mon compte",
  "de": "Mein Konto löschen",
  "ar": "حذف حسابي",
  "es": "Eliminar mi cuenta",
  "tr": "Hesabımı sil",
  "nl": "Mijn account verwijderen",
  "it": "Elimina il mio account",
  "ja": "アカウントを削除",
  "zh": "删除我的账户",
  "ko": "내 계정 삭제",
  "hi": "मेरा खाता हटाएँ",
  "pt": "Excluir minha conta",
  "ru": "Удалить мой аккаунт",
  "sw": "Futa akaunti yangu",
  "pl": "Usuń moje konto",
  "id": "Hapus akun saya",
  "fa": "حذف حساب من",
  "pa": "ਮੇਰਾ ਖਾਤਾ ਮਿਟਾਓ",
  "uk": "Видалити мій акаунт",
  "ro": "Șterge contul meu",
  "tl": "I-delete ang aking account"
  },

  "newsAlert":{
  "en": "News Alerts",
  "fr": "Alertes d'actualités",
  "de": "Nachrichtenwarnungen",
  "ar": "تنبيهات الأخبار",
  "es": "Alertas de noticias",
  "tr": "Haber Uyarıları",
  "nl": "Nieuwswaarschuwingen",
  "it": "Avvisi notizie",
  "ja": "ニュース通知",
  "zh": "新闻提醒",
  "ko": "뉴스 알림",
  "hi": "समाचार अलर्ट",
  "pt": "Alertas de notícias",
  "ru": "Оповещения о новостях",
  "sw": "Tahadhari za Habari",
  "pl": "Alerty wiadomości",
  "id": "Peringatan berita",
  "fa": "هشدارهای خبری",
  "pa": "ਖਬਰ ਅਲਰਟ",
  "uk": "Сповіщення про новини",
  "ro": "Alerte știri",
  "tl": "Mga alerto ng balita"
  },


  "engageAchive":{
  "en": "Engagement Archive",
  "fr": "Archive des interactions",
  "de": "Interaktionsarchiv",
  "ar": "أرشيف التفاعل",
  "es": "Archivo de interacciones",
  "tr": "Etkileşim Arşivi",
  "nl": "Betrokkenheidsarchief",
  "it": "Archivio interazioni",
  "ja": "エンゲージメント履歴",
  "zh": "互动记录",
  "ko": "참여 기록",
  "hi": "सहभागिता संग्रह",
  "pt": "Arquivo de engajamento",
  "ru": "Архив взаимодействий",
  "sw": "Kumbukumbu za Ushiriki",
  "pl": "Archiwum zaangażowania",
  "id": "Arsip keterlibatan",
  "fa": "آرشیو تعاملات",
  "pa": "ਭਾਗੀਦਾਰੀ ਆਰਕਾਈਵ",
  "uk": "Архів взаємодії",
  "ro": "Arhivă interacțiuni",
  "tl": "Archive ng pakikilahok"
  },


  "subscribeAlert":{
  "en": "Subscription Alerts",
  "fr": "Alertes d'abonnement",
  "de": "Abonnement-Benachrichtigungen",
  "ar": "تنبيهات الاشتراك",
  "es": "Alertas de suscripción",
  "tr": "Abonelik uyarıları",
  "nl": "Abonnementsmeldingen",
  "it": "Avvisi di abbonamento",
  "ja": "サブスクリプション通知",
  "zh": "订阅提醒",
  "ko": "구독 알림",
  "hi": "सदस्यता अलर्ट",
  "pt": "Alertas de assinatura",
  "ru": "Оповещения о подписке",
  "sw": "Tahadhari za usajili",
  "pl": "Alerty subskrypcji",
  "id": "Peringatan langganan",
  "fa": "هشدارهای اشتراک",
  "pa": "ਸਬਸਕ੍ਰਿਪਸ਼ਨ ਅਲਰਟ",
  "uk": "Сповіщення про підписку",
  "ro": "Alerte abonament",
  "tl": "Mga alerto sa subscription"
  },



  "security":{
  "en": "Security",
  "fr": "Sécurité",
  "de": "Sicherheit",
  "ar": "الأمان",
  "es": "Seguridad",
  "tr": "Güvenlik",
  "nl": "Beveiliging",
  "it": "Sicurezza",
  "ja": "セキュリティ",
  "zh": "安全",
  "ko": "보안",
  "hi": "सुरक्षा",
  "pt": "Segurança",
  "ru": "Безопасность",
  "sw": "Usalama",
  "pl": "Bezpieczeństwo",
  "id": "Keamanan",
  "fa": "امنیت",
  "pa": "ਸੁਰੱਖਿਆ",
  "uk": "Безпека",
  "ro": "Securitate",
  "tl": "Seguridad"
  },

  "AccessGlobal": {
  "en": "Access to Global Search",
  "fr": "Accès à la recherche mondiale",
  "de": "Zugang zur globalen Suche",
  "ar": "الوصول إلى البحث العالمي",
  "es": "Acceso a búsqueda global",
  "tr": "Küresel aramaya erişim",
  "nl": "Toegang tot wereldwijde zoekfunctie",
  "it": "Accesso alla ricerca globale",
  "ja": "グローバル検索へのアクセス",
  "zh": "访问全球搜索",
  "ko": "글로벌 검색 접근",
  "hi": "वैश्विक खोज तक पहुंच",
  "pt": "Acesso à pesquisa global",
  "ru": "Доступ к глобальному поиску",
  "sw": "Ufikiaji wa utafutaji wa kimataifa",
  "pl": "Dostęp do wyszukiwania globalnego",
  "id": "Akses pencarian global",
  "fa": "دسترسی به جستجوی جهانی",
  "pa": "ਗਲੋਬਲ ਖੋਜ ਤੱਕ ਪਹੁੰਚ",
  "uk": "Доступ до глобального пошуку",
  "ro": "Acces la căutare globală",
  "tl": "Access sa global search"
  },


  "commentNews":{
  "en": "Comment on News Articles",
  "fr": "Commenter les articles",
  "de": "Artikel kommentieren",
  "ar": "التعليق على المقالات",
  "es": "Comentar artículos",
  "tr": "Haber makalelerine yorum yap",
  "nl": "Reageren op artikelen",
  "it": "Commenta gli articoli",
  "ja": "記事にコメントする",
  "zh": "评论新闻文章",
  "ko": "뉴스 기사에 댓글 달기",
  "hi": "समाचार लेखों पर टिप्पणी करें",
  "pt": "Comentar artigos",
  "ru": "Комментировать статьи",
  "sw": "Toa maoni kwenye makala",
  "pl": "Komentuj artykuły",
  "id": "Komentar pada artikel",
  "fa": "اظهار نظر درباره مقالات",
  "pa": "ਖਬਰ ਲੇਖਾਂ 'ਤੇ ਟਿੱਪਣੀ ਕਰੋ",
  "uk": "Коментувати статті",
  "ro": "Comentează articolele",
  "tl": "Magkomento sa mga artikulo"
  },


  "translateTwenty":{
  "en": "Translate News to 22+ languages",
  "fr": "Traduire les actualités en plus de 22 langues",
  "de": "Nachrichten in über 22 Sprachen übersetzen",
  "ar": "ترجمة الأخبار إلى أكثر من 22 لغة",
  "es": "Traducir noticias a más de 22 idiomas",
  "tr": "Haberleri 22+ dile çevir",
  "nl": "Nieuws vertalen naar 22+ talen",
  "it": "Traduci notizie in oltre 22 lingue",
  "ja": "ニュースを22以上の言語に翻訳",
  "zh": "将新闻翻译成22种以上语言",
  "ko": "뉴스를 22개 이상 언어로 번역",
  "hi": "समाचार को 22+ भाषाओं में अनुवाद करें",
  "pt": "Traduzir notícias para mais de 22 idiomas",
  "ru": "Перевод новостей на более чем 22 языка",
  "sw": "Tafsiri habari kwa lugha zaidi ya 22",
  "pl": "Tłumacz wiadomości na ponad 22 języki",
  "id": "Terjemahkan berita ke 22+ bahasa",
  "fa": "ترجمه اخبار به بیش از 22 زبان",
  "pa": "ਖਬਰਾਂ ਨੂੰ 22+ ਭਾਸ਼ਾਵਾਂ ਵਿੱਚ ਅਨੁਵਾਦ ਕਰੋ",
  "uk": "Переклад новин на понад 22 мови",
  "ro": "Traduce știri în peste 22 de limbi",
  "tl": "Isalin ang balita sa 22+ wika"
  },




"getFull":{
"en": "Get Full Access",
  "fr": "Obtenir l'accès complet",
  "de": "Vollen Zugriff erhalten",
  "ar": "احصل على وصول كامل",
  "es": "Obtener acceso completo",
  "tr": "Tam erişim al",
  "nl": "Krijg volledige toegang",
  "it": "Ottieni accesso completo",
  "ja": "フルアクセスを取得",
  "zh": "获取完整访问权限",
  "ko": "전체 이용 권한 받기",
  "hi": "पूर्ण पहुंच प्राप्त करें",
  "pt": "Obter acesso completo",
  "ru": "Получить полный доступ",
  "sw": "Pata ufikiaji kamili",
  "pl": "Uzyskaj pełny dostęp",
  "id": "Dapatkan akses penuh",
  "fa": "دریافت دسترسی کامل",
  "pa": "ਪੂਰਾ ਐਕਸੈੱਸ ਪ੍ਰਾਪਤ ਕਰੋ",
  "uk": "Отримати повний доступ",
  "ro": "Obține acces complet",
  "tl": "Kunin ang full access"
},



"activeTill":{
"en": "Active till",
  "fr": "Actif jusqu'au",
  "de": "Aktiv bis",
  "ar": "نشط حتى",
  "es": "Activo hasta",
  "tr": "Şu tarihe kadar aktif",
  "nl": "Actief tot",
  "it": "Attivo fino al",
  "ja": "有効期限",
  "zh": "有效至",
  "ko": "유효 기간",
  "hi": "सक्रिय है तक",
  "pt": "Ativo até",
  "ru": "Активен до",
  "sw": "Hai hadi",
  "pl": "Aktywny do",
  "id": "Aktif hingga",
  "fa": "فعال تا",
  "pa": "ਇਸ ਤੱਕ ਸਕ੍ਰਿਯ",
  "uk": "Активний до",
  "ro": "Activ până la",
  "tl": "Aktibo hanggang"
},


"trackBill":{
"en": "Track Billing History",
  "fr": "Suivre l'historique de facturation",
  "de": "Abrechnungsverlauf verfolgen",
  "ar": "تتبع سجل الفوترة",
  "es": "Seguir historial de facturación",
  "tr": "Fatura geçmişini takip et",
  "nl": "Factuurgeschiedenis volgen",
  "it": "Monitora cronologia fatturazione",
  "ja": "請求履歴を確認",
  "zh": "查看账单历史",
  "ko": "결제 내역 확인",
  "hi": "बिलिंग इतिहास ट्रैक करें",
  "pt": "Acompanhar histórico de faturamento",
  "ru": "Отслеживать историю платежей",
  "sw": "Fuatilia historia ya malipo",
  "pl": "Śledź historię rozliczeń",
  "id": "Lacak riwayat penagihan",
  "fa": "پیگیری سابقه صورتحساب",
  "pa": "ਬਿਲਿੰਗ ਇਤਿਹਾਸ ਟ੍ਰੈਕ ਕਰੋ",
  "uk": "Відстежувати історію платежів",
  "ro": "Urmărește istoricul facturării",
  "tl": "Subaybayan ang kasaysayan ng pagsingil"
},

"customNotify":{
"en": "Customize notification",
  "fr": "Personnaliser les notifications",
  "de": "Benachrichtigungen anpassen",
  "ar": "تخصيص الإشعارات",
  "es": "Personalizar notificaciones",
  "tr": "Bildirimleri özelleştir",
  "nl": "Meldingen aanpassen",
  "it": "Personalizza notifiche",
  "ja": "通知をカスタマイズ",
  "zh": "自定义通知",
  "ko": "알림 사용자 설정",
  "hi": "सूचनाएँ अनुकूलित करें",
  "pt": "Personalizar notificações",
  "ru": "Настроить уведомления",
  "sw": "Binafsisha arifa",
  "pl": "Dostosuj powiadomienia",
  "id": "Sesuaikan notifikasi",
  "fa": "شخصی‌سازی اعلان‌ها",
  "pa": "ਨੋਟੀਫਿਕੇਸ਼ਨ ਕਸਟਮਾਈਜ਼ ਕਰੋ",
  "uk": "Налаштувати сповіщення",
  "ro": "Personalizează notificările",
  "tl": "I-customize ang mga notipikasyon"
},

"voiceIdentity":{
"en": "Voice Identity",
  "fr": "Identité vocale",
  "de": "Stimmidentität",
  "ar": "هوية الصوت",
  "es": "Identidad de voz",
  "tr": "Ses kimliği",
  "nl": "Stemidentiteit",
  "it": "Identità vocale",
  "ja": "音声アイデンティティ",
  "zh": "语音身份",
  "ko": "음성 정체성",
  "hi": "आवाज़ पहचान",
  "pt": "Identidade de voz",
  "ru": "Голосовая идентичность",
  "sw": "Utambulisho wa sauti",
  "pl": "Tożsamość głosu",
  "id": "Identitas suara",
  "fa": "هویت صدا",
  "pa": "ਆਵਾਜ਼ ਪਹਿਚਾਣ",
  "uk": "Голосова ідентичність",
  "ro": "Identitate vocală",
  "tl": "Pagkakakilanlan ng boses"
},


"chooseVoice":{
"en": "Choose Voice",
  "fr": "Choisir la voix",
  "de": "Stimme auswählen",
  "ar": "اختر الصوت",
  "es": "Elegir voz",
  "tr": "Ses seç",
  "nl": "Stem kiezen",
  "it": "Scegli voce",
  "ja": "音声を選択",
  "zh": "选择语音",
  "ko": "음성 선택",
  "hi": "आवाज़ चुनें",
  "pt": "Escolher voz",
  "ru": "Выбрать голос",
  "sw": "Chagua sauti",
  "pl": "Wybierz głos",
  "id": "Pilih suara",
  "fa": "انتخاب صدا",
  "pa": "ਆਵਾਜ਼ ਚੁਣੋ",
  "uk": "Оберіть голос",
  "ro": "Alege vocea",
  "tl": "Piliin ang boses"
},

"dataControl":{
"en": "Data Control",
  "fr": "Contrôle des données",
  "de": "Datenkontrolle",
  "ar": "التحكم في البيانات",
  "es": "Control de datos",
  "tr": "Veri kontrolü",
  "nl": "Gegevensbeheer",
  "it": "Controllo dei dati",
  "ja": "データ管理",
  "zh": "数据控制",
  "ko": "데이터 관리",
  "hi": "डेटा नियंत्रण",
  "pt": "Controle de dados",
  "ru": "Управление данными",
  "sw": "Udhibiti wa data",
  "pl": "Kontrola danych",
  "id": "Kontrol data",
  "fa": "کنترل داده‌ها",
  "pa": "ਡਾਟਾ ਕੰਟਰੋਲ",
  "uk": "Керування даними",
  "ro": "Controlul datelor",
  "tl": "Kontrol ng data"
},


"manageSub":{
"en": "Manage Subscription",
  "fr": "Gérer l'abonnement",
  "de": "Abonnement verwalten",
  "ar": "إدارة الاشتراك",
  "es": "Administrar suscripción",
  "tr": "Aboneliği yönet",
  "nl": "Abonnement beheren",
  "it": "Gestisci abbonamento",
  "ja": "サブスクリプションを管理",
  "zh": "管理订阅",
  "ko": "구독 관리",
  "hi": "सदस्यता प्रबंधित करें",
  "pt": "Gerenciar assinatura",
  "ru": "Управлять подпиской",
  "sw": "Dhibiti usajili",
  "pl": "Zarządzaj subskrypcją",
  "id": "Kelola langganan",
  "fa": "مدیریت اشتراک",
  "pa": "ਸਬਸਕ੍ਰਿਪਸ਼ਨ ਪ੍ਰਬੰਧਿਤ ਕਰੋ",
  "uk": "Керувати підпискою",
  "ro": "Gestionează abonamentul",
  "tl": "Pamahalaan ang subscription"
},


"report":{
"en": "Report",
  "fr": "Signaler",
  "de": "Melden",
  "ar": "الإبلاغ",
  "es": "Reportar",
  "tr": "Bildir",
  "nl": "Melden",
  "it": "Segnala",
  "ja": "報告",
  "zh": "举报",
  "ko": "신고",
  "hi": "रिपोर्ट करें",
  "pt": "Denunciar",
  "ru": "Сообщить",
  "sw": "Ripoti",
  "pl": "Zgłoś",
  "id": "Laporkan",
  "fa": "گزارش",
  "pa": "ਰਿਪੋਰਟ ਕਰੋ",
  "uk": "Повідомити",
  "ro": "Raportează",
  "tl": "I-report"
},

"feedback":{
  "en": "Feedback",
  "fr": "Commentaires",
  "de": "Feedback",
  "ar": "ملاحظات",
  "es": "Comentarios",
  "tr": "Geri bildirim",
  "nl": "Feedback",
  "it": "Feedback",
  "ja": "フィードバック",
  "zh": "反馈",
  "ko": "피드백",
  "hi": "प्रतिक्रिया",
  "pt": "Feedback",
  "ru": "Обратная связь",
  "sw": "Maoni",
  "pl": "Opinie",
  "id": "Masukan",
  "fa": "بازخورد",
  "pa": "ਫੀਡਬੈਕ",
  "uk": "Зворотний зв’язок",
  "ro": "Feedback",
  "tl": "Feedback"
},

"EntercurrentPass": {
    "en": "Enter current password",
    "fr": "Entrez le mot de passe actuel",
    "de": "Aktuelles Passwort eingeben",
    "ar": "أدخل كلمة المرور الحالية",
    "es": "Ingrese la contraseña actual",
    "tr": "Mevcut şifreyi girin",
    "nl": "Voer huidig wachtwoord in",
    "it": "Inserisci la password attuale",
    "ja": "現在のパスワードを入力",
    "zh": "输入当前密码",
    "ko": "현재 비밀번호 입력",
    "hi": "वर्तमान पासवर्ड दर्ज करें",
    "pt": "Digite a senha atual",
    "ru": "Введите текущий пароль",
    "sw": "Weka nenosiri la sasa",
    "pl": "Wprowadź obecne hasło",
    "id": "Masukkan kata sandi saat ini",
    "fa": "رمز عبور فعلی را وارد کنید",
    "pa": "ਮੌਜੂਦਾ ਪਾਸਵਰਡ ਦਰਜ ਕਰੋ",
    "uk": "Введіть поточний пароль",
    "ro": "Introduceți parola curentă",
    "tl": "Ilagay ang kasalukuyang password"
  },

  "EnternewPass": {
    "en": "Enter new password",
    "fr": "Entrez le nouveau mot de passe",
    "de": "Neues Passwort eingeben",
    "ar": "أدخل كلمة المرور الجديدة",
    "es": "Ingrese la nueva contraseña",
    "tr": "Yeni şifreyi girin",
    "nl": "Voer nieuw wachtwoord in",
    "it": "Inserisci la nuova password",
    "ja": "新しいパスワードを入力",
    "zh": "输入新密码",
    "ko": "새 비밀번호 입력",
    "hi": "नया पासवर्ड दर्ज करें",
    "pt": "Digite a nova senha",
    "ru": "Введите новый пароль",
    "sw": "Weka nenosiri jipya",
    "pl": "Wprowadź nowe hasło",
    "id": "Masukkan kata sandi baru",
    "fa": "رمز عبور جدید را وارد کنید",
    "pa": "ਨਵਾਂ ਪਾਸਵਰਡ ਦਰਜ ਕਰੋ",
    "uk": "Введіть новий пароль",
    "ro": "Introduceți parola nouă",
    "tl": "Ilagay ang bagong password"
  },

  "ConfirmPass": {
    "en": "Confirm password",
    "fr": "Confirmez le mot de passe",
    "de": "Passwort bestätigen",
    "ar": "تأكيد كلمة المرور",
    "es": "Confirmar contraseña",
    "tr": "Şifreyi onayla",
    "nl": "Bevestig wachtwoord",
    "it": "Conferma password",
    "ja": "パスワードを確認",
    "zh": "确认密码",
    "ko": "비밀번호 확인",
    "hi": "पासवर्ड की पुष्टि करें",
    "pt": "Confirmar senha",
    "ru": "Подтвердите пароль",
    "sw": "Thibitisha nenosiri",
    "pl": "Potwierdź hasło",
    "id": "Konfirmasi kata sandi",
    "fa": "تأیید رمز عبور",
    "pa": "ਪਾਸਵਰਡ ਦੀ ਪੁਸ਼ਟੀ ਕਰੋ",
    "uk": "Підтвердьте пароль",
    "ro": "Confirmă parola",
    "tl": "Kumpirmahin ang password"
  },

  "ChangePass": {
    "en": "Change Password",
    "fr": "Changer le mot de passe",
    "de": "Passwort ändern",
    "ar": "تغيير كلمة المرور",
    "es": "Cambiar contraseña",
    "tr": "Şifre değiştir",
    "nl": "Wachtwoord wijzigen",
    "it": "Cambia password",
    "ja": "パスワードを変更",
    "zh": "更改密码",
    "ko": "비밀번호 변경",
    "hi": "पासवर्ड बदलें",
    "pt": "Alterar senha",
    "ru": "Изменить пароль",
    "sw": "Badilisha nenosiri",
    "pl": "Zmień hasło",
    "id": "Ubah kata sandi",
    "fa": "تغییر رمز عبور",
    "pa": "ਪਾਸਵਰਡ ਬਦਲੋ",
    "uk": "Змінити пароль",
    "ro": "Schimbă parola",
    "tl": "Palitan ang password"
  },



  "pleaseClose": {
    "en": "Please confirm your request for account closure.",
    "fr": "Veuillez confirmer votre demande de suppression de compte.",
    "de": "Bitte bestätigen Sie Ihre Anfrage zur Kontoschließung.",
    "ar": "يرجى تأكيد طلبك لإغلاق الحساب.",
    "es": "Por favor confirme su solicitud de cierre de cuenta.",
    "tr": "Lütfen hesap kapatma talebinizi onaylayın.",
    "nl": "Bevestig alstublieft uw verzoek tot accountverwijdering.",
    "it": "Conferma la richiesta di chiusura dell'account.",
    "ja": "アカウント削除のリクエストを確認してください。",
    "zh": "请确认您的账户关闭请求。",
    "ko": "계정 삭제 요청을 확인해 주세요.",
    "hi": "कृपया अपने खाते को बंद करने के अनुरोध की पुष्टि करें।",
    "pt": "Confirme sua solicitação de encerramento de conta.",
    "ru": "Пожалуйста, подтвердите запрос на закрытие аккаунта.",
    "sw": "Tafadhali thibitisha ombi lako la kufunga akaunti.",
    "pl": "Proszę potwierdzić żądanie zamknięcia konta.",
    "id": "Harap konfirmasi permintaan penutupan akun Anda.",
    "fa": "لطفاً درخواست بستن حساب خود را تأیید کنید.",
    "pa": "ਕਿਰਪਾ ਕਰਕੇ ਆਪਣੇ ਖਾਤਾ ਬੰਦ ਕਰਨ ਦੀ ਬੇਨਤੀ ਦੀ ਪੁਸ਼ਟੀ ਕਰੋ।",
    "uk": "Будь ласка, підтвердьте запит на закриття облікового запису.",
    "ro": "Vă rugăm să confirmați solicitarea de închidere a contului.",
    "tl": "Pakikumpirma ang iyong kahilingan sa pagsasara ng account."
  },

  "ByAuthPermt": {
    "en": "By clicking Proceed, you authorize the permanent removal of all your associated data and records with NEWSWORLD.",
    "fr": "En cliquant sur Continuer, vous autorisez la suppression permanente de toutes vos données associées à NEWSWORLD.",
    "de": "Durch Klicken auf Weiter autorisieren Sie die dauerhafte Löschung aller mit NEWSWORLD verbundenen Daten.",
    "ar": "بالنقر على متابعة، فإنك تفوض الحذف الدائم لجميع بياناتك المرتبطة بـ NEWSWORLD.",
    "es": "Al hacer clic en Continuar, autoriza la eliminación permanente de todos sus datos asociados con NEWSWORLD.",
    "tr": "Devam'a tıklayarak, NEWSWORLD ile ilişkili tüm verilerinizin kalıcı olarak silinmesini onaylamış olursunuz.",
    "nl": "Door op Doorgaan te klikken, autoriseert u de permanente verwijdering van al uw gegevens die aan NEWSWORLD zijn gekoppeld.",
    "it": "Facendo clic su Procedi, autorizzi la rimozione permanente di tutti i tuoi dati associati a NEWSWORLD.",
    "ja": "「続行」をクリックすると、NEWSWORLDに関連するすべてのデータが永久に削除されることに同意したものとみなされます。",
    "zh": "点击继续即表示您授权永久删除与 NEWSWORLD 相关的所有数据和记录。",
    "ko": "계속을 클릭하면 NEWSWORLD와 관련된 모든 데이터가 영구적으로 삭제되는 것에 동의하게 됩니다.",
    "hi": "प्रोसीड पर क्लिक करके, आप NEWSWORLD से संबंधित अपने सभी डेटा को स्थायी रूप से हटाने की अनुमति देते हैं।",
    "pt": "Ao clicar em Prosseguir, você autoriza a remoção permanente de todos os seus dados associados à NEWSWORLD.",
    "ru": "Нажимая «Продолжить», вы разрешаете безвозвратное удаление всех данных, связанных с NEWSWORLD.",
    "sw": "Kwa kubofya Endelea, unaidhinisha kuondolewa kabisa kwa data zako zote zinazohusiana na NEWSWORLD.",
    "pl": "Klikając Kontynuuj, wyrażasz zgodę na trwałe usunięcie wszystkich danych powiązanych z NEWSWORLD.",
    "id": "Dengan mengklik Lanjutkan, Anda menyetujui penghapusan permanen semua data yang terkait dengan NEWSWORLD.",
    "fa": "با کلیک روی ادامه، شما مجوز حذف دائمی تمامی داده‌های مرتبط با NEWSWORLD را صادر می‌کنید.",
    "pa": "Proceed 'ਤੇ ਕਲਿੱਕ ਕਰਨ ਨਾਲ ਤੁਸੀਂ NEWSWORLD ਨਾਲ ਸੰਬੰਧਿਤ ਆਪਣੇ ਸਾਰੇ ਡਾਟਾ ਨੂੰ ਸਥਾਈ ਤੌਰ 'ਤੇ ਹਟਾਉਣ ਦੀ ਇਜਾਜ਼ਤ ਦਿੰਦੇ ਹੋ।",
    "uk": "Натискаючи «Продовжити», ви надаєте дозвіл на безповоротне видалення всіх даних, пов’язаних з NEWSWORLD.",
    "ro": "Prin apăsarea pe Continuă, autorizați ștergerea permanentă a tuturor datelor asociate cu NEWSWORLD.",
    "tl": "Sa pag-click ng Magpatuloy, pinapahintulutan mo ang permanenteng pag-alis ng lahat ng iyong kaugnay na datos sa NEWSWORLD."
  },

  "Cancel": {
    "en": "Cancel",
    "fr": "Annuler",
    "de": "Abbrechen",
    "ar": "إلغاء",
    "es": "Cancelar",
    "tr": "İptal",
    "nl": "Annuleren",
    "it": "Annulla",
    "ja": "キャンセル",
    "zh": "取消",
    "ko": "취소",
    "hi": "रद्द करें",
    "pt": "Cancelar",
    "ru": "Отмена",
    "sw": "Ghairi",
    "pl": "Anuluj",
    "id": "Batal",
    "fa": "لغو",
    "pa": "ਰੱਦ ਕਰੋ",
    "uk": "Скасувати",
    "ro": "Anulează",
    "tl": "Kanselahin"
  },

  "Proceed": {
    "en": "Proceed",
    "fr": "Continuer",
    "de": "Weiter",
    "ar": "متابعة",
    "es": "Continuar",
    "tr": "Devam",
    "nl": "Doorgaan",
    "it": "Procedi",
    "ja": "続行",
    "zh": "继续",
    "ko": "계속",
    "hi": "आगे बढ़ें",
    "pt": "Prosseguir",
    "ru": "Продолжить",
    "sw": "Endelea",
    "pl": "Kontynuuj",
    "id": "Lanjutkan",
    "fa": "ادامه",
    "pa": "ਜਾਰੀ ਰੱਖੋ",
    "uk": "Продовжити",
    "ro": "Continuă",
    "tl": "Magpatuloy"
  },

  "deleteAccount": {
    "en": "Delete Account",
    "fr": "Supprimer le compte",
    "de": "Konto löschen",
    "ar": "حذف الحساب",
    "es": "Eliminar cuenta",
    "tr": "Hesabı sil",
    "nl": "Account verwijderen",
    "it": "Elimina account",
    "ja": "アカウントを削除",
    "zh": "删除账户",
    "ko": "계정 삭제",
    "hi": "खाता हटाएँ",
    "pt": "Excluir conta",
    "ru": "Удалить аккаунт",
    "sw": "Futa akaunti",
    "pl": "Usuń konto",
    "id": "Hapus akun",
    "fa": "حذف حساب",
    "pa": "ਖਾਤਾ ਮਿਟਾਓ",
    "uk": "Видалити акаунт",
    "ro": "Șterge contul",
    "tl": "I-delete ang account"
  },

  "ConfirmDelete": {
    "en": "Confirm Delete",
    "fr": "Confirmer la suppression",
    "de": "Löschen bestätigen",
    "ar": "تأكيد الحذف",
    "es": "Confirmar eliminación",
    "tr": "Silmeyi onayla",
    "nl": "Verwijderen bevestigen",
    "it": "Conferma eliminazione",
    "ja": "削除を確認",
    "zh": "确认删除",
    "ko": "삭제 확인",
    "hi": "हटाने की पुष्टि करें",
    "pt": "Confirmar exclusão",
    "ru": "Подтвердить удаление",
    "sw": "Thibitisha kufuta",
    "pl": "Potwierdź usunięcie",
    "id": "Konfirmasi penghapusan",
    "fa": "تأیید حذف",
    "pa": "ਹਟਾਉਣ ਦੀ ਪੁਸ਼ਟੀ ਕਰੋ",
    "uk": "Підтвердити видалення",
    "ro": "Confirmă ștergerea",
    "tl": "Kumpirmahin ang pag-delete"
  },

  "ConfirmDelAcc": {
    "en": "Confirm password to delete account",
    "fr": "Confirmez le mot de passe pour supprimer le compte",
    "de": "Passwort bestätigen, um das Konto zu löschen",
    "ar": "أكد كلمة المرور لحذف الحساب",
    "es": "Confirme la contraseña para eliminar la cuenta",
    "tr": "Hesabı silmek için şifreyi onaylayın",
    "nl": "Bevestig wachtwoord om account te verwijderen",
    "it": "Conferma la password per eliminare l'account",
    "ja": "アカウント削除のためパスワードを確認",
    "zh": "确认密码以删除账户",
    "ko": "계정 삭제를 위해 비밀번호 확인",
    "hi": "खाता हटाने के लिए पासवर्ड की पुष्टि करें",
    "pt": "Confirme a senha para excluir a conta",
    "ru": "Подтвердите пароль для удаления аккаунта",
    "sw": "Thibitisha nenosiri ili kufuta akaunti",
    "pl": "Potwierdź hasło, aby usunąć konto",
    "id": "Konfirmasi kata sandi untuk menghapus akun",
    "fa": "برای حذف حساب، رمز عبور را تأیید کنید",
    "pa": "ਖਾਤਾ ਮਿਟਾਉਣ ਲਈ ਪਾਸਵਰਡ ਦੀ ਪੁਸ਼ਟੀ ਕਰੋ",
    "uk": "Підтвердьте пароль для видалення акаунту",
    "ro": "Confirmă parola pentru a șterge contul",
    "tl": "Kumpirmahin ang password upang i-delete ang account"
  },

  "appActivity":{
  "en": "App Activity",
  "fr": "Activité de l'application",
  "de": "App-Aktivität",
  "ar": "نشاط التطبيق",
  "es": "Actividad de la aplicación",
  "tr": "Uygulama etkinliği",
  "nl": "App-activiteit",
  "it": "Attività dell'app",
  "ja": "アプリのアクティビティ",
  "zh": "应用活动",
  "ko": "앱 활동",
  "hi": "ऐप गतिविधि",
  "pt": "Atividade do aplicativo",
  "ru": "Активность приложения",
  "sw": "Shughuli za programu",
  "pl": "Aktywność aplikacji",
  "id": "Aktivitas aplikasi",
  "fa": "فعالیت برنامه",
  "pa": "ਐਪ ਗਤੀਵਿਧੀ",
  "uk": "Активність додатку",
  "ro": "Activitate aplicație",
  "tl": "Aktibidad ng app"
  },


  "language":{
    "en": "Language",
  "fr": "Langue",
  "de": "Sprache",
  "ar": "اللغة",
  "es": "Idioma",
  "tr": "Dil",
  "nl": "Taal",
  "it": "Lingua",
  "ja": "言語",
  "zh": "语言",
  "ko": "언어",
  "hi": "भाषा",
  "pt": "Idioma",
  "ru": "Язык",
  "sw": "Lugha",
  "pl": "Język",
  "id": "Bahasa",
  "fa": "زبان",
  "pa": "ਭਾਸ਼ਾ",
  "uk": "Мова",
  "ro": "Limbă",
  "tl": "Wika"
  },

  "createAccStart":{
    "en": "Create your account to get started",
  "fr": "Créez votre compte pour commencer",
  "de": "Erstellen Sie Ihr Konto, um zu beginnen",
  "ar": "أنشئ حسابك للبدء",
  "es": "Crea tu cuenta para comenzar",
  "tr": "Başlamak için hesabınızı oluşturun",
  "nl": "Maak je account aan om te beginnen",
  "it": "Crea il tuo account per iniziare",
  "ja": "開始するにはアカウントを作成してください",
  "zh": "创建您的账户以开始使用",
  "ko": "시작하려면 계정을 생성하세요",
  "hi": "शुरू करने के लिए अपना खाता बनाएं",
  "pt": "Crie sua conta para começar",
  "ru": "Создайте аккаунт, чтобы начать",
  "sw": "Unda akaunti yako ili kuanza",
  "pl": "Utwórz konto, aby rozpocząć",
  "id": "Buat akun Anda untuk memulai",
  "fa": "برای شروع حساب خود را ایجاد کنید",
  "pa": "ਸ਼ੁਰੂ ਕਰਨ ਲਈ ਆਪਣਾ ਖਾਤਾ ਬਣਾਓ",
  "uk": "Створіть обліковий запис, щоб розпочати",
  "ro": "Creează-ți contul pentru a începe",
  "tl": "Gumawa ng iyong account upang makapagsimula"
  },


  "Entercode":{
    "en": "Enter the code sent to your email",
  "fr": "Entrez le code envoyé à votre e-mail",
  "de": "Geben Sie den an Ihre E-Mail gesendeten Code ein",
  "ar": "أدخل الرمز المرسل إلى بريدك الإلكتروني",
  "es": "Ingrese el código enviado a su correo electrónico",
  "tr": "E-postanıza gönderilen kodu girin",
  "nl": "Voer de code in die naar uw e-mail is verzonden",
  "it": "Inserisci il codice inviato alla tua e-mail",
  "ja": "メールに送信されたコードを入力してください",
  "zh": "输入发送到您邮箱的验证码",
  "ko": "이메일로 전송된 코드를 입력하세요",
  "hi": "अपने ईमेल पर भेजा गया कोड दर्ज करें",
  "pt": "Digite o código enviado para seu e-mail",
  "ru": "Введите код, отправленный на вашу электронную почту",
  "sw": "Weka msimbo uliotumwa kwa barua pepe yako",
  "pl": "Wprowadź kod wysłany na Twój e-mail",
  "id": "Masukkan kode yang dikirim ke email Anda",
  "fa": "کد ارسال‌شده به ایمیل خود را وارد کنید",
  "pa": "ਆਪਣੇ ਈਮੇਲ ਤੇ ਭੇਜੇ ਗਏ ਕੋਡ ਨੂੰ ਦਰਜ ਕਰੋ",
  "uk": "Введіть код, надісланий на вашу електронну пошту",
  "ro": "Introduceți codul trimis pe e-mail",
  "tl": "Ilagay ang code na ipinadala sa iyong email"
},



"fillinDetail":{
"en": "Fill in your profile details",
  "fr": "Remplissez les détails de votre profil",
  "de": "Füllen Sie Ihre Profildaten aus",
  "ar": "املأ تفاصيل ملفك الشخصي",
  "es": "Complete los detalles de su perfil",
  "tr": "Profil bilgilerinizi doldurun",
  "nl": "Vul uw profielgegevens in",
  "it": "Compila i dettagli del tuo profilo",
  "ja": "プロフィールの詳細を入力してください",
  "zh": "填写您的个人资料信息",
  "ko": "프로필 정보를 입력하세요",
  "hi": "अपनी प्रोफ़ाइल विवरण भरें",
  "pt": "Preencha os detalhes do seu perfil",
  "ru": "Заполните данные профиля",
  "sw": "Jaza maelezo ya wasifu wako",
  "pl": "Uzupełnij dane profilu",
  "id": "Isi detail profil Anda",
  "fa": "جزئیات پروفایل خود را تکمیل کنید",
  "pa": "ਆਪਣੀ ਪ੍ਰੋਫਾਈਲ ਜਾਣਕਾਰੀ ਭਰੋ",
  "uk": "Заповніть дані профілю",
  "ro": "Completați detaliile profilului",
  "tl": "Punan ang detalye ng iyong profile"
},



"emptyCountry":{
  "en": "We’re preparing news for {label}.",
  "fr": "Nous préparons des actualités pour {label}.",
  "de": "Wir bereiten Nachrichten für {label} vor.",
  "ar": "نحن نجهز الأخبار لـ {label}.",
  "es": "Estamos preparando noticias para {label}.",
  "tr": "{label} için haberler hazırlanıyor.",
  "nl": "We bereiden nieuws voor {label} voor.",
  "it": "Stiamo preparando notizie per {label}.",
  "ja": "{label} のニュースを準備しています。",
  "zh": "我们正在为 {label} 准备新闻。",
  "ko": "{label} 관련 뉴스를 준비하고 있습니다.",
  "hi": "हम {label} के लिए समाचार तैयार कर रहे हैं।",
  "pt": "Estamos preparando notícias para {label}.",
  "ru": "Мы готовим новости для {label}.",
  "sw": "Tunaandaa habari za {label}.",
  "pl": "Przygotowujemy wiadomości dla {label}.",
  "id": "Kami sedang menyiapkan berita untuk {label}.",
  "fa": "در حال آماده‌سازی اخبار برای {label} هستیم.",
  "pa": "ਅਸੀਂ {label} ਲਈ ਖਬਰਾਂ ਤਿਆਰ ਕਰ ਰਹੇ ਹਾਂ।",
  "uk": "Ми готуємо новини для {label}.",
  "ro": "Pregătim știri pentru {label}.",
  "tl": "Inihahanda namin ang balita para sa {label}."
},


"emptyCategory":{
"en": "We’re gathering the latest {label} news from {country}",
  "fr": "Nous rassemblons les dernières actualités {label} de {country}",
  "de": "Wir sammeln die neuesten {label}-Nachrichten aus {country}",
  "ar": "نقوم بجمع أحدث أخبار {label} من {country}.",
  "es": "Estamos recopilando las últimas noticias de {label} desde {country}",
  "tr": "{country}’den en son {label} haberlerini topluyoruz",
  "nl": "We verzamelen het laatste {label}-nieuws uit {country}",
  "it": "Stiamo raccogliendo le ultime notizie su {label} da {country}",
  "ja": "{country} からの最新の {label} ニュースを収集中です。",
  "zh": "我们正在收集来自 {country} 的最新 {label} 新闻。",
  "ko": "{country}의 최신 {label} 뉴스를 수집하고 있습니다",
  "hi": "हम {country} से {label} की नवीनतम खबरें एकत्र कर रहे हैं।",
  "pt": "Estamos reunindo as últimas notícias de {label} de {country}",
  "ru": "Мы собираем последние новости о {label} из {country}",
  "sw": "Tunakusanya habari za hivi karibuni za {label} kutoka {country}",
  "pl": "Zbieramy najnowsze wiadomości {label} z {country}",
  "id": "Kami sedang mengumpulkan berita terbaru tentang {label} dari {country}",
  "fa": "در حال جمع‌آوری جدیدترین اخبار {label} از {country} هستیم",
  "pa": "ਅਸੀਂ {country} ਤੋਂ {label} ਦੀਆਂ ਤਾਜ਼ਾ ਖਬਰਾਂ ਇਕੱਠੀਆਂ ਕਰ ਰਹੇ ਹਾਂ।",
  "uk": "Ми збираємо останні новини про {label} з {country}",
  "ro": "Colectăm cele mai recente știri despre {label} din {country}",
  "tl": "Kinokolekta namin ang pinakabagong balita tungkol sa {label} mula sa {country}"
},

"checkBack":{
  "en": "Please check back shortly.",
  "fr": "Veuillez revenir bientôt.",
  "de": "Bitte schauen Sie bald wieder vorbei.",
  "ar": "يرجى التحقق مرة أخرى قريبًا.",
  "es": "Por favor, vuelva a consultar en breve.",
  "tr": "Lütfen kısa süre sonra tekrar kontrol edin.",
  "nl": "Kom binnenkort terug om opnieuw te kijken.",
  "it": "Torna a controllare a breve.",
  "ja": "しばらくしてからもう一度ご確認ください。",
  "zh": "请稍后再查看。",
  "ko": "잠시 후 다시 확인해 주세요.",
  "hi": "कृपया थोड़ी देर बाद फिर से जांचें।",
  "pt": "Por favor, volte em breve.",
  "ru": "Пожалуйста, зайдите позже.",
  "sw": "Tafadhali angalia tena muda mfupi baadaye.",
  "pl": "Proszę sprawdzić ponownie wkrótce.",
  "id": "Silakan periksa kembali sebentar lagi.",
  "fa": "لطفاً کمی بعد دوباره بررسی کنید.",
  "pa": "ਕਿਰਪਾ ਕਰਕੇ ਥੋੜ੍ਹੀ ਦੇਰ ਬਾਅਦ ਮੁੜ ਜਾਂਚ ਕਰੋ।",
  "uk": "Будь ласка, перевірте трохи пізніше.",
  "ro": "Vă rugăm să verificați din nou în curând.",
  "tl": "Pakibalik muli sa ilang sandali."
},

"noTrending":{
  "en": "No trending news available for {label}",
  "fr": "Aucune actualité tendance disponible pour {label}",
  "de": "Keine trendigen Nachrichten für {label} verfügbar",
  "ar": "لا توجد أخبار رائجة متاحة لـ {label}",
  "es": "No hay noticias de tendencia disponibles para {label}",
  "tr": "{label} için trend haber bulunmuyor",
  "nl": "Geen trending nieuws beschikbaar voor {label}",
  "it": "Nessuna notizia di tendenza disponibile per {label}",
  "ja": "{label} のトレンドニュースはまだありません",
  "zh": "{label} 暂无热门新闻",
  "ko": "{label}에 대한 인기 뉴스가 없습니다",
  "hi": "{label} के लिए कोई ट्रेंडिंग समाचार उपलब्ध नहीं है",
  "pt": "Nenhuma notícia em destaque disponível para {label}",
  "ru": "Нет популярных новостей для {label}",
  "sw": "Hakuna habari zinazovuma kwa {label}",
  "pl": "Brak popularnych wiadomości dla {label}",
  "id": "Tidak ada berita tren untuk {label}",
  "fa": "هیچ خبر داغی برای {label} موجود نیست",
  "pa": "{label} ਲਈ ਕੋਈ ਟ੍ਰੈਂਡਿੰਗ ਖਬਰ ਉਪਲਬਧ ਨਹੀਂ ਹੈ",
  "uk": "Немає популярних новин для {label}",
  "ro": "Nu există știri populare pentru {label}",
  "tl": "Walang trending na balita para sa {label}"
},

"channels":{
  "en": "CHANNELS",
  "fr": "CHAÎNES",
  "de": "KANÄLE",
  "ar": "القنوات",
  "es": "CANALES",
  "tr": "KANALLAR",
  "nl": "KANALEN",
  "it": "CANALI",
  "ja": "チャンネル",
  "zh": "频道",
  "ko": "채널",
  "hi": "चैनल",
  "pt": "CANAIS",
  "ru": "КАНАЛЫ",
  "sw": "VITUO",
  "pl": "KANAŁY",
  "id": "SALURAN",
  "fa": "کانال‌ها",
  "pa": "ਚੈਨਲ",
  "uk": "КАНАЛИ",
  "ro": "CANALE",
  "tl": "MGA CHANNEL"
},

"Activity":{
  "en": "Activity",
  "fr": "Activité",
  "de": "Aktivität",
  "ar": "النشاط",
  "es": "Actividad",
  "tr": "Etkinlik",
  "nl": "Activiteit",
  "it": "Attività",
  "ja": "アクティビティ",
  "zh": "活动",
  "ko": "활동",
  "hi": "गतिविधि",
  "pt": "Atividade",
  "ru": "Активность",
  "sw": "Shughuli",
  "pl": "Aktywność",
  "id": "Aktivitas",
  "fa": "فعالیت",
  "pa": "ਗਤੀਵਿਧੀ",
  "uk": "Активність",
  "ro": "Activitate",
  "tl": "Aktibidad"
},

"ReadAll":{
  "en": "Read All",
  "fr": "Tout lire",
  "de": "Alle lesen",
  "ar": "قراءة الكل",
  "es": "Leer todo",
  "tr": "Hepsini oku",
  "nl": "Alles lezen",
  "it": "Leggi tutto",
  "ja": "すべて読む",
  "zh": "阅读全部",
  "ko": "모두 읽기",
  "hi": "सभी पढ़ें",
  "pt": "Ler tudo",
  "ru": "Прочитать всё",
  "sw": "Soma zote",
  "pl": "Przeczytaj wszystko",
  "id": "Baca semua",
  "fa": "خواندن همه",
  "pa": "ਸਭ ਪੜ੍ਹੋ",
  "uk": "Прочитати все",
  "ro": "Citește tot",
  "tl": "Basahin lahat"
},

"replyComment":{
"en": "{label} replied your comment",
"fr": "{label} a répondu à votre commentaire",
"de": "{label} hat auf Ihren Kommentar geantwortet",
"ar": "{label} رد على تعليقك",
"es": "{label} respondió a tu comentario",
"tr": "{label} yorumunuza yanıt verdi",
"nl": "{label} heeft op je reactie gereageerd",
"it": "{label} ha risposto al tuo commento",
"ja": "{label} があなたのコメントに返信しました",
"zh": "{label} 回复了你的评论",
"ko": "{label} 님이 회원님의 댓글에 답글을 남겼습니다",
"hi": "{label} ने आपकी टिप्पणी का जवाब दिया",
"pt": "{label} respondeu ao seu comentário",
"ru": "{label} ответил на ваш комментарий",
"sw": "{label} alijibu maoni yako",
"pl": "{label} odpowiedział na twój komentarz",
"id": "{label} membalas komentar Anda",
"fa": "{label} به نظر شما پاسخ داد",
"pa": "{label} ਨੇ ਤੁਹਾਡੀ ਟਿੱਪਣੀ ਦਾ ਜਵਾਬ ਦਿੱਤਾ",
"uk": "{label} відповів на ваш коментар",
"ro": "{label} a răspuns la comentariul tău",
"tl": "Sinagot ni {label} ang iyong komento"
},



"likeComment":{
"en": "{label} liked your comment",
"fr": "{label} a aimé votre commentaire",
"de": "{label} hat Ihren Kommentar geliked",
"ar": "{label} أعجب بتعليقك",
"es": "A {label} le gustó tu comentario",
"tr": "{label} yorumunuzu beğendi",
"nl": "{label} vond je reactie leuk",
"it": "A {label} è piaciuto il tuo commento",
"ja": "{label} があなたのコメントをいいねしました",
"zh": "{label} 点赞了你的评论",
"ko": "{label} 님이 회원님의 댓글을 좋아합니다",
"hi": "{label} को आपकी टिप्पणी पसंद आई",
"pt": "{label} curtiu seu comentário",
"ru": "{label} понравился ваш комментарий",
"sw": "{label} alipenda maoni yako",
"pl": "{label} polubił twój komentarz",
"id": "{label} menyukai komentar Anda",
"fa": "{label} نظر شما را پسندید",
"pa": "{label} ਨੂੰ ਤੁਹਾਡੀ ਟਿੱਪਣੀ ਪਸੰਦ ਆਈ",
"uk": "{label} вподобав ваш коментар",
"ro": "{label} a apreciat comentariul tău",
"tl": "Nagustuhan ni {label} ang iyong komento"
},


"newsUpdate":{
"en": "News Update",
"fr": "Mise à jour des actualités",
"de": "Nachrichten-Update",
"ar": "تحديث الأخبار",
"es": "Actualización de noticias",
"tr": "Haber Güncellemesi",
"nl": "Nieuwsupdate",
"it": "Aggiornamento notizie",
"ja": "ニュース更新",
"zh": "新闻更新",
"ko": "뉴스 업데이트",
"hi": "समाचार अपडेट",
"pt": "Atualização de notícias",
"ru": "Обновление новостей",
"sw": "Sasisho la Habari",
"pl": "Aktualizacja wiadomości",
"id": "Pembaruan Berita",
"fa": "به‌روزرسانی اخبار",
"pa": "ਖਬਰਾਂ ਅਪਡੇਟ",
"uk": "Оновлення новин",
"ro": "Actualizare știri",
"tl": "Update ng Balita"
},


"liveNow":{
"en": "Live Now",
"fr": "En direct",
"de": "Live jetzt",
"ar": "مباشر الآن",
"es": "En vivo ahora",
"tr": "Şimdi canlı",
"nl": "Nu live",
"it": "In diretta ora",
"ja": "現在ライブ",
"zh": "正在直播",
"ko": "지금 라이브",
"hi": "अभी लाइव",
"pt": "Ao vivo agora",
"ru": "В прямом эфире",
"sw": "Moja kwa moja sasa",
"pl": "Na żywo teraz",
"id": "Sedang live",
"fa": "اکنون زنده",
"pa": "ਹੁਣ ਲਾਈਵ",
"uk": "Наживо зараз",
"ro": "Live acum",
"tl": "Live ngayon"
},



"update":{
  "en": "Update",
  "fr": "Mise à jour",
  "de": "Aktualisierung",
  "ar": "تحديث",
  "es": "Actualización",
  "tr": "Güncelleme",
  "nl": "Update",
  "it": "Aggiornamento",
  "ja": "更新",
  "zh": "更新",
  "ko": "업데이트",
  "hi": "अपडेट",
  "pt": "Atualização",
  "ru": "Обновление",
  "sw": "Sasisho",
  "pl": "Aktualizacja",
  "id": "Pembaruan",
  "fa": "به‌روزرسانی",
  "pa": "ਅਪਡੇਟ",
  "uk": "Оновлення",
  "ro": "Actualizare",
  "tl": "Update"
},

"noYet":{
  "en": "No {label} yet",
  "fr": "Aucun {label} pour le moment",
  "de": "Noch keine {label}",
  "ar": "لا يوجد {label} حتى الآن",
  "es": "Aún no hay {label}",
  "tr": "Henüz {label} yok",
  "nl": "Nog geen {label}",
  "it": "Nessun {label} ancora",
  "ja": "まだ{label}はありません",
  "zh": "还没有{label}",
  "ko": "아직 {label} 없음",
  "hi": "अभी तक कोई {label} नहीं",
  "pt": "Ainda não há {label}",
  "ru": "Пока нет {label}",
  "sw": "Bado hakuna {label}",
  "pl": "Jeszcze brak {label}",
  "id": "Belum ada {label}",
  "fa": "هنوز {label} وجود ندارد",
  "pa": "ਹਾਲੇ ਤੱਕ ਕੋਈ {label} ਨਹੀਂ",
  "uk": "Поки що немає {label}",
  "ro": "Încă nu există {label}",
  "tl": "Wala pang {label}"
},


"noNotify":{
  "en": "No Notification yet",
  "fr": "Aucune notification pour le moment",
  "de": "Noch keine Benachrichtigungen",
  "ar": "لا توجد إشعارات حتى الآن",
  "es": "Aún no hay notificaciones",
  "tr": "Henüz bildirim yok",
  "nl": "Nog geen meldingen",
  "it": "Nessuna notifica ancora",
  "ja": "まだ通知はありません",
  "zh": "还没有通知",
  "ko": "아직 알림이 없습니다",
  "hi": "अभी तक कोई सूचना नहीं",
  "pt": "Ainda não há notificações",
  "ru": "Пока нет уведомлений",
  "sw": "Bado hakuna arifa",
  "pl": "Brak powiadomień",
  "id": "Belum ada notifikasi",
  "fa": "هنوز اعلانی وجود ندارد",
  "pa": "ਹਾਲੇ ਕੋਈ ਸੂਚਨਾ ਨਹੀਂ",
  "uk": "Поки що немає сповіщень",
  "ro": "Încă nu există notificări",
  "tl": "Wala pang mga notipikasyon"
},


"noInternet":{
  "en": "No Internet Connection",
  "fr": "Pas de connexion Internet",
  "de": "Keine Internetverbindung",
  "ar": "لا يوجد اتصال بالإنترنت",
  "es": "Sin conexión a Internet",
  "tr": "İnternet bağlantısı yok",
  "nl": "Geen internetverbinding",
  "it": "Nessuna connessione Internet",
  "ja": "インターネット接続がありません",
  "zh": "没有互联网连接",
  "ko": "인터넷 연결이 없습니다",
  "hi": "कोई इंटरनेट कनेक्शन नहीं",
  "pt": "Sem conexão com a Internet",
  "ru": "Нет подключения к Интернету",
  "sw": "Hakuna muunganisho wa intaneti",
  "pl": "Brak połączenia z Internetem",
  "id": "Tidak ada koneksi internet",
  "fa": "اتصال به اینترنت وجود ندارد",
  "pa": "ਇੰਟਰਨੈੱਟ ਕਨੈਕਸ਼ਨ ਨਹੀਂ ਹੈ",
  "uk": "Немає підключення до Інтернету",
  "ro": "Fără conexiune la internet",
  "tl": "Walang koneksyon sa internet"
},

"global":{
  "en": "Global",
  "fr": "Mondial",
  "de": "Global",
  "ar": "عالمي",
  "es": "Global",
  "tr": "Küresel",
  "nl": "Wereldwijd",
  "it": "Globale",
  "ja": "グローバル",
  "zh": "全球",
  "ko": "글로벌",
  "hi": "वैश्विक",
  "pt": "Global",
  "ru": "Глобальный",
  "sw": "Kimataifa",
  "pl": "Globalny",
  "id": "Global",
  "fa": "جهانی",
  "pa": "ਵਿਸ਼ਵ ਪੱਧਰੀ",
  "uk": "Глобальний",
  "ro": "Global",
  "tl": "Global"
},


"passwordUpdated":{
  "en": "Password Updated!",
  "fr": "Mot de passe mis à jour !",
  "de": "Passwort aktualisiert!",
  "ar": "تم تحديث كلمة المرور!",
  "es": "¡Contraseña actualizada!",
  "tr": "Şifre güncellendi!",
  "nl": "Wachtwoord bijgewerkt!",
  "it": "Password aggiornato!",
  "ja": "パスワードが更新されました！",
  "zh": "密码已更新！",
  "ko": "비밀번호가 업데이트되었습니다!",
  "hi": "पासवर्ड अपडेट हो गया!",
  "pt": "Senha atualizada!",
  "ru": "Пароль обновлён!",
  "sw": "Nenosiri limesasishwa!",
  "pl": "Hasło zaktualizowane!",
  "id": "Kata sandi diperbarui!",
  "fa": "رمز عبور به‌روزرسانی شد!",
  "pa": "ਪਾਸਵਰਡ ਅਪਡੇਟ ਹੋ ਗਿਆ!",
  "uk": "Пароль оновлено!",
  "ro": "Parolă actualizată!",
  "tl": "Na-update na ang password!"
},



'checkYouremail':{
  "en": "Check your email",
  "fr": "Vérifiez votre e-mail",
  "de": "Überprüfen Sie Ihre E-Mail",
  "ar": "تحقق من بريدك الإلكتروني",
  "es": "Revisa tu correo electrónico",
  "tr": "E-postanızı kontrol edin",
  "nl": "Controleer je e-mail",
  "it": "Controlla la tua email",
  "ja": "メールを確認してください",
  "zh": "请检查您的电子邮件",
  "ko": "이메일을 확인하세요",
  "hi": "अपना ईमेल जांचें",
  "pt": "Verifique seu e-mail",
  "ru": "Проверьте свою электронную почту",
  "sw": "Angalia barua pepe yako",
  "pl": "Sprawdź swoją pocztę e-mail",
  "id": "Periksa email Anda",
  "fa": "ایمیل خود را بررسی کنید",
  "pa": "ਆਪਣਾ ਈਮੇਲ ਚੈਕ ਕਰੋ",
  "uk": "Перевірте свою електронну пошту",
  "ro": "Verifică-ți e-mailul",
  "tl": "Suriin ang iyong email"
},



"profileUpdated":{
  "en": "Profile Updated!",
  "fr": "Profil mis à jour !",
  "de": "Profil aktualisiert!",
  "ar": "تم تحديث الملف الشخصي!",
  "es": "¡Perfil actualizado!",
  "tr": "Profil güncellendi!",
  "nl": "Profiel bijgewerkt!",
  "it": "Profilo aggiornato!",
  "ja": "プロフィールが更新されました！",
  "zh": "个人资料已更新！",
  "ko": "프로필이 업데이트되었습니다!",
  "hi": "प्रोफ़ाइल अपडेट हो गया!",
  "pt": "Perfil atualizado!",
  "ru": "Профиль обновлён!",
  "sw": "Wasifu umesasishwa!",
  "pl": "Profil zaktualizowany!",
  "id": "Profil diperbarui!",
  "fa": "پروفایل به‌روزرسانی شد!",
  "pa": "ਪ੍ਰੋਫਾਈਲ ਅਪਡੇਟ ਹੋ ਗਿਆ!",
  "uk": "Профіль оновлено!",
  "ro": "Profil actualizat!",
  "tl": "Na-update na ang profile!"
},


"partOfPremium":{
  "en": "You're now part of NEWSWORLD Premium",
  "fr": "Vous faites désormais partie de NEWSWORLD Premium",
  "de": "Sie sind jetzt Teil von NEWSWORLD Premium",
  "ar": "أنت الآن جزء من NEWSWORLD Premium",
  "es": "Ahora formas parte de NEWSWORLD Premium",
  "tr": "Artık NEWSWORLD Premium'un bir parçasısınız",
  "nl": "Je maakt nu deel uit van NEWSWORLD Premium",
  "it": "Ora fai parte di NEWSWORLD Premium",
  "ja": "あなたは現在、NEWSWORLD Premiumの一員です",
  "zh": "您现在已成为NEWSWORLD Premium的一员",
  "ko": "이제 NEWSWORLD Premium의 일원이 되었습니다",
  "hi": "अब आप NEWSWORLD Premium का हिस्सा हैं",
  "pt": "Agora você faz parte do NEWSWORLD Premium",
  "ru": "Теперь вы часть NEWSWORLD Premium",
  "sw": "Sasa wewe ni sehemu ya NEWSWORLD Premium",
  "pl": "Jesteś teraz częścią NEWSWORLD Premium",
  "id": "Anda sekarang bagian dari NEWSWORLD Premium",
  "fa": "اکنون شما بخشی از NEWSWORLD Premium هستید",
  "pa": "ਹੁਣ ਤੁਸੀਂ NEWSWORLD Premium ਦਾ ਹਿੱਸਾ ਹੋ",
  "uk": "Тепер ви частина NEWSWORLD Premium",
  "ro": "Acum faci parte din NEWSWORLD Premium",
  "tl": "Bahagi ka na ngayon ng NEWSWORLD Premium"
},


"livechannels":{
  "en": "CHANNELS",
  "fr": "CHAÎNES",
  "de": "KANÄLE",
  "ar": "القنوات",
  "es": "CANALES",
  "tr": "KANALLAR",
  "nl": "KANALEN",
  "it": "CANALI",
  "ja": "チャンネル",
  "zh": "频道",
  "ko": "채널",
  "hi": "चैनल",
  "pt": "CANAIS",
  "ru": "КАНАЛЫ",
  "sw": "VITUO",
  "pl": "KANAŁY",
  "id": "SALURAN",
  "fa": "کانال‌ها",
  "pa": "ਚੈਨਲ",
  "uk": "КАНАЛИ",
  "ro": "CANALE",
  "tl": "MGA CHANNEL"
},

"wrongPassword":{
  "en": "Wrong password",
  "fr": "Mot de passe incorrect",
  "de": "Falsches Passwort",
  "ar": "كلمة المرور غير صحيحة",
  "es": "Contraseña incorrecta",
  "tr": "Yanlış şifre",
  "nl": "Onjuist wachtwoord",
  "it": "Password errata",
  "ja": "パスワードが間違っています",
  "zh": "密码错误",
  "ko": "잘못된 비밀번호입니다",
  "hi": "गलत पासवर्ड",
  "pt": "Senha incorreta",
  "ru": "Неверный пароль",
  "sw": "Nenosiri si sahihi",
  "pl": "Nieprawidłowe hasło",
  "id": "Kata sandi salah",
  "fa": "رمز عبور اشتباه است",
  "pa": "ਗਲਤ ਪਾਸਵਰਡ",
  "uk": "Неправильний пароль",
  "ro": "Parolă incorectă",
  "tl": "Maling password"
},


"getPremium":{
  "en": "Upgrade to Premium to use this feature",
  "fr": "Passez à Premium pour utiliser cette fonctionnalité",
  "de": "Upgrade auf Premium, um diese Funktion zu nutzen",
  "ar": "قم بالترقية إلى بريميوم لاستخدام هذه الميزة",
  "es": "Actualiza a Premium para usar esta función",
  "tr": "Bu özelliği kullanmak için Premium'a yükseltin",
  "nl": "Upgrade naar Premium om deze functie te gebruiken",
  "it": "Passa a Premium per usare questa funzione",
  "ja": "この機能を使用するにはプレミアムにアップグレードしてください",
  "zh": "升级到高级版以使用此功能",
  "ko": "이 기능을 사용하려면 프리미엄으로 업그레이드하세요",
  "hi": "इस सुविधा का उपयोग करने के लिए प्रीमियम में अपग्रेड करें",
  "pt": "Faça upgrade para o Premium para usar este recurso",
  "ru": "Обновитесь до Премиум, чтобы использовать эту функцию",
  "sw": "Boresha hadi Premium ili kutumia kipengele hiki",
  "pl": "Przejdź na Premium, aby korzystać z tej funkcji",
  "id": "Upgrade ke Premium untuk menggunakan fitur ini",
  "fa": "برای استفاده از این ویژگی به پریمیوم ارتقا دهید",
  "pa": "ਇਸ ਫੀਚਰ ਨੂੰ ਵਰਤਣ ਲਈ ਪ੍ਰੀਮਿਅਮ ਵਿੱਚ ਅਪਗ੍ਰੇਡ ਕਰੋ",
  "uk": "Оновіться до Преміум, щоб користуватися цією функцією",
  "ro": "Fă upgrade la Premium pentru a folosi această funcție",
  "tl": "Mag-upgrade sa Premium para magamit ang feature na ito"
},


"limitReach":{
  "en": "Daily limit reached, continue at 12am",
  "fr": "Limite quotidienne atteinte, reprenez à minuit",
  "de": "Tägliches Limit erreicht, fahre um 0 Uhr fort",
  "ar": "تم الوصول إلى الحد اليومي، تابع عند الساعة 12 صباحًا",
  "es": "Límite diario alcanzado, continúa a las 12 a. m.",
  "tr": "Günlük limite ulaşıldı, saat 00:00'da devam edin",
  "nl": "Dagelijkse limiet bereikt, ga verder om 00:00 uur",
  "it": "Limite giornaliero raggiunto, continua a mezzanotte",
  "ja": "1日の制限に達しました。午前0時に再開してください",
  "zh": "已达到每日限制，请在凌晨12点继续",
  "ko": "일일 한도에 도달했습니다. 자정에 다시 이용하세요",
  "hi": "दैनिक सीमा पूरी हो गई है, रात 12 बजे फिर से जारी रखें",
  "pt": "Limite diário atingido, continue às 00:00",
  "ru": "Достигнут дневной лимит, продолжите в 00:00",
  "sw": "Kikomo cha kila siku kimefikiwa, endelea saa 12 asubuhi",
  "pl": "Osiągnięto dzienny limit, kontynuuj o północy",
  "id": "Batas harian tercapai, lanjutkan pukul 00.00",
  "fa": "محدودیت روزانه رسید، ساعت ۱۲ شب ادامه دهید",
  "pa": "ਰੋਜ਼ਾਨਾ ਸੀਮਾ ਪੂਰੀ ਹੋ ਗਈ ਹੈ, ਰਾਤ 12 ਵਜੇ ਜਾਰੀ ਰੱਖੋ",
  "uk": "Досягнуто денного ліміту, продовжуйте о 00:00",
  "ro": "Limita zilnică a fost atinsă, continuă la ora 00:00",
  "tl": "Naabot na ang pang-araw-araw na limit, magpatuloy sa 12am"
},

"logOutDevice":{
  "en": "Log out from other device",
  "fr": "Se déconnecter de l'autre appareil",
  "de": "Von anderem Gerät abmelden",
  "ar": "تسجيل الخروج من جهاز آخر",
  "es": "Cerrar sesión en otro dispositivo",
  "tr": "Diğer cihazdan çıkış yap",
  "nl": "Afmelden van ander apparaat",
  "it": "Disconnettersi da un altro dispositivo",
  "ja": "他のデバイスからログアウト",
  "zh": "从其他设备登出",
  "ko": "다른 기기에서 로그아웃",
  "hi": "अन्य डिवाइस से लॉग आउट करें",
  "pt": "Sair de outro dispositivo",
  "ru": "Выйти с другого устройства",
  "sw": "Ondoka kwenye kifaa kingine",
  "pl": "Wyloguj się z innego urządzenia",
  "id": "Keluar dari perangkat lain",
  "fa": "خروج از دستگاه دیگر",
  "pa": "ਹੋਰ ਡਿਵਾਈਸ ਤੋਂ ਲੌਗ ਆਉਟ ਕਰੋ",
  "uk": "Вийти з іншого пристрою",
  "ro": "Deconectează-te de pe alt dispozitiv",
  "tl": "Mag-log out mula sa ibang device"
},



"translatingContent":{
  "en": "Translating to {label}",
  "fr": "Traduction vers {label}",
  "de": "Übersetzung in {label}",
  "ar": "جارٍ الترجمة إلى {label}",
  "es": "Traduciendo a {label}",
  "tr": "{label} diline çevriliyor",
  "nl": "Vertalen naar {label}",
  "it": "Traduzione in {label}",
  "ja": "{label}に翻訳中",
  "zh": "正在翻译为{label}",
  "ko": "{label}(으)로 번역 중",
  "hi": "{label} में अनुवाद हो रहा है",
  "pt": "Traduzindo para {label}",
  "ru": "Перевод на {label}",
  "sw": "Inatafsiri kwenda {label}",
  "pl": "Tłumaczenie na {label}",
  "id": "Menerjemahkan ke {label}",
  "fa": "در حال ترجمه به {label}",
  "pa": "{label} ਵਿੱਚ ਅਨੁਵਾਦ ਕੀਤਾ ਜਾ ਰਿਹਾ ਹੈ",
  "uk": "Переклад на {label}",
  "ro": "Se traduce în {label}",
  "tl": "Isinasalin sa {label}"
},


"encode":{
  "en": "Encoding audio stream...",
  "fr": "Encodage du flux audio...",
  "de": "Audiostream wird codiert...",
  "ar": "جارٍ ترميز البث الصوتي...",
  "es": "Codificando flujo de audio...",
  "tr": "Ses akışı kodlanıyor...",
  "nl": "Audiostream wordt gecodeerd...",
  "it": "Codifica del flusso audio...",
  "ja": "音声ストリームをエンコード中...",
  "zh": "正在编码音频流...",
  "ko": "오디오 스트림 인코딩 중...",
  "hi": "ऑडियो स्ट्रीम एन्कोड की जा रही है...",
  "pt": "Codificando fluxo de áudio...",
  "ru": "Кодирование аудиопотока...",
  "sw": "Inasimbua mkondo wa sauti...",
  "pl": "Kodowanie strumienia audio...",
  "id": "Mengode aliran audio...",
  "fa": "در حال رمزگذاری جریان صوتی...",
  "pa": "ਆਡੀਓ ਸਟ੍ਰੀਮ ਐਨਕੋਡ ਕੀਤੀ ਜਾ ਰਹੀ ਹੈ...",
  "uk": "Кодування аудіопотоку...",
  "ro": "Codificare flux audio...",
  "tl": "Ini-encode ang audio stream..."
},


"deletion":{
  "en": "Your account has been successfully deleted",
  "fr": "Votre compte a été supprimé avec succès",
  "de": "Ihr Konto wurde erfolgreich gelöscht",
  "ar": "تم حذف حسابك بنجاح",
  "es": "Tu cuenta ha sido eliminada con éxito",
  "tr": "Hesabınız başarıyla silindi",
  "nl": "Uw account is succesvol verwijderd",
  "it": "Il tuo account è stato eliminato con successo",
  "ja": "あなたのアカウントは正常に削除されました",
  "zh": "您的账户已成功删除",
  "ko": "귀하의 계정이 성공적으로 삭제되었습니다",
  "hi": "आपका खाता सफलतापूर्वक हटा दिया गया है",
  "pt": "Sua conta foi excluída com sucesso",
  "ru": "Ваш аккаунт был успешно удалён",
  "sw": "Akaunti yako imefutwa kwa mafanikio",
  "pl": "Twoje konto zostało pomyślnie usunięte",
  "id": "Akun Anda berhasil dihapus",
  "fa": "حساب شما با موفقیت حذف شد",
  "pa": "ਤੁਹਾਡਾ ਖਾਤਾ ਸਫਲਤਾਪੂਰਵਕ ਮਿਟਾ ਦਿੱਤਾ ਗਿਆ ਹੈ",
  "uk": "Ваш обліковий запис успішно видалено",
  "ro": "Contul tău a fost șters cu succes",
  "tl": "Matagumpay na natanggal ang iyong account"
},

"processPayment":{
  "en": "Your payment is being processed. Please wait",
  "fr": "Votre paiement est en cours de traitement. Veuillez patienter",
  "de": "Ihre Zahlung wird bearbeitet. Bitte warten Sie",
  "ar": "جارٍ معالجة دفعتك. يرجى الانتظار",
  "es": "Tu pago está siendo procesado. Por favor espera",
  "tr": "Ödemeniz işleniyor. Lütfen bekleyin",
  "nl": "Uw betaling wordt verwerkt. Even geduld alstublieft",
  "it": "Il tuo pagamento è in fase di elaborazione. Attendere prego",
  "ja": "お支払いを処理中です。しばらくお待ちください",
  "zh": "您的付款正在处理中。请稍候",
  "ko": "결제가 처리 중입니다. 잠시만 기다려 주세요",
  "hi": "आपका भुगतान संसाधित किया जा रहा है। कृपया प्रतीक्षा करें",
  "pt": "Seu pagamento está sendo processado. Aguarde",
  "ru": "Ваш платеж обрабатывается. Пожалуйста, подождите",
  "sw": "Malipo yako yanachakatwa. Tafadhali subiri",
  "pl": "Twoja płatność jest przetwarzana. Proszę czekać",
  "id": "Pembayaran Anda sedang diproses. Mohon tunggu",
  "fa": "پرداخت شما در حال پردازش است. لطفاً صبر کنید",
  "pa": "ਤੁਹਾਡੀ ਭੁਗਤਾਨ ਪ੍ਰਕਿਰਿਆ ਜਾਰੀ ਹੈ। ਕਿਰਪਾ ਕਰਕੇ ਉਡੀਕ ਕਰੋ",
  "uk": "Ваш платіж обробляється. Будь ласка, зачекайте",
  "ro": "Plata ta este în curs de procesare. Te rugăm să aștepți",
  "tl": "Pinoproseso ang iyong bayad. Mangyaring maghintay"
},

"renewInfo":{
  "en": "Subscriptions renew automatically unless cancelled 24 hours before the end of the period. Manage in Apple ID settings.",
  "fr": "Les abonnements se renouvellent automatiquement sauf annulation au moins 24 heures avant la fin de la période. Gérez dans les réglages de votre identifiant Apple.",
  "de": "Abonnements verlängern sich automatisch, sofern sie nicht mindestens 24 Stunden vor Ablauf des Zeitraums gekündigt werden. Verwalten Sie dies in den Apple-ID-Einstellungen.",
  "ar": "يتم تجديد الاشتراكات تلقائيًا ما لم يتم إلغاؤها قبل 24 ساعة من نهاية الفترة. يمكنك إدارتها من إعدادات معرف Apple.",
  "es": "Las suscripciones se renuevan automáticamente a menos que se cancelen al menos 24 horas antes del final del período. Adminístralas en la configuración de Apple ID.",
  "tr": "Abonelikler, dönem bitiminden en az 24 saat önce iptal edilmezse otomatik olarak yenilenir. Apple Kimliği ayarlarından yönetin.",
  "nl": "Abonnementen worden automatisch verlengd, tenzij ze ten minste 24 uur vóór het einde van de periode worden geannuleerd. Beheer dit via uw Apple ID-instellingen.",
  "it": "Gli abbonamenti si rinnovano automaticamente a meno che non vengano annullati almeno 24 ore prima della fine del periodo. Gestisci nelle impostazioni ID Apple.",
  "ja": "サブスクリプションは、期間終了の24時間前までにキャンセルされない限り自動更新されます。Apple ID設定で管理できます。",
  "zh": "订阅会自动续订，除非在周期结束前至少24小时取消。可在Apple ID设置中管理。",
  "ko": "구독은 기간 종료 최소 24시간 전에 취소하지 않으면 자동으로 갱신됩니다. Apple ID 설정에서 관리하세요.",
  "hi": "सदस्यताएँ अवधि समाप्त होने से कम से कम 24 घंटे पहले रद्द न किए जाने पर स्वतः नवीनीकृत हो जाती हैं। Apple ID सेटिंग्स में प्रबंधित करें।",
  "pt": "As assinaturas são renovadas automaticamente, a menos que sejam canceladas pelo menos 24 horas antes do final do período. Gerencie nas configurações do ID Apple.",
  "ru": "Подписки автоматически продлеваются, если не отменены как минимум за 24 часа до окончания периода. Управляйте в настройках Apple ID.",
  "sw": "Usajili hujisasisha kiotomatiki isipositishwa angalau saa 24 kabla ya mwisho wa kipindi. Dhibiti katika mipangilio ya Apple ID.",
  "pl": "Subskrypcje odnawiają się automatycznie, chyba że zostaną anulowane co najmniej 24 godziny przed końcem okresu. Zarządzaj w ustawieniach Apple ID.",
  "id": "Langganan akan diperpanjang secara otomatis kecuali dibatalkan setidaknya 24 jam sebelum akhir periode. Kelola di pengaturan Apple ID.",
  "fa": "اشتراک‌ها به‌طور خودکار تمدید می‌شوند مگر اینکه حداقل ۲۴ ساعت قبل از پایان دوره لغو شوند. در تنظیمات Apple ID مدیریت کنید.",
  "pa": "ਸਬਸਕ੍ਰਿਪਸ਼ਨ ਆਪਣੇ ਆਪ ਨਵੀਕਰਿਤ ਹੋ ਜਾਂਦੇ ਹਨ ਜੇਕਰ ਮਿਆਦ ਖਤਮ ਹੋਣ ਤੋਂ ਘੱਟੋ-ਘੱਟ 24 ਘੰਟੇ ਪਹਿਲਾਂ ਰੱਦ ਨਾ ਕੀਤੇ ਜਾਣ। Apple ID ਸੈਟਿੰਗਜ਼ ਵਿੱਚ ਪ੍ਰਬੰਧਿਤ ਕਰੋ।",
  "uk": "Підписки автоматично поновлюються, якщо їх не скасувати щонайменше за 24 години до закінчення періоду. Керуйте в налаштуваннях Apple ID.",
  "ro": "Abonamentele se reînnoiesc automat dacă nu sunt anulate cu cel puțin 24 de ore înainte de sfârșitul perioadei. Gestionează în setările Apple ID.",
  "tl": "Awtomatikong nagre-renew ang mga subscription maliban kung kanselahin nang hindi bababa sa 24 oras bago matapos ang panahon. Pamahalaan sa Apple ID settings."
},


"restorePurchase":{
  "en": "Restore Purchases",
  "fr": "Restaurer les achats",
  "de": "Käufe wiederherstellen",
  "ar": "استعادة المشتريات",
  "es": "Restaurar compras",
  "tr": "Satın alımları geri yükle",
  "nl": "Aankopen herstellen",
  "it": "Ripristina acquisti",
  "ja": "購入を復元",
  "zh": "恢复购买",
  "ko": "구매 복원",
  "hi": "खरीदारी पुनर्स्थापित करें",
  "pt": "Restaurar compras",
  "ru": "Восстановить покупки",
  "sw": "Rejesha ununuzi",
  "pl": "Przywróć zakupy",
  "id": "Pulihkan pembelian",
  "fa": "بازیابی خریدها",
  "pa": "ਖਰੀਦਾਂ ਮੁੜ ਪ੍ਰਾਪਤ ਕਰੋ",
  "uk": "Відновити покупки",
  "ro": "Restabilește achizițiile",
  "tl": "Ibalik ang mga binili"
},


"manageSubscription":{
  "en": "Manage Subscription",
  "fr": "Gérer l’abonnement",
  "de": "Abonnement verwalten",
  "ar": "إدارة الاشتراك",
  "es": "Gestionar suscripción",
  "tr": "Aboneliği yönet",
  "nl": "Abonnement beheren",
  "it": "Gestisci abbonamento",
  "ja": "サブスクリプションを管理",
  "zh": "管理订阅",
  "ko": "구독 관리",
  "hi": "सदस्यता प्रबंधित करें",
  "pt": "Gerenciar assinatura",
  "ru": "Управление подпиской",
  "sw": "Dhibiti usajili",
  "pl": "Zarządzaj subskrypcją",
  "id": "Kelola langganan",
  "fa": "مدیریت اشتراک",
  "pa": "ਸਬਸਕ੍ਰਿਪਸ਼ਨ ਪ੍ਰਬੰਧਿਤ ਕਰੋ",
  "uk": "Керувати підпискою",
  "ro": "Gestionează abonamentul",
  "tl": "Pamahalaan ang subscription"
},

"noActive":{
  "en": "No active subscription",
  "fr": "Aucun abonnement actif",
  "de": "Kein aktives Abonnement",
  "ar": "لا يوجد اشتراك نشط",
  "es": "No hay suscripción activa",
  "tr": "Aktif abonelik yok",
  "nl": "Geen actief abonnement",
  "it": "Nessun abbonamento attivo",
  "ja": "有効なサブスクリプションはありません",
  "zh": "没有有效的订阅",
  "ko": "활성 구독이 없습니다",
  "hi": "कोई सक्रिय सदस्यता नहीं है",
  "pt": "Nenhuma assinatura ativa",
  "ru": "Нет активной подписки",
  "sw": "Hakuna usajili unaotumika",
  "pl": "Brak aktywnej subskrypcji",
  "id": "Tidak ada langganan aktif",
  "fa": "هیچ اشتراک فعالی وجود ندارد",
  "pa": "ਕੋਈ ਸਰਗਰਮ ਸਬਸਕ੍ਰਿਪਸ਼ਨ ਨਹੀਂ",
  "uk": "Немає активної підписки",
  "ro": "Niciun abonament activ",
  "tl": "Walang aktibong subscription"
},


"privacyPolicy":{
  "en": "Privacy Policy",
  "fr": "Politique de confidentialité",
  "de": "Datenschutzrichtlinie",
  "ar": "سياسة الخصوصية",
  "es": "Política de privacidad",
  "tr": "Gizlilik Politikası",
  "nl": "Privacybeleid",
  "it": "Informativa sulla privacy",
  "ja": "プライバシーポリシー",
  "zh": "隐私政策",
  "ko": "개인정보 처리방침",
  "hi": "गोपनीयता नीति",
  "pt": "Política de Privacidade",
  "ru": "Политика конфиденциальности",
  "sw": "Sera ya Faragha",
  "pl": "Polityka prywatności",
  "id": "Kebijakan Privasi",
  "fa": "سیاست حفظ حریم خصوصی",
  "pa": "ਗੋਪਨੀਯਤਾ ਨੀਤੀ",
  "uk": "Політика конфіденційності",
  "ro": "Politica de confidențialitate",
  "tl": "Patakaran sa Privacy"
},


"TermsOfUse":{
  "en": "Terms of Use",
  "fr": "Conditions d’utilisation",
  "de": "Nutzungsbedingungen",
  "ar": "شروط الاستخدام",
  "es": "Términos de uso",
  "tr": "Kullanım Şartları",
  "nl": "Gebruiksvoorwaarden",
  "it": "Termini di utilizzo",
  "ja": "利用規約",
  "zh": "使用条款",
  "ko": "이용 약관",
  "hi": "उपयोग की शर्तें",
  "pt": "Termos de Uso",
  "ru": "Условия использования",
  "sw": "Masharti ya Matumizi",
  "pl": "Warunki użytkowania",
  "id": "Syarat Penggunaan",
  "fa": "شرایط استفاده",
  "pa": "ਵਰਤੋਂ ਦੀਆਂ ਸ਼ਰਤਾਂ",
  "uk": "Умови використання",
  "ro": "Termeni de utilizare",
  "tl": "Mga Tuntunin sa Paggamit"
},

'betterExperience':{
  "en": "Create an account for a better experience",
  "fr": "Créez un compte pour une meilleure expérience",
  "de": "Erstellen Sie ein Konto für ein besseres Erlebnis",
  "ar": "أنشئ حسابًا لتجربة أفضل",
  "es": "Crea una cuenta para una mejor experiencia",
  "tr": "Daha iyi bir deneyim için hesap oluştur",
  "nl": "Maak een account aan voor een betere ervaring",
  "it": "Crea un account per un’esperienza migliore",
  "ja": "より良い体験のためにアカウントを作成してください",
  "zh": "创建账户以获得更好的体验",
  "ko": "더 나은 경험을 위해 계정을 만드세요",
  "hi": "बेहतर अनुभव के लिए खाता बनाएं",
  "pt": "Crie uma conta para uma melhor experiência",
  "ru": "Создайте аккаунт для лучшего опыта",
  "sw": "Unda akaunti kwa matumizi bora",
  "pl": "Utwórz konto, aby uzyskać lepsze doświadczenie",
  "id": "Buat akun untuk pengalaman yang lebih baik",
  "fa": "برای تجربه بهتر یک حساب ایجاد کنید",
  "pa": "ਬਿਹਤਰ ਅਨੁਭਵ ਲਈ ਖਾਤਾ ਬਣਾਓ",
  "uk": "Створіть обліковий запис для кращого досвіду",
  "ro": "Creează un cont pentru o experiență mai bună",
  "tl": "Gumawa ng account para sa mas magandang karanasan"
},

"guest":{
  "en": "Guest",
  "fr": "Invité",
  "de": "Gast",
  "ar": "زائر",
  "es": "Invitado",
  "tr": "Misafir",
  "nl": "Gast",
  "it": "Ospite",
  "ja": "ゲスト",
  "zh": "访客",
  "ko": "게스트",
  "hi": "अतिथि",
  "pt": "Convidado",
  "ru": "Гость",
  "sw": "Mgeni",
  "pl": "Gość",
  "id": "Tamu",
  "fa": "مهمان",
  "pa": "ਮਹਿਮਾਨ",
  "uk": "Гість",
  "ro": "Oaspete",
  "tl": "Bisita"
},

"createUseFeature":{
  "en": "Create your account to use this feature",
  "fr": "Créez votre compte pour utiliser cette fonctionnalité",
  "de": "Erstellen Sie ein Konto, um diese Funktion zu nutzen",
  "ar": "أنشئ حسابك لاستخدام هذه الميزة",
  "es": "Crea tu cuenta para usar esta función",
  "tr": "Bu özelliği kullanmak için bir hesap oluştur",
  "nl": "Maak een account aan om deze functie te gebruiken",
  "it": "Crea il tuo account per utilizzare questa funzione",
  "ja": "この機能を使用するにはアカウントを作成してください",
  "zh": "创建账户以使用此功能",
  "ko": "이 기능을 사용하려면 계정을 만드세요",
  "hi": "इस सुविधा का उपयोग करने के लिए अपना खाता बनाएं",
  "pt": "Crie sua conta para usar este recurso",
  "ru": "Создайте аккаунт, чтобы использовать эту функцию",
  "sw": "Unda akaunti yako ili kutumia kipengele hiki",
  "pl": "Utwórz konto, aby skorzystać z tej funkcji",
  "id": "Buat akun Anda untuk menggunakan fitur ini",
  "fa": "برای استفاده از این قابلیت حساب خود را ایجاد کنید",
  "pa": "ਇਸ ਫੀਚਰ ਨੂੰ ਵਰਤਣ ਲਈ ਆਪਣਾ ਖਾਤਾ ਬਣਾਓ",
  "uk": "Створіть обліковий запис, щоб використовувати цю функцію",
  "ro": "Creează-ți contul pentru a utiliza această funcție",
  "tl": "Gumawa ng iyong account upang magamit ang feature na ito"
},

'warning':{
  "en": "Help us keep our community safe. We review all reported comments and take prompt action when necessary. To report a comment, go to Settings > Support and select “Report Inappropriate Comments.”",
  "fr": "Aidez-nous à maintenir une communauté sûre. Nous examinons tous les commentaires signalés et prenons des mesures rapides si nécessaire. Pour signaler un commentaire, allez dans Paramètres > Assistance et sélectionnez « Signaler des commentaires inappropriés ».",
  "de": "Helfen Sie uns, unsere Community sicher zu halten. Wir überprüfen alle gemeldeten Kommentare und ergreifen bei Bedarf umgehend Maßnahmen. Um einen Kommentar zu melden, gehen Sie zu Einstellungen > Support und wählen Sie „Unangemessene Kommentare melden“.",
  "ar": "ساعدنا في الحفاظ على مجتمع آمن. نقوم بمراجعة جميع التعليقات المبلغ عنها ونتخذ الإجراءات اللازمة بسرعة عند الحاجة. للإبلاغ عن تعليق، انتقل إلى الإعدادات > الدعم واختر «الإبلاغ عن التعليقات غير المناسبة».",
  "es": "Ayúdanos a mantener nuestra comunidad segura. Revisamos todos los comentarios reportados y tomamos medidas rápidamente cuando es necesario. Para reportar un comentario, ve a Configuración > Soporte y selecciona «Reportar comentarios inapropiados».",
  "tr": "Topluluğumuzu güvenli tutmamıza yardımcı olun. Bildirilen tüm yorumları inceler ve gerektiğinde hızlıca işlem yaparız. Bir yorumu bildirmek için Ayarlar > Destek bölümüne gidin ve “Uygunsuz yorumları bildir” seçeneğini seçin.",
  "nl": "Help ons onze community veilig te houden. We beoordelen alle gemelde reacties en ondernemen indien nodig snel actie. Ga naar Instellingen > Ondersteuning en selecteer ‘Ongepaste reacties melden’ om een reactie te melden.",
  "it": "Aiutaci a mantenere la nostra community sicura. Esaminiamo tutti i commenti segnalati e interveniamo tempestivamente quando necessario. Per segnalare un commento, vai su Impostazioni > Supporto e seleziona «Segnala commenti inappropriati».",
  "ja": "コミュニティを安全に保つためにご協力ください。報告されたすべてのコメントを確認し、必要に応じて迅速に対応します。コメントを報告するには、「設定 > サポート」から「不適切なコメントを報告」を選択してください。",
  "zh": "帮助我们保持社区安全。我们会审核所有被举报的评论，并在必要时迅速采取行动。要举报评论，请前往“设置 > 支持”，选择“举报不当评论”。",
  "ko": "커뮤니티를 안전하게 유지하는 데 도움을 주세요. 신고된 모든 댓글을 검토하고 필요한 경우 신속하게 조치합니다. 댓글을 신고하려면 설정 > 지원으로 이동하여 “부적절한 댓글 신고”를 선택하세요.",
  "hi": "हमारे समुदाय को सुरक्षित रखने में मदद करें। हम सभी रिपोर्ट की गई टिप्पणियों की समीक्षा करते हैं और आवश्यकता पड़ने पर तुरंत कार्रवाई करते हैं। किसी टिप्पणी की रिपोर्ट करने के लिए सेटिंग्स > सपोर्ट पर जाएं और “अनुचित टिप्पणियों की रिपोर्ट करें” चुनें।",
  "pt": "Ajude-nos a manter nossa comunidade segura. Revisamos todos os comentários denunciados e tomamos medidas rápidas quando necessário. Para denunciar um comentário, vá em Configurações > Suporte e selecione «Denunciar comentários inadequados».",
  "ru": "Помогите нам поддерживать безопасное сообщество. Мы проверяем все сообщения о комментариях и при необходимости оперативно принимаем меры. Чтобы сообщить о комментарии, перейдите в «Настройки > Поддержка» и выберите «Сообщить о неподобающих комментариях».",
  "sw": "Tusaidie kudumisha jamii salama. Tunakagua maoni yote yaliyoripotiwa na kuchukua hatua haraka inapohitajika. Kuripoti maoni, nenda kwenye Mipangilio > Msaada na uchague “Ripoti maoni yasiyofaa”.",
  "pl": "Pomóż nam utrzymać bezpieczną społeczność. Sprawdzamy wszystkie zgłoszone komentarze i w razie potrzeby szybko podejmujemy działania. Aby zgłosić komentarz, przejdź do Ustawienia > Wsparcie i wybierz „Zgłoś nieodpowiednie komentarze”.",
  "id": "Bantu kami menjaga komunitas tetap aman. Kami meninjau semua komentar yang dilaporkan dan mengambil tindakan dengan cepat bila diperlukan. Untuk melaporkan komentar, buka Pengaturan > Dukungan dan pilih “Laporkan komentar yang tidak pantas”.",
  "fa": "به ما کمک کنید جامعه‌ای ایمن داشته باشیم. ما همه نظرات گزارش‌شده را بررسی کرده و در صورت لزوم سریعاً اقدام می‌کنیم. برای گزارش یک نظر، به تنظیمات > پشتیبانی بروید و «گزارش نظرات نامناسب» را انتخاب کنید.",
  "pa": "ਸਾਡੇ ਸਮੂਹ ਨੂੰ ਸੁਰੱਖਿਅਤ ਰੱਖਣ ਵਿੱਚ ਸਾਡੀ ਮਦਦ ਕਰੋ। ਅਸੀਂ ਸਾਰੀਆਂ ਰਿਪੋਰਟ ਕੀਤੀਆਂ ਟਿੱਪਣੀਆਂ ਦੀ ਸਮੀਖਿਆ ਕਰਦੇ ਹਾਂ ਅਤੇ ਲੋੜ ਪੈਣ ‘ਤੇ ਤੁਰੰਤ ਕਾਰਵਾਈ ਕਰਦੇ ਹਾਂ। ਟਿੱਪਣੀ ਦੀ ਰਿਪੋਰਟ ਕਰਨ ਲਈ ਸੈਟਿੰਗਜ਼ > ਸਹਾਇਤਾ ‘ਤੇ ਜਾਓ ਅਤੇ “ਗੈਰ-ਉਚਿਤ ਟਿੱਪਣੀਆਂ ਦੀ ਰਿਪੋਰਟ ਕਰੋ” ਚੁਣੋ।",
  "uk": "Допоможіть нам підтримувати безпечну спільноту. Ми перевіряємо всі повідомлені коментарі та за потреби швидко вживаємо заходів. Щоб повідомити про коментар, перейдіть у Налаштування > Підтримка та оберіть «Повідомити про неприйнятні коментарі».",
  "ro": "Ajută-ne să menținem comunitatea în siguranță. Revizuim toate comentariile raportate și luăm măsuri rapide atunci când este necesar. Pentru a raporta un comentariu, mergi la Setări > Suport și selectează «Raportează comentarii necorespunzătoare».",
  "tl": "Tulungan kaming panatilihing ligtas ang aming komunidad. Sinusuri namin ang lahat ng naiulat na komento at agad kaming kumikilos kung kinakailangan. Upang mag-report ng komento, pumunta sa Settings > Support at piliin ang “I-report ang hindi angkop na komento”."
}
























}




