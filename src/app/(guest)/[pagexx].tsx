...
const { pagexx } = useLocalSearchParams()
...
settranstext({title:result.lineA , desc:result.lineB,content:result.lineC,summary:result.lineD === 'undefined' ? '':result.lineD})
...
<TouchableOpacity style={styles.rowBboxi} onPress={showGuest}>

<AppIcon name={shouldSave ? activeImage : inactiveImage} size={typo.h1_8} />

</TouchableOpacity>

<TouchableOpacity style={styles.rowBbox} onPress={showGuest} >
<AppIcon name={placeholderV} size={typo.h1_8}/>
</TouchableOpacity>

<TouchableOpacity style={styles.rowBboxii} onPress={showGuest}>
...
<Text allowFontScaling={false} style={[styles.textMR200,{lineHeight:typo.h1_5,fontSize:typo.h3,color:theme === 'dark' ? Colors.light.border :Colors.dark.primary }]}>{istransActive ? transtext.desc : result.description}</Text>
</View>


<View style={[styles.contentBox,{marginVertical:typo.h4,width:WIDTH - typo.h2}]}>
...
<View style={styles.threeA}></View>

<View style={styles.threeB}>

<TouchableOpacity style={styles.boxTwoi} onLongPress={() => shouldDisplay.value = false} onPress={showGuest}>
{
...
<KeyboardStickyView  offset={platform === 'ios' ? {closed:-40,opened:0}:{closed:-96,opened:0}} style={[styles.footer,{borderRadius:typo.h4,backgroundColor:theme === 'dark' ? Colors.dark.secondary : Colors.light.secondary,borderColor:theme === 'dark' ? Colors.dark.border : Colors.light.border}]}>
